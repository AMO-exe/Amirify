"""skillgen: render amirify's committed skill artifacts from edited fragments.
...
files under ``amirify/skill*.md`` and ``amirify/skills/<platform>/references/``
...
except ModuleNotFoundError:  # Python 3.10 - amirify supports >=3.10
...
# straight from git instead of from disk. claude's v8 body is amirify/skill.md;
# every other split host has its own amirify/skill-<host>.md. Auditing each host
...
def _v8_baseline_ref(platform_key: str) -> str:
    ...
    if platform_key == "claude":
        return f"{_V8_BASELINE_SHA}:amirify/skill.md"
    if platform_key == "agents":
        ...
        return f"{_V8_BASELINE_SHA}:amirify/skill-amp.md"
    return f"{_V8_BASELINE_SHA}:amirify/skill-{platform_key}.md"

# Immutable baseline for --always-on-roundtrip. The six always-on instruction
# blocks used to be triple-quoted constants in amirify/__main__.py; they are now
# packaged amirify/always_on/*.md files the module reads at load. This ref points
...
ALWAYS_ON_BASELINE_REF = f"{_V8_BASELINE_SHA}:amirify/__main__.py"

# The always-on instruction blocks: rendered-file basename -> the __main__.py
# constant it must reproduce. Rendered to amirify/always_on/<basename>.md from
...
ALWAYS_ON_SANCTIONED_EDITS: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    # invoke a literal `skill` tool with `skill: "amirify"`, which is
...
# always-on via `amirify claude install` (the shared body). The agents-md hosts
# (trae, trae-cn, amp) read AGENTS.md and wire it via `amirify <host> install`.
...
_TRAE_PRETOOLUSE_NOTE = (
    ...
    "rebuild on tool use. Run `/amirify --update` manually after code changes if "
...
_AGENTS_MD_HOOKS: dict[str, dict[str, str]] = {
    "trae": {
        "heading_suffix": " (Trae)",
        "host_display": "Trae",
        "install_block": "amirify trae install       # or: amirify trae-cn install",
        "uninstall_block": "amirify trae uninstall     # or: amirify trae-cn uninstall   # remove the section",
        "pretooluse_note": _TRAE_PRETOOLUSE_NOTE,
    },
    "amp": {
        "heading_suffix": "",
        "host_display": "Amp",
        "install_block": "amirify amp install",
        "uninstall_block": "amirify amp uninstall  # remove the section",
        "pretooluse_note": "",
    },
    "agents": {
        ...
        # pointing at `amirify agents install` (which wires AGENTS.md, like amp).
        "heading_suffix": "",
        "host_display": "your agent",
        "install_block": "amirify agents install",
        "uninstall_block": "amirify agents uninstall  # remove the section",
        "pretooluse_note": "",
    },
}
...
# them. Two classes:
#
# 1. SHARED_INTRO_ALLOWLIST — the lean intro consolidation. "## What amirify is
...
SHARED_INTRO_ALLOWLIST: frozenset[str] = frozenset({
    "## What amirify is for",  # lean intro; v8 hosts had verbose intro prose, no heading.
})
...
#   * the inline ``$(cat amirify-out/.amirify_python) -c "..."`` python blocks
#     become single-quoted here-strings piped to the interpreter's stdin
#     (``@'...'@ | & (Get-Content amirify-out\.amirify_python) -``). A
...
_PY_INVOKE_POSIX = '$(cat amirify-out/.amirify_python) -c "'
_PY_INVOKE_PS_OPEN = "@'"
_PY_INVOKE_PS_CLOSE = "'@ | & (Get-Content amirify-out\\.amirify_python) -"
_MKDIR_POSIX = "mkdir -p amirify-out"
_MKDIR_PS = "New-Item -ItemType Directory -Force -Path amirify-out | Out-Null"
_FIND_CHUNKS_POSIX = "find amirify-out -maxdepth 1 -name '.amirify_chunk_*.json' -delete 2>/dev/null"
_FIND_CHUNKS_PS = (
    "Get-ChildItem amirify-out -Filter '.amirify_chunk_*.json' -File "
...
def _translate_bash_block(lines: list[str]) -> list[str]:
    ...
    for line in lines:
        ...
        elif not line.strip() or line.lstrip().startswith("#") or line.startswith("amirify "):
            out.append(line)  # blank lines, comments, and amirify CLI calls are shell-neutral
        else:
...
def render_always_on() -> list[RenderedArtifact]:
    """Render the six always-on instruction blocks to amirify/always_on/*.md.
    ...
    for basename in sorted(ALWAYS_ON_BLOCKS):
        ...
        out.append(RenderedArtifact(f"amirify/always_on/{basename}.md", body))
    return out
...
def audit_coverage(platform: Platform) -> list[str]:
    ...
    The audit reads the host's OWN v8 skill body (amirify/skill.md for claude,
    amirify/skill-<host>.md otherwise) and checks that every v8 heading lands in
    ...
    v8 headings are exempt and documented as deltas, not holes:
      - waves 2-3 consolidations (the lean "## What amirify is for" intro and the
...
def _is_frontmatter_description_line(line: str) -> bool:
    ...
    The unified description (amirify #1106) rewrites the frontmatter
...
def _is_chunk_cleanup_line(line: str) -> bool:
    ...
    The bare glob ``.amirify_chunk_*.json`` in the v8 cleanup line aborts the
    whole ``rm`` under fish/zsh when no chunk files exist (no-match is a hard
    error there, unlike bash). The fix (amirify #1172) drops the glob from the
    ...
    Both the v8 form (bare ``.amirify_chunk_*.json`` glob, removed) and the fixed
    ...
    if not s.startswith("rm -f"):
        return False
    return ".amirify_chunk_*.json" in line or ("find " in line and "-name '.amirify_chunk_" in line)
...
def _is_cache_unlink_fix_line(line: str) -> bool:
    ...
    ``.amirify_cached.json`` for Part C to merge. The miss branch now deletes it.
    """
    return (
        ".amirify_cached.json').unlink(missing_ok=True)" in line
        or line.strip() == "else:"
        or "Always (re)write the cache file" in line
        or "stale .amirify_cached.json" in line
    )
...
def _is_zero_node_guard_fix_line(line: str) -> bool:
    ...
    return (
        ...
        or s == "if not wrote:"
        or "refused to shrink amirify-out/graph.json" in line
...
def _is_manifest_root_fix_line(line: str) -> bool:
    ...
    relativized to the scan root, matching the native ``amirify update`` path.
    Both the old bare call (removed) and the new rooted call (added) match here;
    the ``import`` guard avoids matching the ``from amirify.detect import
    save_manifest`` line.
    """
...
def _is_manifest_stamp_fix_line(line: str) -> bool:
    ...
    ``amirify extract`` path. The rooted ``save_manifest`` call itself is
...
def _is_no_api_key_fix_line(line: str) -> bool:
    ...
    stating that amirify needs no API key, and (like the subagent-host skills)
    ...
    """
    return "amirify needs no API key" in line
...
def _is_shebang_allowlist_fix_line(line: str) -> bool:
    ...
    detection fell through to a bare ``python3`` that lacked amirify. ``@`` is now
...
def _is_obsidian_usage_comment_line(line: str) -> bool:
    """Whether a line is part of the ``/amirify`` usage-comment fix (#1681).

    The Usage block's bare ``/amirify`` comment said "full pipeline on current
...
def _is_uv_from_interpreter_fix_line(line: str) -> bool:
    ...
    Step 1's POSIX interpreter probe ran ``uv tool run amirify python -c ...``,
    but ``amirify`` exposes its executable as ``amirify``, so uv treated
    ``python`` as a missing ``amirify`` command and the probe silently failed
    (the ``2>/dev/null`` swallowed uv's "use --from" hint), leaving PYTHON on a
    amirify-less system interpreter. The probe now runs
    ``uv tool run --from amirify python -c ...``. Both the old (removed) and new
    (added) forms match here.
    """
    return "uv tool run" in line and "amirify python" in line
...
def _is_semantic_cache_scope_fix_line(line: str) -> bool:
    ...
    return (
        ...
        and ".amirify_uncached.txt" in stripped
...
def _is_community_label_export_fix_line(line: str) -> bool:
    ...
    ``.amirify_labels.json`` sidecar carried the names). Step 5 now imports
    ...
    return (
        ...
        or stripped == "from amirify.export import to_json"
...
def always_on_roundtrip() -> list[str]:
    ...
    for basename, const_name in sorted(ALWAYS_ON_BLOCKS.items()):
        path = f"amirify/always_on/{basename}.md"
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        ...
        description="Render and guard amirify's committed skill artifacts.",
    )
