...
cat > "${PROJECT_ROOT}/amirify-out/.amirify_chunk_0N.json" <<'CHUNK_JSON'
PASTED_JSON
CHUNK_JSON
```

Repeat for every chunk. Each chunk's JSON must land in its own `amirify-out/.amirify_chunk_NN.json` before Step B3 runs.
