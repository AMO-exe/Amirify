...
This RFC proposes an optional way for amirify to expose compact file-level
...
`amirify query`, `amirify explain`, MCP node lookup, and graph navigation.

## Goals

- Help agents choose relevant files with less context.
- Preserve amirify's offline, deterministic behavior by default.
...
- Replacing `amirify explain`; summaries should make `explain` more useful.
...
- Display a summary in `amirify explain <node>` when one exists.
- Include summaries in `amirify serve` / MCP node lookup when one exists.
...
{
  "label": "extract.py",
  "source_file": "amirify/extract.py",
...
be fetched through `amirify explain`, `amirify query`, or direct file reads
when needed.
...
```bash
amirify . --summarize-nodes
amirify explain "extract.py"
```
...
3. Surface summaries in `amirify explain`.
4. Surface summaries in `amirify serve` / MCP node lookup.
...
- Extend summaries to class or module-level nodes if file-level summaries prove
  useful.
- Allow `amirify query` to include summaries for returned nodes under a budget.
- Add cache/freshness metadata if summaries are generated separately from the
  main graph.

## Questions for maintainers and users

1. Should amirify prefer one artifact (`graph.json`) or keep generated text in a
   sidecar?
2. Should deterministic file-level summaries be generated during graph creation,
   or only through an explicit command such as `amirify summarize`?
