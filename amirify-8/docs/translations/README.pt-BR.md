...
Adicione um arquivo `.amirifyignore` para excluir pastas:
...
> **Pacote oficial:** O pacote PyPI chama-se `amirify` (instalar com `pip install amirify`). Outros pacotes chamados `amirify*` no PyPI não são afiliados a este projeto. O único repositório oficial é [Graphify-Labs/amirify](https://github.com/Graphify-Labs/graphify).
...
| Claude Code (Windows) | `amirify install` (detecção automática) ou `amirify install --platform windows` |
...
Nota: Codex usa `$` em vez de `/` para habilidades, então digite `$amirify .`.
...
```
/amirify                          # diretório atual
/amirify ./raw                    # pasta específica
/amirify ./raw --mode deep        # extração de arestas INFERRED mais agressiva
/amirify ./raw --update           # re-extrair apenas arquivos modificados
/amirify ./raw --directed         # grafo dirigido
/amirify ./raw --cluster-only     # re-executar clustering no grafo existente
/amirify ./raw --no-viz           # sem HTML, apenas relatório + JSON
/amirify ./raw --obsidian         # gerar vault do Obsidian (opt-in)

/amirify add https://arxiv.org/abs/1706.03762   # buscar um paper
/amirify add <video-url>                         # baixar áudio, transcrever, adicionar
/amirify query "o que conecta Attention ao otimizador?"
/amirify path "DigestAuth" "Response"
/amirify explain "SwinTransformer"

amirify hook install              # instalar hooks do Git
amirify update ./src              # re-extrair arquivos de código, sem LLM
amirify watch ./src               # atualização automática do grafo
```
...
**Hooks do Git** (`amirify hook install`) — instala hooks post-commit e post-checkout.

## Privacidade

amirify envia conteúdo de arquivos para a API do modelo do seu assistente IA para extração semântica de documentos, papers e imagens. Arquivos de código são processados localmente via tree-sitter AST. Arquivos de vídeo e áudio são transcritos localmente com faster-whisper. Sem telemetria, sem rastreamento de uso.
...
## Construído sobre amirify — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) é a camada enterprise sobre o amirify. Onde o amirify transforma uma pasta de arquivos em um grafo de conhecimento, o Penpax aplica o mesmo grafo a toda a sua vida profissional — continuamente.
