...
**Навык для AI-ассистента по написанию кода.** Введите `/amirify` в Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Hermes, Kiro или Google Antigravity — он прочитает ваши файлы, построит граф знаний и вернёт вам структуру, о существовании которой вы не подозревали. Понимайте кодовую базу быстрее. Находите «почему» за архитектурными решениями.

Полностью мультимодальный. Добавляйте код, PDF, markdown, скриншоты, диаграммы, фотографии досок, изображения на других языках, видео и аудиофайлы — amirify извлекает концепции и связи из всего этого и объединяет их в один граф. Видео транскрибируются локально с Whisper, используя доменный промпт из вашего корпуса. Поддерживается 25 языков программирования через tree-sitter AST (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart).

> Андрей Карпати ведёт папку `/raw`, куда складывает статьи, твиты, скриншоты и заметки. amirify — ответ на эту проблему: в **71,5 раза** меньше токенов на запрос по сравнению с чтением сырых файлов, сохранение между сессиями, честность относительно того, что найдено, а что выведено.

```
/amirify .                        # работает с любой папкой — код, заметки, статьи, всё что угодно
```
...
Добавьте файл `.amirifyignore` для исключения папок:
...
> **Официальный пакет:** Пакет PyPI называется `amirify` (установить через `pip install amirify`). Другие пакеты с именем `amirify*` на PyPI не связаны с этим проектом. Единственный официальный репозиторий — [Graphify-Labs/amirify](https://github.com/Graphify-Labs/graphify).
...
| Claude Code (Windows) | `amirify install` (авто-определение) или `amirify install --platform windows` |
...
Примечание: Codex использует `$` вместо `/` для навыков, поэтому вводите `$amirify .`.
...
```
/amirify                          # текущая директория
/amirify ./raw                    # конкретная папка
/amirify ./raw --mode deep        # более агрессивное извлечение INFERRED-рёбер
/amirify ./raw --update           # повторно извлечь только изменённые файлы
/amirify ./raw --directed         # направленный граф
/amirify ./raw --cluster-only     # перезапустить кластеризацию на существующем графе
/amirify ./raw --no-viz           # без HTML, только отчёт + JSON
/amirify ./raw --obsidian         # создать Obsidian vault (opt-in)

/amirify add https://arxiv.org/abs/1706.03762   # получить статью
/amirify add <video-url>                         # скачать аудио, транскрибировать, добавить
/amirify query "что связывает Attention с оптимизатором?"
/amirify path "DigestAuth" "Response"
/amirify explain "SwinTransformer"

amirify hook install              # установить Git-хуки
amirify update ./src              # повторно извлечь файлы кода, без LLM
amirify watch ./src               # автоматическое обновление графа
```
...
**Git-хуки** (`amirify hook install`) — устанавливает post-commit и post-checkout хуки.

## Конфиденциальность

amirify отправляет содержимое файлов в API модели вашего AI-ассистента для семантического извлечения из документов, статей и изображений. Файлы кода обрабатываются локально через tree-sitter AST. Видео и аудиофайлы транскрибируются локально с faster-whisper. Никакой телеметрии, никакого отслеживания использования.
...
## Построено на amirify — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) — корпоративный слой поверх amirify. Там, где amirify превращает папку с файлами в граф знаний, Penpax применяет тот же граф ко всей вашей рабочей жизни — непрерывно.
