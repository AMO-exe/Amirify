...
**مهارة لمساعد برمجة الذكاء الاصطناعي.** اكتب `/amirify` في Claude Code أو Codex أو OpenCode أو Cursor أو Gemini CLI أو GitHub Copilot CLI أو VS Code Copilot Chat أو Aider أو OpenClaw أو Factory Droid أو Trae أو Hermes أو Kiro أو Google Antigravity — يقرأ ملفاتك ويبني رسماً بيانياً للمعرفة ويعيد إليك البنية التي لم تكن تعلم بوجودها. افهم قاعدة الكود بشكل أسرع. اكتشف "السبب" وراء القرارات المعمارية.

متعدد الوسائط بالكامل. أضف كوداً أو ملفات PDF أو markdown أو لقطات شاشة أو رسوماً بيانية أو صور سبورة أو صوراً بلغات أخرى أو ملفات فيديو وصوت — يستخرج amirify المفاهيم والعلاقات من كل ذلك ويربطها في رسم بياني واحد. يتم نسخ مقاطع الفيديو محلياً باستخدام Whisper. يدعم 25 لغة برمجة عبر tree-sitter AST (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart).

> يحتفظ Andrej Karpathy بمجلد `/raw` يضع فيه الأوراق البحثية والتغريدات ولقطات الشاشة والملاحظات. amirify هو الإجابة على تلك المشكلة — **71.5 مرة** أقل في الرموز لكل استعلام مقارنةً بقراءة الملفات الخام، مستمر عبر الجلسات، صادق حول ما تم العثور عليه مقابل ما تم استنتاجه.

</div>

```
/amirify .                        # يعمل مع أي مجلد — الكود، الملاحظات، الأوراق البحثية، كل شيء
```
...
أضف ملف `.amirifyignore` لاستبعاد المجلدات:
...
> **الحزمة الرسمية:** اسم حزمة PyPI هو `amirify` (تثبيت بـ `pip install amirify`). الحزم الأخرى المسماة `amirify*` على PyPI ليست تابعة لهذا المشروع. المستودع الرسمي الوحيد هو [Graphify-Labs/amirify](https://github.com/Graphify-Labs/graphify).
...
| Claude Code (Windows) | `amirify install` (كشف تلقائي) أو `amirify install --platform windows` |
...
ملاحظة: يستخدم Codex `$` بدلاً من `/` للمهارات، لذا اكتب `$amirify .`.
...
```
/amirify                          # المجلد الحالي
/amirify ./raw                    # مجلد محدد
/amirify ./raw --update           # إعادة استخراج الملفات المتغيرة فقط
/amirify ./raw --directed         # رسم بياني موجّه
/amirify ./raw --no-viz           # تقرير + JSON فقط، بدون HTML
/amirify ./raw --obsidian         # إنشاء Obsidian vault

/amirify add https://arxiv.org/abs/1706.03762   # جلب ورقة بحثية
/amirify add <video-url>                         # تحميل صوت، نسخ، إضافة
/amirify query "ما الذي يربط Attention بالمحسِّن؟"
/amirify path "DigestAuth" "Response"
/amirify explain "SwinTransformer"

amirify hook install              # تثبيت Git hooks
amirify update ./src              # إعادة استخراج ملفات الكود، بدون LLM
amirify watch ./src               # تحديث تلقائي للرسم البياني
```
...
**Git hooks** (`amirify hook install`) — يثبّت خطافات post-commit وpost-checkout.

## الخصوصية

يرسل amirify محتوى الملفات إلى API نموذج مساعد الذكاء الاصطناعي الخاص بك للاستخراج الدلالي من المستندات والأوراق البحثية والصور. تتم معالجة ملفات الكود محلياً عبر tree-sitter AST. يتم نسخ ملفات الفيديو والصوت محلياً باستخدام faster-whisper. لا قياس عن بُعد، لا تتبع للاستخدام.
...
## مبني على amirify — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) هو الطبقة المؤسسية فوق amirify. حيث يحوّل amirify مجلداً من الملفات إلى رسم بياني للمعرفة، يطبّق Penpax نفس الرسم البياني على حياتك المهنية بأكملها — باستمرار.
