...
from amirify.cluster import cluster, cohesion_score, remap_communities_to_previous, score_all
...
def test_cluster_does_not_write_to_stderr(capsys):
    ...
    cluster(G)
    captured = capsys.readouterr()
    # Allow logging output (starts with [amirify]) but no raw ANSI codes
...
def test_partition_is_invariant_to_edge_endpoint_orientation():
    ...
    from amirify.cluster import _partition
