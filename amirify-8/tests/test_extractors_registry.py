...
own ``amirify/extractors/<lang>.py`` module, kept a verbatim re-export in
``amirify.extract`` (the facade every existing importer uses), and seeded a
``amirify.extractors.LANGUAGE_EXTRACTORS`` registry. Three things must stay
true for that split to be behavior-preserving:

- the function is importable from its new per-language module,
- ``amirify.extract`` still re-exports the SAME function object (facade
...
import amirify.extract as facade
from amirify.extractors import LANGUAGE_EXTRACTORS
...
def test_every_registry_extractor_is_reexported_from_facade():
    ...
    assert not missing, f"registry extractors not re-exported from amirify.extract: {missing}"
