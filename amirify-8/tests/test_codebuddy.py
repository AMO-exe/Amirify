"""Tests for amirify codebuddy install / uninstall commands."""
import json
...
def _skill_path_user(tmp_path):
    return tmp_path / ".codebuddy" / "skills" / "amirify" / "SKILL.md"


def _skill_path_project(project_dir):
    return project_dir / ".codebuddy" / "skills" / "amirify" / "SKILL.md"
...
# ---------------------------------------------------------------------------
# User-scope install (amirify install --platform codebuddy)
# ---------------------------------------------------------------------------

def test_codebuddy_install_user_creates_skill_file(tmp_path):
    """User-scope install copies skill to ~/.codebuddy/skills/amirify/SKILL.md."""
...
def test_codebuddy_skill_file_contains_frontmatter(tmp_path):
    """Installed skill file must include amirify YAML frontmatter."""
    _codebuddy_install_user(tmp_path)
    content = _skill_path_user(tmp_path).read_text()
    assert "name: amirify" in content
    assert "description:" in content


def test_codebuddy_skill_file_references_amirify_query(tmp_path):
    """/amirify skill must mention amirify query (query-first policy)."""
    _codebuddy_install_user(tmp_path)
    content = _skill_path_user(tmp_path).read_text()
    assert "amirify query" in content or "/amirify query" in content


# ---------------------------------------------------------------------------
# Project-scope install (amirify codebuddy install)
# ---------------------------------------------------------------------------

def test_codebuddy_install_project_writes_codebuddy_md(tmp_path):
    """Project-scope install writes CODEBUDDY.md with amirify section."""
...
def test_codebuddy_install_hook_has_bash_matcher(tmp_path):
    ...
    assert any("amirify" in str(h) for h in bash_hooks)
...
def test_codebuddy_install_hook_has_read_glob_matcher(tmp_path):
    ...
    assert any("amirify" in str(h) for h in read_hooks)


def test_codebuddy_install_idempotent(tmp_path):
    """Re-install does not duplicate ## amirify sections."""
    from amirify.__main__ import codebuddy_install
    codebuddy_install(tmp_path)
    codebuddy_install(tmp_path)
    md = _codebuddy_md_path(tmp_path)
...
def test_codebuddy_install_upgrades_stale_section(tmp_path):
    """Re-install replaces an old amirify section with the current template."""
    from amirify.__main__ import codebuddy_install, _CODEBUDDY_MD_MARKER
...
def test_codebuddy_install_hint_git_add(tmp_path, capsys):
    ...
    try:
        ...
        with patch("amirify.__main__.Path.home", return_value=home):
            sys.argv = ["amirify", "codebuddy", "install"]
            main()
    finally:
        import os
        os.chdir(old_cwd)
...
def test_codebuddy_uninstall_removes_section(tmp_path):
    """Uninstall removes the ## amirify section from CODEBUDDY.md."""
...
def test_codebuddy_uninstall_noop_if_no_section(tmp_path):
    """Uninstall should not error when CODEBUDDY.md exists but no amirify section."""
    from amirify.__main__ import codebuddy_uninstall
...
def test_codebuddy_uninstall_preserves_other_content(tmp_path):
    """Uninstall preserves non-amirify content in CODEBUDDY.md."""
    from amirify.__main__ import codebuddy_install, codebuddy_uninstall
    _codebuddy_md_path(tmp_path).write_text("# My project rules\n")
    codebuddy_install(tmp_path)
    codebuddy_uninstall(tmp_path)
    # When amirify section was appended, uninstall removes it and the file
...
def test_uninstall_all_removes_codebuddy_md(tmp_path, monkeypatch):
    """amirify uninstall must clean up CODEBUDDY.md."""
    from amirify.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    with patch("amirify.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["amirify", "codebuddy", "install"])
        main()
...
def test_uninstall_all_removes_codebuddy_hook(tmp_path, monkeypatch):
    """amirify uninstall must clean up .codebuddy/settings.json hooks."""
    from amirify.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
...
def test_codebuddy_in_platform_config():
    ...
    from amirify.__main__ import _PLATFORM_CONFIG
...
def test_codebuddy_platform_skill_destination_user_scope(tmp_path):
    """User-scope destination must be ~/.codebuddy/skills/amirify/SKILL.md."""
    from amirify.__main__ import _platform_skill_destination
    with patch("amirify.__main__.Path.home", return_value=tmp_path):
        dst = _platform_skill_destination("codebuddy", project=False)
    assert dst == tmp_path / ".codebuddy" / "skills" / "amirify" / "SKILL.md"


def test_codebuddy_platform_skill_destination_project_scope(tmp_path):
    """Project-scope destination must be <project>/.codebuddy/skills/amirify/SKILL.md."""
    from amirify.__main__ import _platform_skill_destination
...
def test_codebuddy_in_main_help_text(capsys, monkeypatch):
    """`amirify --help` must list codebuddy in the platform list and per-platform section."""
    from amirify.__main__ import main
    monkeypatch.setattr(sys, "argv", ["amirify", "--help"])
    main()
    captured = capsys.readouterr().out
    # codebuddy should appear in the top-level platform list
    assert "|codebuddy)" in captured or "codebuddy" in captured, (
        "codebuddy missing from `amirify --help` platform list"
    )
