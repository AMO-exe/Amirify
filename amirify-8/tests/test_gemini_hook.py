...
Since #522 it runs as `amirify hook-guard gemini` (not a `python -c` one-liner
...
from amirify.__main__ import _gemini_hook
...
def _run(cwd, *, graph: bool):
    ...
    return subprocess.run(
        [sys.executable, "-m", "amirify", "hook-guard", "gemini"],
...
def test_matcher_and_command_shape():
    ...
    assert "amirify" in cmd and "hook-guard gemini" in cmd
...
def test_honors_amirify_out_override(tmp_path):
    ...
    assert "amirify query" in json.loads(r.stdout).get("additionalContext", "")
