"""`amirify extract --code-only` indexes code without an LLM key (#1734).
...
def _run(repo: Path, *extra: str):
    ...
    env["AMIRIFY_OUT"] = str(repo / "amirify-out")
    return subprocess.run(
        [PYTHON, "-m", "amirify", "extract", ".", *extra],
...
def test_code_only_succeeds_without_key(tmp_path):
    ...
    graph = repo / "amirify-out" / "graph.json"
...
def test_extract_usage_advertises_code_only(tmp_path):
    ...
    by triggering the no-key error. `amirify extract` with no path prints usage."""
    r = subprocess.run(
        [PYTHON, "-m", "amirify", "extract"],
...
def _run_relative_out(repo: Path, *extra: str):
    """Like _run but with a RELATIVE AMIRIFY_OUT so --out/--output controls the
    parent dir (an absolute AMIRIFY_OUT would override the flag)."""
    env = {k: v for k, v in os.environ.items() if k not in _KEY_VARS}
    env["AMIRIFY_OUT"] = "amirify-out"
    return subprocess.run(
        [PYTHON, "-m", "amirify", "extract", ".", *extra],
        cwd=repo, capture_output=True, text=True, env=env,
    )
...
def test_output_flag_is_alias_of_out(tmp_path):
    ...
    to the default `<path>/amirify-out/`). It is now an alias of `--out`."""
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / "app.py").write_text("def hello():\n    return 1\n")
    ...
    assert (custom / "amirify-out" / "graph.json").exists(), "--output was ignored (#2004)"
    assert not (repo / "amirify-out").exists(), "output must not go to the default dir"
...
def test_output_flag_inline_form(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    assert (custom / "amirify-out" / "graph.json").exists()


def test_no_gitignore_indexes_vcs_ignored_code_but_keeps_amirifyignore(tmp_path):
    repo = tmp_path / "repo"
    ...
    (repo / "proj" / ".amirifyignore").write_text("hidden/\n")
    ...
    graph = json.loads((repo / "amirify-out" / "graph.json").read_text())
...
def test_no_gitignore_setting_persists_across_flagless_extract(tmp_path):
    ...
    `amirify extract` must NOT clobber it back to honoring .gitignore (which
    ...
    def _sources():
        g = json.loads((repo / "amirify-out" / "graph.json").read_text())
...
def test_explicit_exclude_replaces_persisted_setting_with_custom_out(tmp_path):
    ...
    graph_out = out_root / "amirify-out"
    def _sources():
    ...
    assert json.loads((graph_out / ".amirify_build.json").read_text()) == {
        "excludes": ["generated"]
    }
...
def test_code_only_force_rescan_re_resolves_tsconfig_paths_and_preserves_semantics(tmp_path: Path):
    ...
    assert r1.returncode == 0, r1.stderr
    graph_path = repo / "amirify-out" / "graph.json"
