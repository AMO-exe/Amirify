"""`amirify merge-graphs` tolerates inputs that disagree on graph type (#1606).
...
def _run(args, cwd):
    return subprocess.run([PYTHON, "-m", "amirify"] + args, cwd=cwd,
...
def test_merge_graphs_mixed_directed_and_multigraph(tmp_path):
    a = tmp_path / "r1" / "amirify-out" / "graph.json"
    b = tmp_path / "r2" / "amirify-out" / "graph.json"
    c = tmp_path / "r3" / "amirify-out" / "graph.json"
...
def test_merge_graphs_same_named_repo_dirs_do_not_collapse(tmp_path):
    # #1729: two graphs under a same-named repo dir (src/amirify-out and
    # frontend/src/amirify-out both → tag "src") share the `src::` prefix, so a
    ...
    a = tmp_path / "src" / "amirify-out" / "graph.json"
    b = tmp_path / "frontend" / "src" / "amirify-out" / "graph.json"
...
def test_distinct_repo_tags_unit(tmp_path):
    ...
    assert distinct_repo_tags([
        Path("backend/amirify-out/graph.json"),
        Path("web/amirify-out/graph.json"),
    ]) == ["backend", "web"]
    # same-named repo dirs are widened to stay distinct
    tags = distinct_repo_tags([
        Path("proj/src/amirify-out/graph.json"),
        Path("proj/frontend/src/amirify-out/graph.json"),
    ])
    ...
    tags3 = distinct_repo_tags([
        Path("a/src/amirify-out/graph.json"),
        Path("b/src/amirify-out/graph.json"),
        Path("c/src/amirify-out/graph.json"),
    ])
...
def test_merge_graphs_preserves_import_edge_direction(tmp_path):
    ...
    a = tmp_path / "repo1" / "amirify-out" / "graph.json"
    b = tmp_path / "repo2" / "amirify-out" / "graph.json"
    a.parent.mkdir(parents=True, exist_ok=True)
    b.parent.mkdir(parents=True, exist_ok=True)
...
def test_merge_graphs_carries_hyperedges_from_all_inputs(tmp_path):
    ...
    a = tmp_path / "alpha" / "amirify-out" / "graph.json"
    b = tmp_path / "beta" / "amirify-out" / "graph.json"
