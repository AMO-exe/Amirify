"""Rigorous edge-case coverage for the `amirify hook-guard` subcommand (#522).
...
from amirify import __main__ as m


# --------------------------------------------------------------------------- #
# Direct-call harness: hermetic w.r.t. the ambient AMIRIFY_OUT env.
# --------------------------------------------------------------------------- #
def _invoke(kind, payload, tmp_path, monkeypatch, *, graph=True, out_name="amirify-out"):
    monkeypatch.setattr("amirify.paths.AMIRIFY_OUT", out_name)
    monkeypatch.setattr("amirify.paths.AMIRIFY_OUT_NAME", out_name)
...
def test_read_nudges(tool_input, tmp_path, monkeypatch):
    ...
    assert "amirify query" in out, f"{tool_input!r} should nudge"
...
@pytest.mark.parametrize("tool_input", [
    ...
    {"file_path": "amirify-out/GRAPH_REPORT.md"},  # the graph's own output
    {"file_path": ""},
    {},                                          # nothing at all
])
...
def test_read_respects_custom_output_dir_name(tmp_path, monkeypatch):
    ...
    # not just the literal 'amirify-out/'.
...
def test_search_out_path_error_is_swallowed(tmp_path, monkeypatch):
    ...
    def _boom(*a, **k):
        raise OSError("boom")
    monkeypatch.setattr("amirify.paths.out_path", _boom)
...
def test_gemini_allow_with_nudge(tmp_path, monkeypatch):
    ...
    assert "amirify query" in payload["additionalContext"]
...
def test_read_nudge_em_dash_survives_utf8(tmp_path):
    ...
    (tmp_path / "amirify-out" / "graph.json").write_text("{}", encoding="utf-8")
    r = subprocess.run(
        [sys.executable, "-m", "amirify", "hook-guard", "read"],
