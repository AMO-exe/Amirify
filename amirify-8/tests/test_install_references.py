...
The real claude bundle now ships in the package (amirify/skills/claude/), so
...
PKG_DIR = Path(amirify.__file__).parent
...
def test_install_stages_references_sidecar(tmp_path, fake_bundle):
    ...
    skill_dir = tmp_path / ".claude" / "skills" / "amirify"
...
def test_single_version_stamp_covers_skill_and_references(tmp_path, fake_bundle):
    """One .amirify_version stamp versions SKILL.md + references/ together."""
    platform = fake_bundle
    _install(tmp_path, platform)
    skill_dir = tmp_path / ".claude" / "skills" / "amirify"
    stamps = list(skill_dir.rglob(".amirify_version"))
    assert len(stamps) == 1
    assert stamps[0] == skill_dir / ".amirify_version"
...
def test_claude_install_ships_lean_core_and_references(tmp_path):
    ...
    assert (skill_dir / ".amirify_version").read_text() == mainmod.__version__
...
def test_gemini_install_references_all_resolve(tmp_path):
    ...
    skill = tmp_path / ".gemini" / "skills" / "amirify" / "SKILL.md"
    assert skill.exists()
...
def test_pyproject_declares_references_globs():
    ...
    The references sidecar ships under amirify/skills/<host>/references/ and the
    always-on injection blocks under amirify/always_on/. The earlier
    skills/*/SKILL.md glob matched nothing (no amirify/skills/<host>/SKILL.md
    exists; the skill bodies are amirify/skill*.md, listed explicitly), so it was
    ...
    pkg_data = data["tool"]["setuptools"]["package-data"]["amirify"]
...
def test_built_wheel_ships_the_full_skill_payload():
    ...
    assert "amirify/skills/claude/references/extraction-spec.md" in names
    assert "amirify/skills/trae/references/hooks.md" in names
    # amp is now a split host too; its bundle must ship like every other.
    assert "amirify/skills/amp/references/hooks.md" in names
...
def test_monolith_install_clears_orphan_references(tmp_path, fake_bundle):
    ...
    skill_dst = tmp_path / ".aider" / "amirify" / "SKILL.md"
...
def test_amp_user_install_carries_references(tmp_path, monkeypatch):
    ...
    amp's real bundle now ships in the package (amirify/skills/amp/), so the
    ...
    with patch("amirify.__main__.Path.home", return_value=home):
        ...
        main()
        skill_dir = home / ".config" / "agents" / "skills" / "amirify"
        assert (skill_dir / "SKILL.md").exists()
