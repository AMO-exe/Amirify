...
Pre-fix, the installers wrote a "## amirify" section with report-first
instructions and skipped writing if the marker was already present. So users
who installed amirify and then upgraded to the fixed package still had the
...
# wrote a variant of "ALWAYS read amirify-out/GRAPH_REPORT.md before ...".
_OLD_CLAUDE_SECTION = """\
## amirify
...
Rules:
- ALWAYS read amirify-out/GRAPH_REPORT.md before reading any source files, running grep/glob searches, or answering codebase questions. The graph is your primary map of the codebase.
- IF amirify-out/wiki/index.md EXISTS, navigate it instead of reading raw files
- For cross-module "how does X relate to Y" questions, prefer `amirify query "<question>"`, `amirify path "<A>" "<B>"`, or `amirify explain "<concept>"` over grep — these traverse the graph's EXTRACTED + INFERRED edges instead of scanning files
...
code, your **first tool call must be** to read `amirify-out/GRAPH_REPORT.md` (if it exists).
...
_OLD_CURSOR_RULE = """\
---
description: amirify knowledge graph context
alwaysApply: true
---

This project has an amirify knowledge graph at amirify-out/.

- Before answering architecture or codebase questions, read amirify-out/GRAPH_REPORT.md for god nodes and community structure
- If amirify-out/wiki/index.md exists, navigate it instead of reading raw files
"""
...
amirify: A knowledge graph of this project lives in `amirify-out/`. \
If `amirify-out/GRAPH_REPORT.md` exists, read it before answering architecture questions, \
...
_OLD_HOOK_PAYLOAD_SNIPPET = "Read amirify-out/GRAPH_REPORT.md for god nodes and community structure before searching raw files"


def _assert_no_report_first(text: str, ctx: str) -> None:
    assert "ALWAYS read amirify-out/GRAPH_REPORT.md" not in text, (
...
def _assert_query_first(text: str, ctx: str) -> None:
    assert "amirify query" in text, (
        f"{ctx}: new 'amirify query' guidance missing after upgrade"
    )
...
def test_claude_install_upgrades_stale_section(tmp_path, monkeypatch):
    ...
    `amirify claude install` again after upgrading to a fixed package."""
    ...
    # Pre-existing non-amirify content must be preserved
...
def test_claude_install_upgrades_stale_hook_payload(tmp_path, monkeypatch):
    ...
    # Since #522 the nudge text lives in the `amirify hook-guard` subcommand, not
    ...
    assert "hook-guard" in new_settings_text, (
        "new hook payload should route to the `amirify hook-guard` subcommand"
    )
...
def test_cursor_install_upgrades_stale_rule(tmp_path, monkeypatch):
    """Same upgrade behavior for .cursor/rules/amirify.mdc.
    The Cursor rule file is wholly amirify-owned; overwrite on upgrade."""
    monkeypatch.chdir(tmp_path)
    rule_path = tmp_path / ".cursor" / "rules" / "amirify.mdc"
    ...
    assert "read amirify-out/GRAPH_REPORT.md for god nodes and community structure" not in after
    _assert_query_first(after, ".cursor/rules/amirify.mdc")
...
def test_kiro_install_upgrades_stale_steering(tmp_path, monkeypatch):
    """Same upgrade behavior for .kiro/steering/amirify.md (wholly owned)."""
    monkeypatch.chdir(tmp_path)
    steering = tmp_path / ".kiro" / "steering" / "amirify.md"
    ...
    _assert_query_first(after, ".kiro/steering/amirify.md")
...
def test_kiro_install_ships_references_sidecar_and_version_stamp(tmp_path, monkeypatch):
    ...
    and .amirify_version stamp are written alongside SKILL.md (#1142).
    ...
    skill_dir = tmp_path / ".kiro" / "skills" / "amirify"
    ...
    # .amirify_version stamp written
    version_file = skill_dir / ".amirify_version"
    assert version_file.exists(), ".amirify_version stamp must be written (#1142)"
    ...
    assert (tmp_path / ".kiro" / "steering" / "amirify.md").exists()
    ...
    assert not (tmp_path / ".kiro" / "steering" / "amirify.md").exists()
