...
These strings live in amirify/__main__.py and are written into project-local
files (CLAUDE.md, AGENTS.md, GEMINI.md, .cursor/rules/, .kiro/steering/, etc.)
or into in-process hook payloads. Earlier versions of amirify told every
assistant to "ALWAYS read amirify-out/GRAPH_REPORT.md before answering" —
...
def test_every_install_surface_recommends_amirify_query():
    """All ten install surfaces must point the assistant at `amirify query`
    ...
    for name, text in _INSTALL_TEXTS.items():
        if "amirify query" not in text:
            missing.append(name)
    assert not missing, (
        f"these install surfaces no longer mention `amirify query`: {missing}. "
...
def test_agents_section_does_not_skip_dirty_graph_output():
    assert "Dirty amirify-out/ files are expected" in _AGENTS_MD_SECTION
    assert "not a reason to skip amirify" in _AGENTS_MD_SECTION


def test_agents_section_uses_generic_amirify_instruction():
    assert "`skill` tool" not in _AGENTS_MD_SECTION
    assert 'skill: "amirify"' not in _AGENTS_MD_SECTION
    assert "use the installed amirify skill" in _AGENTS_MD_SECTION


def test_skill_registration_uses_host_generic_instruction():
    reg = _skill_registration()
    assert 'skill: "amirify"' not in reg
    assert "Skill tool" not in reg
    assert "use the installed amirify skill or instructions" in reg
