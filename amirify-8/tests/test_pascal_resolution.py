...
file's own scope. amirify.pascal_resolution closes that gap as a
...
from amirify.extract import extract, extract_pascal
...
def test_calls_resolve_across_files_via_inherits_chain(tmp_path):
    # cache_root only controls where amirify-out/cache/ is written -- it has
...
def test_pascal_resolver_registered():
    from amirify.resolver_registry import registered_resolvers
