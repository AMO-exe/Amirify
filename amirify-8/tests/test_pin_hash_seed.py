"""#3641: `amirify update`/`extract`/`cluster-only` must pin PYTHONHASHSEED
...
`amirify.__main__.main()` directly with a monkeypatched sys.argv to
...
def test_does_not_reexec_for_unrelated_commands():
    for cmd in ("query", "install", "path", "explain"):
        outcome = _run_probe(["amirify", cmd, "x"])
...
def test_update_still_runs_end_to_end_with_hashseed_unset(tmp_path):
    ...
    still let `amirify update .` complete successfully all the way through
    the re-exec."""
    ...
    result = subprocess.run(
        [sys.executable, "-m", "amirify", "update", "."],
        cwd=tmp_path, capture_output=True, text=True, env=env,
    )

    assert result.returncode == 0, result.stderr
    assert (tmp_path / "amirify-out" / "graph.json").exists()
