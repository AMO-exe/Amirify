"""Integration tests for amirify export subcommands and CLI commands.
...
def _make_graph(tmp_path: Path) -> Path:
    """Build a minimal graph.json + analysis/labels files in tmp_path/amirify-out/."""
    ...
    from amirify.analyze import god_nodes, surprising_connections
    ...
    (out / ".amirify_analysis.json").write_text(json.dumps(analysis))
    (out / ".amirify_labels.json").write_text(
...
def test_export_html_creates_file(tmp_path):
    ...
    html = tmp_path / "amirify-out" / "graph.html"
    assert html.exists()
...
def test_export_wiki_creates_articles(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    wiki = tmp_path / "amirify-out" / "wiki"
    assert wiki.exists()
...
# ── amirify export falkordb (cypher) ────────────────────────────────────────
...
def test_query_uses_amirify_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
    env["AMIRIFY_OUT"] = custom_out.name
...
def test_extract_writes_to_amirify_out_env(tmp_path):
    """#1423: `amirify extract` honours AMIRIFY_OUT for where it WRITES, not only
    where readers look — previously it hardcoded amirify-out/ and ignored the
    override. Code-only corpus, so no LLM backend is needed."""
    (tmp_path / "m.py").write_text("def a():\n    return b()\n\n\ndef b():\n    return 1\n")
    env = os.environ.copy()
    env["AMIRIFY_OUT"] = "custom-out"
    ...
    assert not (tmp_path / "amirify-out").exists(), "extract ignored AMIRIFY_OUT and wrote amirify-out/"
...
def test_path_uses_amirify_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
def test_explain_uses_amirify_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
# Regression test for #934 - cluster-only crashes when amirify-out/ doesn't exist

def test_cluster_only_creates_output_dir_when_missing(tmp_path):
    """cluster-only must not crash with FileNotFoundError when amirify-out/ is absent (#934)."""
    # Build graph.json somewhere other than the default amirify-out/ location
    # so we can point --graph at it while amirify-out/ doesn't exist yet.
    ...
    assert r.returncode == 0, r.stderr
    assert (tmp_path / "amirify-out" / "GRAPH_REPORT.md").exists()


def test_cluster_only_graph_in_amirify_out_writes_beside_it(tmp_path):
    """#1747 Case 2: `cluster-only --graph <elsewhere>/amirify-out/graph.json`
    must write GRAPH_REPORT.md and the re-clustered graph beside that graph, not
    into a stray amirify-out/ in the CWD."""
    project = tmp_path / "project"
    project.mkdir()
    out_dir = _make_graph(project)  # project/amirify-out/graph.json
...
def test_extract_out_does_not_pollute_corpus(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    assert (out / "amirify-out" / "graph.json").exists()   # graph in --out
    assert not (corpus / "amirify-out").exists()           # corpus untouched
...
def test_cluster_only_persists_analysis_sidecar(tmp_path):
    """cluster-only must refresh .amirify_analysis.json alongside graph.json.
    ...
    """
    out = _make_graph(tmp_path)
    analysis_path = out / ".amirify_analysis.json"
...
def test_cluster_only_remaps_labels_to_previous_cids(tmp_path):
    ...
    .amirify_labels.json keeps tracking the same conceptual communities after
    ...
    labels_json = out / ".amirify_labels.json"
...
# ── communities-fallback when .amirify_analysis.json is absent ──────────────
# The watch / post-commit rebuild path only writes graph.json + GRAPH_REPORT.md;
# it does NOT regenerate .amirify_analysis.json. The full `amirify extract`
...
# Without these tests, `amirify export html|obsidian|wiki|svg|graphml|neo4j`
...
def test_export_html_falls_back_to_node_community_attribute(tmp_path):
    """When .amirify_analysis.json is absent, export html should reconstruct
    ...
    (out / ".amirify_analysis.json").unlink()
...
def test_export_html_fallback_recovers_multiple_communities(tmp_path):
    ...
    analysis = json.loads((out / ".amirify_analysis.json").read_text(encoding="utf-8"))
...
# regenerate .amirify_analysis.json, so it can describe an earlier
...
def test_graph_json_node_ids_are_portable_across_checkout_paths(tmp_path):
    ...
    def _build(root: Path):
        ...
        assert r.returncode == 0, r.stderr
        data = json.loads((root / "amirify-out" / "graph.json").read_text())
...
def test_cluster_only_warns_when_labeling_flags_are_ignored(tmp_path):
    """#2534 case 1: with a saved .amirify_labels.json the reuse branch never
    ...
    _make_graph(tmp_path)  # persists .amirify_labels.json -> reuse branch
...
# ── amirify query names its graph (#2789) ───────────────────────────────────
...
def test_query_command_header_names_the_graph(tmp_path):
    ...
    assert first_line.startswith("Graph: amirify-out/graph.json ("), first_line
