...
`amirify hook-guard read` subcommand (not inline bash), so it works on Windows
too. These tests invoke that subcommand with crafted stdin JSON and assert it
nudges only for a source/doc file outside amirify-out/ when a graph exists, and
...
def _env():
    # The guard resolves the graph via AMIRIFY_OUT (default "amirify-out",
...
def test_command_has_no_shell_syntax():
    ...
    assert "amirify" in cmd and "hook-guard read" in cmd
...
def test_silent_on_amirify_out_targets(tmp_path):
    """Reading the graph's own report must not start a go-read-the-graph loop."""
    out = _run({"file_path": "amirify-out/GRAPH_REPORT.md"}, tmp_path, graph=True).stdout
    assert out.strip() == ""
...
def test_nudges_on_framework_source(tmp_path):
    ...
    for path in ("src/components/Hero.astro", "src/App.vue", "src/Card.svelte"):
        out = _run({"file_path": path}, tmp_path, graph=True).stdout
        assert "amirify query" in out, f"{path} should nudge"
