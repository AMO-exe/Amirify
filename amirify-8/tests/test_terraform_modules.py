...
from amirify.extract import extract, extract_terraform
...
def test_incremental_source_change_target_addition_and_deletion(tmp_path, monkeypatch, mode, no_cluster):
    ...
    def rebuild(changed=None):
        if mode == "watch":
            ...
            args = [sys.executable, "-m", "amirify", "extract", str(root), "--code-only"]
            if no_cluster:
                args.append("--no-cluster")
            p = subprocess.run(args, cwd=root, env=os.environ.copy(), capture_output=True, text=True)
            assert p.returncode == 0, p.stdout + p.stderr
        result = json.loads((root / "amirify-out/graph.json").read_text())
        _assert_integrity(result)
        return result
...
def test_incremental_exclusion_removes_incoming_module_links(tmp_path, monkeypatch, mode):
    ...
    def rebuild(changed=None):
        if mode == "watch":
            ...
            p = subprocess.run(
                [sys.executable, "-m", "amirify", "extract", str(root), "--code-only", "--no-cluster"],
                capture_output=True, text=True,
            )
            assert p.returncode == 0, p.stdout + p.stderr
        return json.loads((root / "amirify-out/graph.json").read_text())

    assert len(_module_edges(rebuild())) == 1
    ignore = _write(root, ".amirifyignore", "app/\n")
