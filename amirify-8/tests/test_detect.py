...
from amirify.detect import classify_file, count_words, detect, detect_incremental, save_manifest, FileType, _looks_like_paper, _is_ignored, _load_amirifyignore, _is_sensitive
from amirify import detect as detect_mod
...
def test_detect_skips_noise_dot_dirs():
    """Noise dot dirs (.next, .nuxt, .amirify cache, …) are skipped (#873).
    Non-noise dot dirs (.github, .claude, …) are now allowed through."""
    result = detect(FIXTURES)
    for files in result["files"].values():
        for f in files:
            # amirify's own cache is always skipped
            assert "/.amirify/" not in f
...
def test_amirifyignore_excludes_file(tmp_path):
    """Files matching .amirifyignore patterns are excluded from detect()."""
    (tmp_path / ".amirifyignore").write_text("vendor/\n*.generated.py\n")
    ...
    assert result["amirifyignore_patterns"] == 2


def test_amirifyignore_matches_nfd_path_with_nfc_pattern(tmp_path):
    ...
    (tmp_path / ".amirifyignore").write_text(f"{nfc_name}/\n", encoding="utf-8")
...
def test_amirifyignore_matches_nfc_path_with_nfd_pattern(tmp_path):
    """The reverse direction also holds: NFD pattern, NFC path on disk."""
    nfc_name = unicodedata.normalize("NFC", "Or\u00e7amento")
    nfd_name = unicodedata.normalize("NFD", nfc_name)

    (tmp_path / ".amirifyignore").write_text(f"{nfd_name}/\n", encoding="utf-8")
    d = tmp_path / nfc_name
    d.mkdir()
...
def test_amirifyignore_ascii_patterns_unaffected(tmp_path):
...
def test_amirifyignore_missing_is_fine(tmp_path):
    """No .amirifyignore is not an error."""
    (tmp_path / "main.py").write_text("x = 1")
    result = detect(tmp_path)
    assert result["amirifyignore_patterns"] == 0


def test_amirifyignore_comments_ignored(tmp_path):
    """Comment lines in .amirifyignore are not treated as patterns."""
    (tmp_path / ".amirifyignore").write_text("# this is a comment\n\nmain.py\n")
    (tmp_path / "main.py").write_text("x = 1")
...
def test_amirifyignore_utf8_bom_first_pattern_honored(tmp_path):
    """A UTF-8 BOM at the start of .amirifyignore must not corrupt the first
    pattern (#2163): git strips a single leading BOM, so `*.log` on line 1
    must still exclude app.log."""
    (tmp_path / ".amirifyignore").write_bytes(b"\xef\xbb\xbf*.log\nbuild/\n")
...
def test_amirifyignore_bom_only_file(tmp_path):
    """A .amirifyignore containing only a BOM yields zero patterns, not one
    bogus U+FEFF pattern (#2163)."""
    (tmp_path / ".amirifyignore").write_bytes(b"\xef\xbb\xbf")
    (tmp_path / "main.py").write_text("x = 1")
...
def test_amirifyignore_bom_then_comment(tmp_path):
    """A BOM followed by a comment must still parse as a comment, not become
    a `\\ufeff# comment` pattern (#2163)."""
    (tmp_path / ".amirifyignore").write_bytes(b"\xef\xbb\xbf# comment\nmain.py\n")
    (tmp_path / "main.py").write_text("x = 1")
    (tmp_path / "other.py").write_text("x = 2")
    ...
    assert not any("main.py" in f for f in result["files"]["code"])
    assert any("other.py" in f for f in result["files"]["code"])
    assert result["amirifyignore_patterns"] == 1, "BOM'd comment became a pattern"
...
def test_git_info_exclude_utf8_bom(tmp_path):
    ...
    pattern either (#2163) — second read site in _load_amirifyignore."""
...
def test_amirifyignore_hermetic_without_vcs(tmp_path):
    """Without a VCS root, parent .amirifyignore does NOT apply (hermetic)."""
    (tmp_path / ".amirifyignore").write_text("vendor/\n")
    ...
    # parent .amirifyignore must NOT leak into a non-VCS scan
    assert any("vendor" in f for f in code_files)
    assert result["amirifyignore_patterns"] == 0


def test_amirifyignore_discovered_from_parent_in_vcs(tmp_path):
    """Inside a VCS repo, parent .amirifyignore applies to subdirectory scans."""
    (tmp_path / ".git").mkdir()
    (tmp_path / ".amirifyignore").write_text("vendor/\n")
    sub = tmp_path / "packages" / "mylib"
    sub.mkdir(parents=True)
    ...
    assert result["amirifyignore_patterns"] >= 1


def test_amirifyignore_stops_at_git_boundary(tmp_path):
    """Upward search stops at the git repo root (.git directory)."""
    (tmp_path / ".amirifyignore").write_text("main.py\n")
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / ".git").mkdir()
    sub = repo / "sub"
...
def test_amirifyignore_at_git_root_is_included(tmp_path):
    """A .amirifyignore at the git repo root is included when scanning a subdir."""
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / ".git").mkdir()
    (repo / ".amirifyignore").write_text("vendor/\n")
...
def test_amirifyignore_still_excludes_git_tracked_file(tmp_path):
    ...
    (tmp_path / ".amirifyignore").write_text("storage/\n", encoding="utf-8")
...
def test_detect_incremental_propagates_follow_symlinks(requires_symlinks, tmp_path, monkeypatch):
    ...
    # Store manifest inside amirify-out/ so it is pruned by _SKIP_DIRS
    # and doesn't get re-detected as a code file now that .json is indexed.
    manifest_dir = tmp_path / "amirify-out"
    manifest_dir.mkdir()
...
def test_classify_video_extensions():
    ...
    from amirify.detect import FileType
...
def test_detect_converts_google_workspace_shortcuts_when_enabled(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("amirify.detect.convert_google_workspace_file", fake_convert)
...
def test_detect_office_sidecar_survives_a_gitignored_output_dir(tmp_path, monkeypatch):
    """#3504: the documented .gitignore advice puts amirify-out/ (and so
    amirify-out/converted/, where Office sidecars land) inside a gitignored
    ...
    follows that advice."""
    (tmp_path / ".gitignore").write_text("amirify-out/\n", encoding="utf-8")
    ...
    monkeypatch.setattr("amirify.detect.convert_office_file", fake_convert)
...
def test_collect_files_keeps_coverage_code_namespace(tmp_path):
    ...
    from amirify.extract import collect_files
...
def test_detect_honors_git_info_exclude(tmp_path):
    ...
    .amirifyignore — otherwise nested worktree copies get fully indexed (#1810)."""
    (tmp_path / ".git" / "info").mkdir(parents=True)
...
def test_git_info_exclude_ranks_below_gitignore_negation(tmp_path):
    ...
    from amirify.detect import _load_amirifyignore, _is_ignored
    (tmp_path / ".git" / "info").mkdir(parents=True)
    ...
    patterns = _load_amirifyignore(tmp_path)
...
def test_detect_skips_amirify_own_cache(tmp_path):
    """.amirify/ (extraction cache) must never be re-indexed as source (#873)."""
    cache = tmp_path / ".amirify" / "cache"
    cache.mkdir(parents=True)
    (cache / "abc123.json").write_text('{"nodes": [], "edges": []}')
    (tmp_path / "app.py").write_text("def go(): pass")
    result = detect(tmp_path)
    all_files = [f for files in result["files"].values() for f in files]
    assert not any(".amirify" in f for f in all_files)
    assert any("app.py" in f for f in all_files)
...
def test_anchored_root_wildcard_negation_reincludes_subtree(tmp_path):
    ...
    for rel in ("src/app/main.py", "src/lib/util.py", "docs/guide.md", "README.md"):
        ...
        path.write_text("x\n")
    (tmp_path / ".amirifyignore").write_text("/*\n!/src/\n")
...
def test_path_pattern_single_star_does_not_cross_segment(tmp_path):
    ...
    for pattern in ("/src/*.py", "src/*.py"):
        (tmp_path / ".amirifyignore").write_text(f"{pattern}\n")
        result = detect(tmp_path)
        files = as_posix_list(
...
def test_anchored_double_star_crosses_path_segments(tmp_path):
    ...
    direct.write_text("x\n")
    nested.write_text("x\n")
    (tmp_path / ".amirifyignore").write_text("/src/**/generated.py\n")
...
def test_negation_cannot_rescue_file_under_excluded_dir(tmp_path):
    ...
    from amirify.detect import _is_ignored, _load_amirifyignore
    android = tmp_path / "android" / "app" / "src"
    android.mkdir(parents=True)
    victim = android / "Main.kt"
    victim.write_text("fun main() {}")
    (tmp_path / ".amirifyignore").write_text("android/\n!src/\n")
    patterns = _load_amirifyignore(tmp_path)
...
def test_negation_works_when_no_ancestor_excluded(tmp_path):
    ...
    keep = src / "keep.py"
    keep.write_text("x = 1")
    (tmp_path / ".amirifyignore").write_text("*.py\n!src/keep.py\n")
    patterns = _load_amirifyignore(tmp_path)
...
def test_negation_does_not_disable_directory_pruning(tmp_path, monkeypatch):
    ...
    """
    import os
    import amirify.detect as det

    (tmp_path / ".amirifyignore").write_text("myignored/\n*.md\n!docs/**\n")
...
def test_anchored_dir_not_matched_at_depth(tmp_path):
    ...
    (tmp_path / ".amirifyignore").write_text("/inbox/\n")
    patterns = _load_amirifyignore(tmp_path)
    assert not _is_ignored(f, tmp_path, patterns), (
...
def test_anchored_file_not_matched_at_depth(tmp_path):
    ...
    (tmp_path / ".amirifyignore").write_text("/build\n")
    patterns = _load_amirifyignore(tmp_path)
...
def test_anchored_multi_segment_pattern(tmp_path):
    ...
    (tmp_path / ".amirifyignore").write_text("/src/inbox/\n")
    patterns = _load_amirifyignore(tmp_path)
...
def test_detect_does_not_ignore_scan_root_itself_via_parent_gitignore(tmp_path):
    ...
    corpus_dir = tmp_path / "amirify-corpus"
    corpus_dir.mkdir()
    ...
    (tmp_path / ".gitignore").write_text("amirify-corpus/\n", encoding="utf-8")
...
def test_is_ignored_cache_evaluates_each_dir_once():
    ...
    """
    from amirify.detect import _is_ignored
...
# a 76k-file vault with a 29k-file directory listed in .amirifyignore pinned a
...
def test_ignored_dir_pruned_walk_never_lists_contents(tmp_path, monkeypatch):
    ...
    a regenerated notes/ dir listed in .amirifyignore.
    """
    import amirify.detect as det

    (tmp_path / ".amirifyignore").write_text("amirify-notes/\n")
    for d in range(5):
        sub = tmp_path / "amirify-notes" / f"note-dir-{d}"
        sub.mkdir(parents=True)
        for f in range(4):
    ...
    assert not any("amirify-notes" in Path(v).parts for v in visited), (
    ...
    assert not any("amirify-notes" in p for p in all_files)
    ...
    assert sum("amirify-notes" in p for p in ignored) == 1
...
def test_scan_ignore_cost_is_per_directory_not_per_file(tmp_path, monkeypatch):
    ...
    for d in range(n_ignored_dirs):
        sub = tmp_path / "amirify-notes" / f"note-dir-{d:03d}"
        sub.mkdir(parents=True)
    ...
    def counting_scan(path, *args, **kwargs):
        scan_calls["total"] += 1
        if "amirify-notes" in path.parts:
...
def test_save_manifest_skips_semantic_hash_for_files_without_cache(tmp_path):
    ...
    import json
    from amirify.cache import save_cached
...
def test_save_manifest_clear_semantic_erases_stale_hash_for_omitted_file(tmp_path):
    ...
    doc.parent.mkdir()
    doc.write_text("# Doc\n\ncontent")
    manifest_path = str(tmp_path / "amirify-out" / "manifest.json")
...
def test_save_manifest_clear_ast_blanks_both_hashes_for_failed_extra(tmp_path):
    ...
    the next extract re-queues the file without deleting amirify-out/."""
    import json
...
# Regression tests for #945 - .gitignore fallback when no .amirifyignore exists

def test_gitignore_fallback_when_no_amirifyignore(tmp_path):
    """When no .amirifyignore exists, .gitignore patterns are honored (#945)."""
    (tmp_path / ".git").mkdir()
...
def test_amirifyignore_and_gitignore_are_merged(tmp_path):
    """When both exist, their patterns are MERGED — a file excluded only by
    .gitignore stays excluded even though .amirifyignore says nothing about it
    (#1363). Previously the presence of a .amirifyignore silently disabled the
    ...
    (tmp_path / ".amirifyignore").write_text("other.py\n")  # says nothing about main.py
    (tmp_path / "main.py").write_text("x = 1")
    (tmp_path / "other.py").write_text("x = 2")
    ...
    assert not any("other.py" in f for f in code)  # amirifyignore applied
    assert any("keep.py" in f for f in code)       # neither excludes it


def test_amirifyignore_negation_overrides_gitignore(tmp_path):
    """.amirifyignore is evaluated after .gitignore, so a `!` negation in it can
    re-include a file the .gitignore excluded (last-match-wins, #1363)."""
    (tmp_path / ".git").mkdir()
    (tmp_path / ".gitignore").write_text("*.py\n")           # exclude all .py
    (tmp_path / ".amirifyignore").write_text("!keep.py\n")  # but rescue keep.py
    (tmp_path / "main.py").write_text("x = 1")
    (tmp_path / "keep.py").write_text("x = 2")

    result = detect(tmp_path)
    code = result["files"]["code"]
    assert any("keep.py" in f for f in code)      # rescued by amirifyignore negation
...
def test_shebang_interpreter_plain(tmp_path):
    ...
    from amirify.detect import _shebang_interpreter
...
# relative keys so a committed ``amirify-out/`` round-trips across machines
...
def test_save_manifest_relativizes_keys_when_root_given(tmp_path):
    ...
    import json
    from amirify.detect import save_manifest, load_manifest
...
def test_save_manifest_without_root_keeps_absolute_keys(tmp_path):
    ...
    import json
    from amirify.detect import save_manifest
...
def test_load_manifest_absolutizes_relative_keys(tmp_path):
    ...
    import json
    from amirify.detect import load_manifest

    manifest_path = tmp_path / "amirify-out" / "manifest.json"
...
def test_detect_incremental_portable_across_paths(tmp_path):
    ...
    import json
    from amirify.detect import save_manifest, detect_incremental
    ...
    manifest_a = str(repo_a / "amirify-out" / "manifest.json")
    files = {
    ...
    (repo_b / "amirify-out").mkdir()
    manifest_b = repo_b / "amirify-out" / "manifest.json"
...
def test_convert_office_file_sidecar_name_stable_across_checkouts(tmp_path, monkeypatch):
    ...
    produces the same sidecar name (no unbounded duplicates when amirify-out/ is
    ...
    def _sidecar(root):
        ...
        out_dir = root / "amirify-out" / "converted"
    ...
    fallback = detect_mod.convert_office_file(
        checkout_a / "docs" / "report.xlsx", checkout_a / "amirify-out" / "converted"
    )
...
def test_convert_office_file_outside_root_falls_back(tmp_path, monkeypatch):
    ...
    root = tmp_path / "repo"
    (root / "amirify-out" / "converted").mkdir(parents=True)
...
def test_detect_office_conversion_respects_cache_root(tmp_path, monkeypatch):
    ...
    cache_root/AMIRIFY_OUT/converted, leaving the scanned corpus untouched, while
    ...
    assert not (corpus / detect_mod.AMIRIFY_OUT).exists(), (
        "detect() must not write amirify-out into the scanned corpus tree when cache_root is provided (#2787)"
    )

    # 2. Converted sidecar must exist under cache_root
    converted_dir = cache_out / detect_mod.AMIRIFY_OUT / "converted"
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("amirify-out/nlp", False, None, id="default-parent"),
...
def test_nested_amirify_out_prunes_only_configured_path(
    ...
    probe = subprocess.run(
        [
            ...
            (
                "import json, sys\n"
                "from pathlib import Path\n"
                "from amirify.detect import detect\n"
        ...
        env={**os.environ, "AMIRIFY_OUT": configured_out},
        check=True,
        capture_output=True,
        text=True,
    )
...
def test_amirifyinclude_is_inert_and_not_unclassified(tmp_path, capsys):
    """#2112: .amirifyinclude support was removed (dead since #873).

    A leftover .amirifyinclude must not error, must not surface in the
    ...
    (tmp_path / ".amirifyinclude").write_text(".github/\ndocs/**\n")
    result = detect(tmp_path)

    # not surfaced as an unclassified scan input
    assert not any(".amirifyinclude" in p for p in result["unclassified"])
    # real files are indexed exactly as before; the file changes nothing
    assert result["files"] == baseline["files"]
    assert any("main.py" in f for f in result["files"]["code"])
    # one-time stderr note, matching the [amirify] warning convention
    err = capsys.readouterr().err
    assert err.count("[amirify] WARNING: .amirifyinclude is no longer supported") == 1
...
def test_save_manifest_noop_skips_disk_write(tmp_path):
    ...
    a = tmp_path / "a.py"
    a.write_text("x = 1\n", encoding="utf-8")
    manifest_path = Path(tmp_path / "amirify-out" / "manifest.json")
...
def test_save_manifest_ast_kind_noop_then_change(tmp_path):
    """#2838's literal path: `amirify update` calls save_manifest with kind='ast'.
...
def test_sensitive_filter_indexes_topic_prose_and_source(path):
    from amirify.detect import _is_sensitive
...
def test_lexical_relative_matches_pathlib_relative_to():
    ...
    from pathlib import Path
    from amirify.detect import _lexical_relative, _nfc
...
def test_globstar_matcher_leaves_no_reference_cycle():
    ...
    import gc
    from amirify.detect import _match_anchored_ignore_pattern
