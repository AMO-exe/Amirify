...
the SKILL.md but never wrote `.agents/rules/amirify.md` or
`.agents/workflows/amirify.md` - even though the uninstall path removes them.
"""
import amirify.__main__ as m


def test_antigravity_project_install_writes_rules_and_workflows(tmp_path):
    m._project_install("antigravity", tmp_path)
    skill = tmp_path / ".agents" / "skills" / "amirify" / "SKILL.md"
    rules = tmp_path / ".agents" / "rules" / "amirify.md"
    workflow = tmp_path / ".agents" / "workflows" / "amirify.md"
...
def test_antigravity_workflow_names_no_skill_path(tmp_path):
    ...
    point at ~/.gemini/config/skills/amirify/SKILL.md, which it never writes.
    """
    m._project_install("antigravity", tmp_path)
    body = (tmp_path / ".agents" / "workflows" / "amirify.md").read_text(encoding="utf-8")
    ...
    assert "amirify skill" in body, "workflow should still point at the skill by name"
...
def test_antigravity_global_install_workflow_names_no_skill_path(tmp_path, monkeypatch):
    ...
    assert (
        tmp_path / "home" / ".gemini" / "config" / "skills" / "amirify" / "SKILL.md"
    ).exists()
...
def test_antigravity_project_uninstall_clears_rules_and_workflows(tmp_path):
    ...
    assert not (tmp_path / ".agents" / "rules" / "amirify.md").exists()
    assert not (tmp_path / ".agents" / "workflows" / "amirify.md").exists()
    assert not (tmp_path / ".agents" / "skills" / "amirify" / "SKILL.md").exists()
