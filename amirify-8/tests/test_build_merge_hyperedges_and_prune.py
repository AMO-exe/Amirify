...
build_merge backs `amirify --update`. Two regressions covered here:
...
from amirify.build import build_merge, _infer_merge_root
...
def test_prune_without_root_removes_ghost_nodes_via_grandparent_fallback(tmp_path):
    root = tmp_path / "corpus"
    (root / "amirify-out").mkdir(parents=True)
    graph_path = root / "amirify-out" / "graph.json"
    nodes = [
...
def test_prune_without_root_uses_amirify_root_marker(tmp_path):
    # graph.json not under a <root>/amirify-out layout, so grandparent wouldn't
    # help — the committed .amirify_root marker must be honored instead.
    out = tmp_path / "out"
    out.mkdir()
    graph_path = out / "graph.json"
    real_root = tmp_path / "elsewhere" / "repo"
    real_root.mkdir(parents=True)
    (out / ".amirify_root").write_text(str(real_root), encoding="utf-8")
...
def test_stale_amirify_root_marker_falls_back_to_amirify_out_parent(tmp_path):
    root = tmp_path / "repo"
    out = root / "amirify-out"
    out.mkdir(parents=True)
    graph_path = out / "graph.json"
    _write_graph(graph_path, [], [], [])
    (out / ".amirify_root").write_text(str(tmp_path / "moved" / "repo"), encoding="utf-8")
...
def test_prune_matches_across_symlinked_root(tmp_path):
    ...
    real = tmp_path / "real"
    (real / "amirify-out").mkdir(parents=True)
    link = tmp_path / "link"
    os.symlink(real, link)
    graph_path = real / "amirify-out" / "graph.json"
    _write_graph(graph_path, [
        {"id": "h1", "label": "handoff", "file_type": "document", "source_file": "HANDOFF.md"},
        {"id": "k1", "label": "keep", "file_type": "document", "source_file": "KEEP.md"},
    ], [], [])
...
def test_prune_matches_node_stored_absolute_against_relative_delete(tmp_path):
    ...
    # Runbook-style: NO root passed (eff_root inferred from the amirify-out
...
def test_amirify_root_marker_with_a_utf8_bom_still_resolves(tmp_path):
