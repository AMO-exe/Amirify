...
skillgen renders amirify's committed skill artifacts from human-edited
...
def test_no_version_or_timestamp_in_output():
    ...
    from amirify.__main__ import __version__
...
def _claude_artifacts():
    ...
    core = next(a for a in arts if a.path == "amirify/skill.md")
    refs = {a.path.rsplit("/", 1)[-1]: a.content for a in arts if a.path != "amirify/skill.md"}
...
def test_lean_core_has_no_reference_only_content():
    ...
    assert "from amirify.build import build_merge" not in core
    assert "amirify cluster-only ." not in core
    ...
    assert "amirify export wiki" not in core
    assert "amirify export neo4j" not in core
    # The add / watch / hook flows live only in their references.
    assert "from amirify.ingest import ingest" not in core
    assert "amirify hook install" not in core
    assert "python3 -m amirify.watch" not in core
...
def test_lean_core_runs_default_pipeline_with_zero_references():
    ...
    for needed in (
        "### Step 1 - Ensure amirify is installed",
        "### Step 2 - Detect files",
...
def test_extraction_states_no_api_key_required_for_every_host():
    ...
    for a in bodies:
        assert "amirify needs no API key" in a.content, a.path
        ...
        if tip in a.content:
            assert a.content.index("amirify needs no API key") < a.content.index(tip), \
...
def test_references_contain_no_core_pipeline_content():
    ...
    core_only_markers = (
        "from amirify.cluster import cluster, score_all",
        "### Step 4 - Build graph, cluster, analyze, generate outputs",
        "### Step 5 - Label communities",
        "## Honesty Rules",
    )
...
def test_query_heading_is_homed_in_core_stub_only():
    ...
    assert "## For /amirify query" in core_headings
    assert "## For /amirify query" not in query_headings
    # The deeper query content moved into the reference.
    assert "## For /amirify path" in query_headings
    assert "## For /amirify explain" in query_headings
    assert "## For /amirify path" not in core_headings
...
UNIFIED_DESCRIPTION = (
    ...
    "or project content — especially when amirify-out/ exists, where the question "
    "should be treated as an amirify query first. Turns any input (code, docs, "
...
CODEX_ASTRA_NOTE = (
    " Tuned for Codex with GPT Astra 6: the agent reads the compact graph and report "
    "instead of re-reading raw files, so it can keep full-depth reasoning while spending "
    "far fewer tokens and credits."
)
...
def test_descriptions_are_unified():
    ...
    for key, p in platforms.items():
        # The codex host alone appends a GPT Astra 6 tuning note to the shared line.
        description = UNIFIED_DESCRIPTION + CODEX_ASTRA_NOTE if key == "codex" else UNIFIED_DESCRIPTION
        expected_line = f'description: "{description}"'
        ...
        assert "treat the question as a /amirify query." not in body, f"[{key}] kept old wording"
        assert "clustered communities" not in body, f"[{key}] kept old wording"


def test_windows_frontmatter_name_and_shell_and_extra():
    """windows: name must be `amirify` (folder-name rule, #1635), powershell
    ...
    # (amirify); a `amirify-windows` name broke skill discovery (#1635).
    assert core.startswith("---\nname: amirify\n")
    assert "```powershell" in core
    assert "function Find-AmirifyPython" in core
...
def test_every_platform_query_has_expansion_and_fallback():
    ...
    for key in ("claude", "codex", "windows", "opencode"):
        ...
        assert "## For /amirify path" in q
        assert "## For /amirify explain" in q
...
def test_powershell_hosts_carry_no_bash_only_shell():
    ...
    for key in _powershell_platform_keys():
        ...
        assert "```powershell" in core
        assert "'@ | & (Get-Content amirify-out\\.amirify_python) -" in core, (
        ...
        assert "Get-ChildItem amirify-out -Filter '.amirify_chunk_*.json'" in core
...
def test_windows_and_posix_cores_have_step_and_2490_parity():
    ...
    line_2490 = "to_json(G, communities, 'amirify-out/graph.json', community_labels=labels)"
...
def test_powershell_translator_rejects_unknown_bash():
    ...
    assert gen._rm_to_remove_item("rm -f amirify-out/.needs_update 2>/dev/null || true") == (
        "Remove-Item -Force -ErrorAction SilentlyContinue amirify-out\\.needs_update"
    )
...
def test_monoliths_render_inline_single_file_no_references():
    ...
    for key in ("aider", "devin"):
        ...
        assert arts[0].path == f"amirify/skill-{key}.md"
...
def test_monoliths_carry_the_1392_runbook_fixes():
    ...
    for key in ("aider", "devin"):
        ...
        assert ".amirify_cached.json').unlink(missing_ok=True)" in body
...
def test_monoliths_scope_semantic_cache_writes_to_uncached_files():
    ...
    for key in ("aider", "devin"):
        body = gen.render(platforms[key])[0].content
        assert ".amirify_uncached.txt').read_text(" in body
...
def test_generated_runbooks_pass_root_to_save_manifest():
    ...
    """
    targets = [
        REPO_ROOT / "amirify" / "skill.md",
        REPO_ROOT / "amirify" / "skill-aider.md",
        REPO_ROOT / "amirify" / "skill-devin.md",
    ]
    targets += sorted((REPO_ROOT / "amirify" / "skills").glob("*/references/update.md"))
    checked = 0
    for path in targets:
...
def test_always_on_renders_six_blocks():
    ...
    assert paths == [
        "amirify/always_on/agents-md.md",
        "amirify/always_on/antigravity-rules.md",
        "amirify/always_on/claude-md.md",
        "amirify/always_on/gemini-md.md",
        "amirify/always_on/kiro-steering.md",
        "amirify/always_on/vscode-instructions.md",
    ]
...
def test_always_on_included_in_full_render_not_per_platform():
    ...
    assert "amirify/always_on/claude-md.md" in full
    assert "amirify/always_on/claude-md.md" not in claude_only
...
def test_always_on_roundtrip_is_byte_faithful():
    ...
    amirify.__main__, so the packaged markdown must round-trip exactly or those
    ...
    rendered_agents = next(
        ...
        if a.path == "amirify/always_on/agents-md.md"
    )
    old_instruction = (
        "When the user types `/amirify`, invoke the `skill` tool with "
        '`skill: "amirify"` before doing anything else.'
    )
    new_instruction = (
        "When the user types `/amirify`, use the installed amirify skill or instructions "
    ...
    assert 'skill: "amirify"' not in rendered_agents
...
def test_extracted_constants_equal_the_packaged_always_on_files():
    ...
    from amirify import __main__ as mainmod
...
def test_always_on_files_are_guarded_by_check(tmp_path):
    ...
    mutated = [
        ...
        if a.path == "amirify/always_on/claude-md.md"
        else a
        for a in arts
    ]
...
def test_audit_reads_each_host_against_its_own_v8_body():
    ...
    """
    assert gen._v8_baseline_ref("claude") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:amirify/skill.md"
    assert gen._v8_baseline_ref("trae") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:amirify/skill-trae.md"
    assert gen._v8_baseline_ref("vscode") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:amirify/skill-vscode.md"
...
def test_trae_renders_native_agents_md_integration_not_claude():
    """trae wires `amirify trae install` -> AGENTS.md, never `amirify claude install`."""
    ...
    assert "amirify trae install" in hooks
    assert "amirify trae-cn install" in hooks
    assert "writes a `## amirify` section to the local `AGENTS.md`" in hooks
    # The claude-flavored install command must NOT appear for trae.
    assert "amirify claude install" not in hooks
    ...
    assert "wire amirify into a project's AGENTS.md" in core
...
def test_trae_hooks_reference_includes_the_pretooluse_note():
    ...
    assert "Run `/amirify --update` manually after code changes" in hooks
...
def test_claude_flavored_hosts_keep_their_hooks_text_unchanged():
    ...
    for key in ("claude", "droid", "codex", "windows", "kilo", "vscode"):
        core, refs = _platform_artifacts(key)
        hooks = refs["hooks.md"]
        assert "amirify claude install" in hooks, f"[{key}] lost the claude install command"
...
def test_amp_renders_native_agents_md_integration_v8_faithfully():
    """amp wires `amirify amp install` -> AGENTS.md exactly as its v8 body had it.
    ...
    assert "make amirify always-on in Amp sessions" in hooks
    assert "instructs Amp to check the graph" in hooks
    # amp's single-line install/uninstall, no trae-cn alt comments.
    assert "amirify amp install" in hooks
    assert "amirify amp uninstall  # remove the section" in hooks
    assert "amirify trae install" not in hooks
    assert "amirify trae-cn" not in hooks
    assert "or: amirify" not in hooks
...
def test_amp_audit_coverage_passes_against_its_own_v8():
    ...
    against a lean split. The audit reads origin/v8:amirify/skill-amp.md and
    confirms every heading single-homes in amp's core + references.
    """
    platforms = gen.load_platforms()
    assert gen._v8_baseline_ref("amp") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:amirify/skill-amp.md"
...
def test_agents_renders_its_own_agents_md_hooks_wording():
    ...
    `amirify agents install` and is worded for an unspecified host.
    """
    core, refs = _platform_artifacts("agents")
    hooks = refs["hooks.md"]
    assert "## For native AGENTS.md integration" in hooks
    assert "## For native AGENTS.md integration (Trae)" not in hooks
    assert "make amirify always-on in your agent sessions" in hooks
    assert "amirify agents install" in hooks
    assert "amirify agents uninstall  # remove the section" in hooks
    # No amp/trae/claude wording leaks into the agents render.
    assert "amirify amp install" not in hooks
    assert "amirify trae" not in hooks
    assert "amirify claude install" not in hooks
...
def test_agents_audit_baseline_is_amps_v8_body():
    ...
    platforms = gen.load_platforms()
    assert gen._v8_baseline_ref("agents") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:amirify/skill-amp.md"
...
def test_windows_skill_writes_marker_files_without_a_bom():
    ...
    it. The BOM then rode into `.amirify_python` / `.amirify_root`, so every
    ...
    """
    core, _ = _platform_artifacts("windows")
    for marker in (".amirify_python", ".amirify_root"):
        assert f"Out-File -FilePath amirify-out\\{marker} -Encoding utf8" not in core, (
            f"the windows render still writes {marker} with a BOM-emitting Out-File"
        )
        assert f"WriteAllText((Join-Path $PWD 'amirify-out\\{marker}')" in core, (
