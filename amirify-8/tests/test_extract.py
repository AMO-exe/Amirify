...
from amirify.extract import extract_python, extract, collect_files, _make_id, extract_bash, extract_json, _DISPATCH
...
def test_go_imported_type_stubs_do_not_collide_across_source_files(tmp_path):
    ...
    #1462 kept unresolved bare stubs distinct per source file because Amirify
...
def test_collect_files_from_dir():
    from amirify.extract import _DISPATCH
...
def test_extract_generic_surfaces_tree_sitter_version_mismatch_hint(monkeypatch):
    ...
    import sys
    import types
    from amirify.extract import _extract_generic, LanguageConfig
...
def test_extract_js_destructured_require_imports_from():
    ...
    from amirify.extract import extract_js
...
def test_extract_js_destructured_require_named_symbols():
    ...
    from amirify.extract import extract_js, _make_id, _file_stem
    result = extract_js(FIXTURES / "cjs_require.js")
...
def test_extract_js_nested_function_declarations(tmp_path):
    ...
    from amirify.extract import extract
...
def test_extract_tsx_uses_tsx_grammar():
    ...
    from amirify.extract import _TSX_CONFIG, _TS_CONFIG
...
def test_extract_falls_back_to_sequential_when_parallel_returns_false(tmp_path, monkeypatch):
    ...
    from amirify import extract as extract_mod
...
def test_extract_parallel_skips_pool_when_max_workers_is_one(tmp_path, monkeypatch):
    ...
    The Windows post-commit hook exports AMIRIFY_MAX_WORKERS=1, so before this the
    ...
    monkeypatch.setenv("AMIRIFY_MAX_WORKERS", "1")
...
def test_extract_parallel_still_spawns_pool_for_multiple_workers(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AMIRIFY_MAX_WORKERS", "4")
...
def test_extract_falls_back_when_worker_future_breaks_pool(
    ...
    monkeypatch.setenv("AMIRIFY_MAX_WORKERS", "2")
...
def test_semantic_reference_edges_carry_context_and_source():
    from amirify.extract import _semantic_reference_edge
...
def test_dart_child_node_ids_are_stem_based(tmp_path):
    ...
    from amirify.extract import extract_dart, _file_stem, _make_id
...
def test_non_colliding_path_id_is_not_salted(tmp_path):
    ...
    from amirify.ids import make_id
...
def test_get_extractor_routes_matlab_m_away_from_objc(tmp_path):
    ...
    from amirify.extract import _get_extractor, extract_objc
...
def test_markdown_dispatch_matches_resolution_suffixes():
    from amirify.extract import _DISPATCH, extract_markdown
    from amirify.markdown_resolution import MARKDOWN_MENTION_SUFFIXES
...
def test_rewire_binds_cross_module_function_reference_to_definition():
    ...
    rewire targets)."""
    from amirify.extract import _rewire_unique_stub_nodes
    nodes = [
...
def test_3252_unimported_ambiguous_type_remains_unresolved(tmp_path):
    """#3252: When multiple definitions exist but consumer has no import, Amirify
