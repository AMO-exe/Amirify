...
def test_pdf_routed_through_pypdf_not_readtext(tmp_path, monkeypatch):
    ...
    import amirify.detect as detect
...
def test_capability_flags(monkeypatch):
    ...
    monkeypatch.delenv("AMIRIFY_OLLAMA_VISION", raising=False)
    assert not llm._backend_supports_vision("ollama")
    monkeypatch.setenv("AMIRIFY_OLLAMA_VISION", "1")
...
def test_anthropic_response_text_returns_first_text_block_not_concatenation():
    """Locks the first-wins semantics: amirify's claude calls return a single
...
def test_call_bedrock_honors_api_timeout(monkeypatch):
    # AMIRIFY_API_TIMEOUT must reach the botocore client's read_timeout; else
    ...
    monkeypatch.setenv("AMIRIFY_API_TIMEOUT", "1800")
