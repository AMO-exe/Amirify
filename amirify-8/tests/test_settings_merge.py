...
- back up to <name>.amirify-bak before any modifying write,
...
def test_claude_install_preserves_existing_settings(tmp_path):
    """#2167 core case: every key amirify does not own must survive install."""
    seeded = {
    ...
    # top-level keys amirify does not own are untouched
    assert result["mcpServers"] == seeded["mcpServers"]
    assert result["enabledPlugins"] == seeded["enabledPlugins"]
    assert result["theme"] == "dark"
    # hooks sections amirify does not manage are untouched
    assert result["hooks"]["PostToolUse"] == seeded["hooks"]["PostToolUse"]
    # the user's own PreToolUse entry survives alongside amirify's
    pre_tool = result["hooks"]["PreToolUse"]
    assert seeded["hooks"]["PreToolUse"][0] in pre_tool
    amirify_hooks = [h for h in pre_tool if "amirify" in str(h)]
    assert len(amirify_hooks) == 2
    # strict=True lands on the read guard
    assert any(h["hooks"][0]["command"].endswith("--strict") for h in amirify_hooks)
...
def test_bom_settings_are_merged_not_clobbered(tmp_path, installer):
    ...
    assert any("amirify" in str(h) for h in result["hooks"][section])
...
def test_invalid_json_aborts_without_clobbering(tmp_path, installer, capsys):
    ...
    assert not settings_path.with_name(settings_path.name + ".amirify-bak").exists()
...
def test_backup_written_before_modify_and_stable_on_reinstall(tmp_path, installer):
    ...
    backup = settings_path.with_name(settings_path.name + ".amirify-bak")
    ...
    # nor the backup may be rewritten (the backup keeps the pre-amirify content).
    _run(tmp_path, installer)
...
def test_non_dict_hook_entry_is_preserved_not_fatal(tmp_path, installer):
    ...
    assert any("amirify" in str(h) for h in entries)
