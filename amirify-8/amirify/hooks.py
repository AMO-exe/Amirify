# git hook integration - install/uninstall amirify post-commit and post-checkout hooks
...
_HOOK_MARKER = "# amirify-hook-start"
_HOOK_MARKER_END = "# amirify-hook-end"
_CHECKOUT_MARKER = "# amirify-checkout-hook-start"
_CHECKOUT_MARKER_END = "# amirify-checkout-hook-end"

# __PINNED_PYTHON__ is replaced at install time with the absolute path of the
# Python interpreter that ran `amirify hook install`.  For uv-tool and pipx
...
# when the amirify launcher is not on PATH (common in GUI clients and CI).
#
# Probes check availability with importlib.util.find_spec instead of importing
# the package: a probe that imports amirify wholesale executes the full package
...
_GFY_PROBE="import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('amirify') else 1)"
AMIRIFY_PYTHON=""
_PINNED='__PINNED_PYTHON__'
if [ -n "$_PINNED" ] && [ -x "$_PINNED" ] && "$_PINNED" -c "$_GFY_PROBE" 2>/dev/null; then
    AMIRIFY_PYTHON="$_PINNED"
fi
# Second probe: read amirify-out/.amirify_python (written by the skill and
# CLI; survives uv-tool reinstalls and is the same source the README documents).
if [ -z "$AMIRIFY_PYTHON" ]; then
    _GFY_PYTHON_FILE="amirify-out/.amirify_python"
    if [ -f "$_GFY_PYTHON_FILE" ]; then
        ...
        if [ -n "$_FROM_FILE" ] && [ -x "$_FROM_FILE" ] && "$_FROM_FILE" -c "$_GFY_PROBE" 2>/dev/null; then
            AMIRIFY_PYTHON="$_FROM_FILE"
        fi
    fi
fi
# Third probe: resolve via the amirify launcher on PATH.
if [ -z "$AMIRIFY_PYTHON" ]; then
    AMIRIFY_BIN=$(command -v amirify 2>/dev/null)
    if [ -n "$AMIRIFY_BIN" ]; then
        # Windows pip layout: Scripts/amirify(.exe) sits beside ..\\python.exe
    ...
    if [ -z "$AMIRIFY_PYTHON" ] && [ -n "$AMIRIFY_BIN" ]; then
        ...
        case "$AMIRIFY_BIN" in
            *.exe) _GFY_HEAD="" ;;
            *)     _GFY_HEAD=$(head -c 256 "$AMIRIFY_BIN" 2>/dev/null | tr -d '\\000') ;;
        esac
        case "$_GFY_HEAD" in
        ...
        case "$_SHEBANG" in
            */env\\ *) AMIRIFY_PYTHON="${_SHEBANG#*/env }" ;;
            *)         AMIRIFY_PYTHON="$_SHEBANG" ;;
        esac
        # Allowlist: only keep characters valid in a filesystem path to prevent
        # injection if the shebang contains shell metacharacters.
        case "$AMIRIFY_PYTHON" in
            *[!a-zA-Z0-9/_.@:\\\\-]*) AMIRIFY_PYTHON="" ;;
        esac
        if [ -n "$AMIRIFY_PYTHON" ] && ! "$AMIRIFY_PYTHON" -c "$_GFY_PROBE" 2>/dev/null; then
            AMIRIFY_PYTHON=""
        fi
    fi
fi
# Fourth probe: uv tool environments. `uv tool install` (the README's
# recommended method) puts amirify in an isolated venv that no ambient
...
# co-installed tool without amirify never satisfies it.
#
...
if [ -z "$AMIRIFY_PYTHON" ]; then
    if command -v python3 >/dev/null 2>&1 && python3 -c "$_GFY_PROBE" 2>/dev/null; then
        AMIRIFY_PYTHON="python3"
    elif command -v python >/dev/null 2>&1 && python -c "$_GFY_PROBE" 2>/dev/null; then
        AMIRIFY_PYTHON="python"
    else
        echo "[amirify hook] could not locate a Python with amirify installed. Add the amirify bin dir to PATH or re-run 'amirify hook install' from the env where amirify lives." >&2
        exit 0
    fi
fi
"""
...
changed_raw = os.environ.get('AMIRIFY_CHANGED', '')
changed = [Path(f.strip()) for f in changed_raw.strip().splitlines() if f.strip()]

if not changed:
    sys.exit(0)

print(f'[amirify hook] {len(changed)} file(s) changed - rebuilding graph...')

try:
    from amirify.watch import _rebuild_code, _apply_resource_limits
    _apply_resource_limits()
    _timeout = int(os.environ.get('AMIRIFY_REBUILD_TIMEOUT', '600'))
    if _timeout > 0:
        if hasattr(signal, 'SIGALRM'):
            def _sigalrm_bail(*_a):
                ...
                for _child in multiprocessing.active_children():
                    _child.kill()
                raise TimeoutError(f'amirify rebuild exceeded {_timeout}s')
            signal.signal(signal.SIGALRM, _sigalrm_bail)
            signal.alarm(_timeout)
        else:
            def _bail():
                print(f'[amirify hook] amirify rebuild exceeded {_timeout}s', flush=True)
                for _child in multiprocessing.active_children():
                    _child.kill()
                os._exit(1)
            ...
            _watchdog.start()
    _force = os.environ.get('AMIRIFY_FORCE', '').lower() in ('1', 'true', 'yes')
    _root = Path('.')
    _out = os.environ.get('AMIRIFY_OUT', 'amirify-out')
    _saved = Path(_out) / '.amirify_root'
    if _saved.exists():
        ...
        if _txt:
            ...
            if _in_repo:
                ...
                print(f'[amirify hook] ignoring out-of-repo .amirify_root: {_txt}')
    ...
    try:
        ...
        if _md.is_dir() and any(_md.glob('*.md')):
            from amirify.reflect import reflect as _reflect
    ...
    print(f'[amirify hook] {exc}')
    sys.exit(1)
except Exception as exc:
    print(f'[amirify hook] Rebuild failed: {exc}')
    sys.exit(1)
"""
...
try:
    ...
    if _timeout > 0:
        if hasattr(signal, 'SIGALRM'):
            ...
            def _bail():
                print(f'[amirify] amirify rebuild exceeded {_timeout}s', flush=True)
                for _child in multiprocessing.active_children():
                    _child.kill()
                os._exit(1)
            _watchdog = threading.Timer(_timeout, _bail)
    ...
    if _saved.exists():
        ...
        if _txt:
            ...
            if _in_repo:
                ...
                print(f'[amirify] ignoring out-of-repo .amirify_root: {_txt}')
    ...
    print(f'[amirify] Rebuild failed: {exc}')
    sys.exit(1)
"""

# Cross-platform detached-launch shim (#1161). The hooks used to background the
# rebuild with `nohup "$AMIRIFY_PYTHON" -c "..." &`, but Git for Windows' bundled
...
# still returned 0, so the graph just went stale with no signal. amirify already
...
__REBUILD_BODY__
'''
_log = os.environ.get('AMIRIFY_REBUILD_LOG') or os.path.join(os.path.expanduser('~'), '.cache', 'amirify-rebuild.log')
try:
...
def _detached_launch(rebuild_body: str) -> str:
    ...
    Python process via ``$AMIRIFY_PYTHON``.
    ...
    The launcher writes the child's output to ``$AMIRIFY_REBUILD_LOG`` and
    ...
    return '"$AMIRIFY_PYTHON" -c "' + launcher + '"\n'
...
# fires these hooks; the canonical amirify-out/ belongs to the primary checkout,
...
# rebuild ("failed to remove amirify-out/: Directory not empty") (#1809, #1806).
...
# anything after amirify's end marker - on every root commit (HEAD~1 does
# not exist), every rebase/merge, every linked worktree, every
# AMIRIFY_SKIP_HOOK=1 (#2986). Inside the subshell an `exit` ends only
# amirify's section; the detached rebuild launch is unaffected, and the
# hook's own exit status stays 0 as before.
_HOOK_SCRIPT = """\
# amirify-hook-start
# Auto-rebuilds the knowledge graph after each commit (code files only, no LLM needed).
# Installed by: amirify hook install
(
...
# churn run-to-run. Pinning it makes amirify-out reproducible.
...
# explicit AMIRIFY_MAX_WORKERS still wins for users who want parallelism.
if [ -n "${WINDIR:-}" ] || [ -n "${MSYSTEM:-}" ]; then
    export AMIRIFY_MAX_WORKERS="${AMIRIFY_MAX_WORKERS:-1}"
fi
...
# outputs are tracked in git). The dir is whatever AMIRIFY_OUT names, the same
# source the rebuild body reads (#1423): a literal amirify-out/ here let a
# commit touching only a renamed output dir's graph.json trigger a full rebuild.
_GFY_OUT="${AMIRIFY_OUT:-amirify-out}"
_GFY_OUT="${_GFY_OUT%/}"
...
""" + _PYTHON_DETECT + """
export AMIRIFY_CHANGED="$CHANGED"
...
_AMIRIFY_LOG="${HOME}/.cache/amirify-rebuild.log"
mkdir -p "$(dirname "$_AMIRIFY_LOG")"
export AMIRIFY_REBUILD_LOG="$_AMIRIFY_LOG"
echo "[amirify hook] launching background rebuild (log: $_AMIRIFY_LOG)"
...
# from AMIRIFY_OUT like the rebuild body does (#1423): a literal amirify-out/
...
# Honor the same opt-out as post-commit: without this, AMIRIFY_SKIP_HOOK=1
...
mkdir -p "$(dirname "$_AMIRIFY_LOG")"
export AMIRIFY_REBUILD_LOG="$_AMIRIFY_LOG"
echo "[amirify] Branch switched - launching background rebuild (log: $_AMIRIFY_LOG)"
""" + _detached_launch(_REBUILD_BODY_CHECKOUT) + """)
# amirify-checkout-hook-end
"""


def _load_amirifyrc(root: Path) -> dict[str, str | int]:
    """Load key/value options from <root>/.amirifyrc if present.

    Supported options:
      viz_node_limit: integer >= 0 (e.g. viz_node_limit=0)
    """
    rc_path = root / ".amirifyrc"
...
def _hooks_dir(root: Path) -> Path:
    ...
    try:
        ...
        if res.returncode != 0:
            ...
            print(
                f"[amirify hooks] git could not resolve the hooks path for "
...
def _install_hook(
    ...
    an existing amirify block in-place."""
...
def _uninstall_hook(hooks_dir: Path, name: str, marker: str, marker_end: str) -> str:
    """Remove amirify section from a git hook using start/end markers."""
    hook_path = hooks_dir / name
    if not hook_path.exists():
        return f"no {name} hook found - nothing to remove."
    content = hook_path.read_text(encoding="utf-8")
    if marker not in content:
        return f"amirify hook not found in {name} - nothing to remove."
    new_content = re.sub(
    ...
    return f"amirify removed from {name} at {hook_path} (other hook content preserved)"
...
def _pinned_python() -> str:
    ...
    means callers must fall back to the `amirify` launcher on PATH — safe
    degradation.
    """
...
def _merge_attr_line() -> str:
    """The .gitattributes line assigning the amirify merge driver to graph.json.

    The graph lives under the configured output directory (amirify.paths,
    AMIRIFY_OUT env override). gitattributes patterns are repo-relative, so an
    absolute output-dir override cannot be expressed there — fall back to the
    default name in that case.
    """
    from amirify.paths import AMIRIFY_OUT
    out = AMIRIFY_OUT
    if not out or Path(out).is_absolute() or "\\" in out:
        out = "amirify-out"
    return f"{out.rstrip('/')}/graph.json merge=amirify"


def _has_merge_attr(content: str) -> bool:
    """True if a (non-comment) `<...>graph.json ... merge=amirify` line exists."""
    for raw in content.splitlines():
        line = raw.strip()
        if not line or line.startswith("#"):
            continue
        fields = line.split()
        if fields and fields[0].endswith("graph.json") and "merge=amirify" in fields[1:]:
            return True
    return False
...
def _register_merge_driver(root: Path) -> str:
    ...
    README and CHANGELOG 0.7.0 document `amirify merge-driver` as being set up
    ...
    same way the hook scripts pin it, so the driver works even when the amirify
    ...
    if pinned:
        ...
        driver = f'"{pinned}" -m amirify merge-driver %O %A %B'
    else:
        driver = "amirify merge-driver %O %A %B"
    try:
        for key, value in (
            ("merge.amirify.name", "amirify graph.json union merge"),
            ("merge.amirify.driver", driver),
        ):
            _sp.run(
...
def _unregister_merge_driver(root: Path) -> str:
    ...
    import subprocess as _sp
    for key in ("merge.amirify.name", "merge.amirify.driver"):
        try:
...
def _merge_driver_status(root: Path) -> str:
    ...
    try:
        res = _sp.run(
            ["git", "-C", str(root), "config", "--get", "merge.amirify.driver"],
            capture_output=True, text=True,
        )
...
def install(path: Path = Path(".")) -> str:
    """Install amirify post-commit and post-checkout hooks in the nearest git repo."""
    root = _git_root(path)
    if root is None:
    ...
    cfg = _load_amirifyrc(root)
    viz_limit = cfg.get("viz_node_limit")
    if viz_limit is not None:
        # Use the `:-` default form (like AMIRIFY_MAX_WORKERS below) so an
        # explicit `AMIRIFY_VIZ_NODE_LIMIT=... git commit` still wins over the
        # baked project default — persisting config must not clobber a per-run
        # override.
        viz_export = f'export AMIRIFY_VIZ_NODE_LIMIT="${{AMIRIFY_VIZ_NODE_LIMIT:-{viz_limit}}}"\n'
    else:
        viz_export = ""
...
def uninstall(path: Path = Path(".")) -> str:
    """Remove amirify post-commit and post-checkout hooks."""
    root = _git_root(path)
    if root is None:
        raise RuntimeError(f"No git repository found at or above {path.resolve()}")
...
def status(path: Path = Path(".")) -> str:
    """Check if amirify hooks are installed."""
    root = _git_root(path)
    if root is None:
        return "Not in a git repository."
    hooks_dir = _user_hooks_dir(_hooks_dir(root))
    # status is a read-only diagnostic: a malformed .amirifyrc must not turn it
    ...
    def _check(name: str, marker: str) -> str:
        ...
        if marker not in text:
            return "not installed (hook exists but amirify not found)"
        if cfg_limit is not None:
            # Baked as `"${AMIRIFY_VIZ_NODE_LIMIT:-<n>}"` so a per-run override
            # wins; match the default <n>, and still accept the older bare
            # `"<n>"` form from hooks installed before that change.
            m = re.search(
                r'export AMIRIFY_VIZ_NODE_LIMIT="(?:\$\{AMIRIFY_VIZ_NODE_LIMIT:-(\d+)\}|(\d+))"',
                text,
            )
