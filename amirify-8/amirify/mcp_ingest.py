...
and turns the `mcpServers` map into Amirify nodes and edges.

Symmetry with `serve.py`: Amirify exposes itself AS an MCP server. This module
indexes MCP servers AS a corpus type, completing the loop — an agent that runs
amirify with `--mcp` can now query its own configured MCP layer.

Entry point:
  extract_mcp_config(path: Path) -> dict[str, list[dict]]

  Returns `{"nodes": [...], "edges": [...]}` compatible with Amirify's
...
def extract_mcp_config(path: Path) -> dict[str, Any]:
    """Parse an MCP config file into Amirify nodes and edges.
...
# ── Node / edge construction (Amirify schema) ────────────────────────────────
...
# only amirify.ids), so this can no longer drift from extract._file_stem.
from amirify.extractors.base import _file_stem  # noqa: E402
