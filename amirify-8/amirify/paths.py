"""Single source of truth for the amirify output-directory name.

The output directory is ``amirify-out`` by default and overridable with the
``AMIRIFY_OUT`` env var (worktrees or shared-output setups, #686). It accepts a
relative name (``"amirify-out-feature"``) or an absolute path
(``"/shared/amirify-out"``).

This used to be duplicated as an identical ``_AMIRIFY_OUT`` constant in
``__main__``, ``cache``, and ``watch``, while ``security`` and ``callflow_html``
hardcoded the literal ``"amirify-out"`` and silently ignored the override
...
``AMIRIFY_OUT`` before the process starts (the normal worktree/shared-output
...
AMIRIFY_OUT = os.environ.get("AMIRIFY_OUT", "amirify-out")
...
# Bare directory name even when AMIRIFY_OUT is an absolute path. Used by path
# guards that walk parents looking for the output directory by name.
AMIRIFY_OUT_NAME = os.path.basename(os.path.normpath(AMIRIFY_OUT))


def out_path(*parts: str) -> Path:
    """A path inside the configured output dir, e.g. ``out_path("cache")``.

    ``Path(AMIRIFY_OUT) / ...`` resolves correctly for both a relative name
    ("amirify-out") and an absolute override ("/shared/amirify-out").
    """
    return Path(AMIRIFY_OUT, *parts)
...
def default_graph_json() -> str:
    ...
    commands so a ``AMIRIFY_OUT`` override is honoured everywhere, not just where
...
def stem_filename_budget(output_dir: "str | Path", *, reserve: int = 0, limit: int = 200) -> int:
    ...
    ``C:\\Users\\me\\projects\\svc\\amirify-out\\obsidian`` exceeds MAX_PATH and
...
def load_node_link_graph(path_or_data):
    """Load an amirify graph.json into a networkx graph, accepting both writers.
    ...
    if not isinstance(data, dict):
        p = Path(data)
        from amirify.security import check_graph_file_size_cap  # lazy: security imports paths
