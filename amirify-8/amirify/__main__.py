"""amirify CLI - `amirify install` sets up the Claude Code skill."""
...
try:
    ...
    __version__ = _pkg_version("amirify")
except Exception:
    __version__ = "unknown"

# Output directory — override with AMIRIFY_OUT env var for worktrees or shared-output setups.
# Accepts a relative name ("amirify-out-feature") or an absolute path ("/shared/amirify-out").
# Defined once in amirify.paths so the security/callflow path guards honour the
# same override (#1423).
from amirify.paths import AMIRIFY_OUT as _AMIRIFY_OUT

# Install/uninstall subsystem moved to amirify/install.py; re-exported here so
# `from amirify.__main__ import <name>` keeps working unchanged.
from amirify.install import (  # noqa: E402,F401
    ...
    _resolve_amirify_exe,
    _skill_registration,
    _strip_amirify_hook,
    _strip_amirify_md_section,
...
from amirify.cli import (  # noqa: E402,F401
    dispatch_command,
    _StageTimer,
    _clone_repo,
    _default_graph_path,
...
def __getattr__(name: str) -> str:
    ...
    # brick the CLI on `import amirify.__main__` (#1121 follow-up).
...
def _check_skill_version(skill_dst: Path, platform_names: "list[str] | None" = None) -> None:
    """Warn if the installed skill is from an older amirify version.
    ...
    a plain `amirify install` only refreshes the detected platform, and a
    ...
    if platform_names is None:
        try:
            ...
            platform_names = []
    version_file = skill_dst.parent / ".amirify_version"
    try:
    ...
    try:
        ...
        return
    if not skill_exists:
        print("  warning: skill dir exists but SKILL.md is missing. Run 'amirify install' to repair.", file=sys.stderr)
        return
    ...
    if "references/" in body and not (skill_dst.parent / "references").exists():
        print("  warning: skill references/ sidecar is missing. Run 'amirify install' to repair.", file=sys.stderr)
    try:
    ...
    if installed != __version__:
        if _version_tuple(installed) > _version_tuple(__version__):
            # The skill on disk is NEWER than the running package. `amirify install`
            ...
            print(
                f"  warning: skill is from amirify {installed}, but the package is "
                f"{__version__} (older). Upgrade the package "
                f"(e.g. 'uv tool upgrade amirify' or 'pip install -U amirify'); "
                f"running 'amirify install' would downgrade the skill.",
                file=sys.stderr,
            )
        else:
            _cmd = (
                f"amirify install --platform {platform_names[0]}"
                if platform_names else "amirify install"
            )
            print(
                f"  warning: skill at {skill_dst.parent} is from amirify {installed}, "
                f"package is {__version__}. Run '{_cmd}' to update it "
                f"(a plain 'amirify install' refreshes only the detected platform).",
                file=sys.stderr,
            )
...
# `amirify hook-guard` subcommand (see _run_hook_guard). The previous hooks
...
# silently vanished — users had to invoke /amirify by hand (#522). Moving the
...
# The always-on instruction blocks are packaged markdown under amirify/always_on/,
...
# when a graph exists. Emitted by `amirify hook-guard gemini`. The old hook was a
...
_KIRO_STEERING_MARKER = "amirify: A knowledge graph of this project"
        ...
        "PreToolUse": [
            {
                ...
                "hooks": [
                    {
                        "type": "command",
                        # Use the amirify CLI itself so the hook is shell-agnostic:
...
def _pin_hash_seed_if_needed() -> None:
    ...
    A bare `amirify update .` (which the CLAUDE.md template tells agents to
    run after every code change) skipped this, re-clustering differently
    every time and producing a large, spurious amirify-out diff.
    ...
    that disallows it should still be able to run amirify, just without
...
def main() -> None:
    ...
    stdout early, amirify treats it as success instead of crashing with an
...
def _run_cli() -> None:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] in ("-v", "--version", "version"):
        print(f"amirify {__version__}")
        return

    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        print("Usage: amirify <command>")
        print()
        print("Commands:")
        ...
        print("  uninstall               remove amirify from all detected platforms in one shot")
        print("    --purge                 also delete amirify-out/ directory")
        ...
        print("                            (default amirify-out/graph.json)")
        ...
        print("  clone <github-url>      clone a GitHub repo locally and print its path for /amirify")
        ...
        print("    --out <path>            output path (default: amirify-out/merged-graph.json)")
        print("    --branch <branch>       checkout a specific branch (default: repo default)")
        print("    --out <dir>             clone to a custom directory (default: ~/.amirify/repos/<owner>/<repo>)")
        ...
        print("                            (also: AMIRIFY_FORCE=1 env var; use after refactors that delete code)")
        print("    --no-cluster            skip clustering, write raw extraction only")
        print("  cluster-only <path>     rerun clustering on an existing graph.json and regenerate report")
        print("    --no-viz                skip graph.html generation (useful for >5000 node graphs / CI)")
        print("    --graph <path>          path to graph.json (default <path>/amirify-out/graph.json)")
        ...
        print("  save-result             save a Q&A result to amirify-out/memory/ for graph feedback loop")
        ...
        print("    --memory-dir DIR        memory directory (default: amirify-out/memory)")
        print("  reflect                 aggregate amirify-out/memory/ outcomes into a deterministic lessons doc")
        print("    --memory-dir DIR        memory directory (default: amirify-out/memory)")
        print("    --out FILE              output path (default: amirify-out/reflections/LESSONS.md)")
        print("    --graph PATH            graph.json, for community grouping + dropping stale nodes (optional)")
        print("    --analysis PATH         .amirify_analysis.json (optional, auto-detected next to --graph)")
        print("    --labels PATH           .amirify_labels.json (optional, auto-detected next to --graph)")
        ...
        print("                            manifest gate and semantic cache reads (env: AMIRIFY_FORCE=1)")
        ...
        print("    --out DIR, --output DIR output dir (default: <path>); writes <DIR>/amirify-out/")
        print("    --google-workspace      export .gdoc/.gsheet/.gslides shortcuts via gws before extraction")
        print("    --no-gitignore         ignore .gitignore and .git/info/exclude (prioritizes .amirifyignore)")
        print("    --no-cluster            skip clustering, write raw extraction only")
        ...
        print("  global add <graph.json>  add/update a project graph in the global graph (~/.amirify/global-graph.json)")
        ...
        print("  cursor install          write .cursor/rules/amirify.mdc (Cursor)")
        print("  cursor uninstall        remove .cursor/rules/amirify.mdc")
        print("  claude install          write amirify section to CLAUDE.md + PreToolUse hook (Claude Code)")
        print("  claude uninstall        remove amirify section from CLAUDE.md + PreToolUse hook")
        print("  codebuddy install       write amirify section to CODEBUDDY.md + PreToolUse hook (CodeBuddy)")
        print("  codebuddy uninstall     remove amirify section from CODEBUDDY.md + PreToolUse hook")
        print("  codex install           write amirify section to AGENTS.md (Codex)")
        print("  codex uninstall         remove amirify section from AGENTS.md")
        print(
            "  opencode install        write amirify section to AGENTS.md + tool.execute.before plugin (OpenCode)"
        )
        print(
            "  opencode uninstall      remove amirify section from AGENTS.md + plugin"
        )
        print(
        ...
        print("  aider install           write amirify section to AGENTS.md (Aider)")
        print("  aider uninstall         remove amirify section from AGENTS.md")
        print(
            "  copilot install         copy amirify skill to ~/.copilot/skills (GitHub Copilot CLI)"
        )
        print("  copilot uninstall       remove amirify skill from ~/.copilot/skills")
        print(
        ...
        print(
            "  claw install            write amirify section to AGENTS.md (OpenClaw)"
        )
        print("  claw uninstall          remove amirify section from AGENTS.md")
        print(
            "  droid install           write amirify section to AGENTS.md (Factory Droid)"
        )
        print("  droid uninstall        remove amirify section from AGENTS.md")
        print("  trae install            write amirify section to AGENTS.md (Trae)")
        print("  trae uninstall         remove amirify section from AGENTS.md")
        print("  trae-cn install         write amirify section to AGENTS.md (Trae CN)")
        print("  trae-cn uninstall      remove amirify section from AGENTS.md")
        print(
        ...
        print(
            "  hermes install          write skill to ~/.hermes/skills/amirify/ (Hermes)"
        )
        print("  hermes uninstall        remove skill from ~/.hermes/skills/amirify/")
        print(
            "  kiro install            write skill to .kiro/skills/amirify/ + steering file (Kiro IDE/CLI)"
        )
        print("  kiro uninstall          remove skill + steering file")
        print("  pi install              write skill to ~/.pi/agent/skills/amirify/ (Pi coding agent)")
        print("  pi uninstall            remove skill from ~/.pi/agent/skills/amirify/")
        print("  devin install           write skill to ~/.config/devin/skills/amirify/ (Devin CLI)")
        print("  devin uninstall         remove skill from ~/.config/devin/skills/amirify/")
        print()
        return
    ...
    if cmd not in _FREE_TEXT_CMDS and any(a in {"-h", "--help", "-?"} for a in sys.argv[2:]):
        print(f"Run 'amirify --help' for full usage.")
        return
