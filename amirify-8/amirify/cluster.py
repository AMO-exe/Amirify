...
def _suppress_output():
    ...
    the call prevents this without losing any amirify output.
    """
...
def _native_leiden(stable: nx.Graph, resolution: float) -> dict[str, int] | None:
    ...
    in AMIRIFY_BUILD_PERF.md for the measurements this is based on.
    ...
    # _IdentityMapper guards against). Amirify's own node IDs are already
...
def community_member_sigs(communities: dict[int, list[str]]) -> dict[int, str]:
    ...
    Persisted next to ``.amirify_labels.json`` so a later ``cluster-only`` can tell
