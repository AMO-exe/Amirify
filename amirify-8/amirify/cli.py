"""amirify command dispatch — every non-install subcommand.
...
module. The path-redirect (`amirify <path>` -> extract) re-enters via a lazy
...
import sys
import time
from amirify.paths import AMIRIFY_OUT as _AMIRIFY_OUT
...
_SEARCH_NUDGE = json.dumps({
    "hookSpecificOutput": {
        ...
        "additionalContext": (
            'MANDATORY: amirify-out/graph.json exists. You MUST run '
            '`amirify query "<question>"` before grepping raw files. Only grep '
            'after amirify has oriented you, or to modify/debug specific lines.'
        ),
    }
...
_READ_NUDGE = json.dumps({
    "hookSpecificOutput": {
        "hookEventName": "PreToolUse",
        "additionalContext": (
            'MANDATORY: amirify-out/graph.json exists. You MUST run amirify '
            'before reading source files. Use: `amirify query "<question>"` '
            '(scoped subgraph), `amirify explain "<concept>"`, or '
            '`amirify path "<A>" "<B>"`. Only read raw files after amirify has '
...
_READ_NUDGE_STALE = json.dumps({
    "hookSpecificOutput": {
        "hookEventName": "PreToolUse",
        "additionalContext": (
            'amirify-out/graph.json exists but may be STALE for this file (the file '
            'changed after the last build). Prefer `amirify query "<question>"` for '
            'orientation, and run `amirify update` to refresh the graph. Reading the '
...
_READ_DENY = json.dumps({
    "hookSpecificOutput": {
        ...
        "permissionDecisionReason": (
            'amirify strict mode: this project has a fresh knowledge graph that covers '
            'this file. Run `amirify query "<your question>"` (or `amirify explain` / '
            '`amirify path`) FIRST to orient yourself, then re-issue this Read — it '
...
_GEMINI_NUDGE_TEXT = (
    'amirify: knowledge graph at amirify-out/. For focused questions, run '
    '`amirify query "<question>"` (scoped subgraph, usually much smaller than '
...
def _default_graph_path() -> str:
    return str(Path(_AMIRIFY_OUT) / "graph.json")
...
def _stamped_manifest_files(
    ...
    Mirrors the #1890 path normalization in amirify.llm.
...
def _handle_unverified_semantic_shrink(
    ...
    *,
    ...
    print(
        f"[amirify extract] semantic extraction is incomplete: unverified semantic "
...
def _stale_graph_sources(
    ...
    (.amirifyignore/.gitignore/--exclude changed) without being listed in the
    ...
    try:
        ...
        root_res = scan_root
    # <out>/amirify-out/graph.json — relative source_files may be anchored here.
    ...
    def _provably_excluded(c: Path) -> bool:
        ...
        for s in spellings:
            ...
            if any(s.startswith(pref) for pref in excluded_prefixes):
                return True
        try:
            from amirify.detect import _is_sensitive as _det_sensitive
            if _det_sensitive(c):
                return True
        except Exception:
            pass
    ...
    if kept_alive:
        print(
            f"[amirify] fail-closed: kept node(s) from {len(kept_alive)} "
...
def _zero_node_stamped_code_sources(
    ...
    documented recovery was deleting amirify-out/. A stamped file that the
    ...
    try:
        ...
        root_res = scan_root
    # <out>/amirify-out/graph.json — legacy relative source_files may be
    ...
    from amirify.extract import _get_extractor
    healed: list[str] = []
...
def _prune_graph_json_sources(graph_path: Path, stale_sources: list[str]) -> int:
    ...
    from amirify.export import backup_if_protected as _backup
    _backup(graph_path.parent)
    from amirify.paths import write_json_atomic
...
class _StageTimer:
    ...
    Monotonic (perf_counter), diagnostic-only: emits ``[amirify timing] <stage>:
    ...
    def mark(self, stage: str) -> None:
        now = self._now()
        if self.enabled:
            print(f"[amirify timing] {stage}: {now - self._last:.1f}s", file=sys.stderr)
        self._last = now

    def total(self) -> None:
        if self.enabled:
            print(f"[amirify timing] total: {self._now() - self.start:.1f}s", file=sys.stderr)
def _enforce_graph_size_cap_or_exit(gp: Path) -> None:
    """Reject oversized graph files before parsing (CLI exit-on-fail flavor).

    Delegates to ``amirify.security.check_graph_file_size_cap`` and turns the
    ...
    """
    from amirify.security import check_graph_file_size_cap
    try:
...
def _touch_query_stamp(graph_path: "Path") -> None:
    """Record that amirify oriented the agent recently, next to the queried graph.
    The strict guard suppresses its block while this stamp is fresh. Fail-silent."""
    try:
        from amirify.paths import write_text_atomic
...
def _query_stamp_fresh() -> bool:
    """True if a query/explain/path ran within AMIRIFY_HOOK_STRICT_TTL (default
    1800s) — recent orientation, so strict mode does not block this read."""
    from amirify.paths import out_path
    try:
        ttl = float(os.environ.get("AMIRIFY_HOOK_STRICT_TTL", "1800"))
...
def _bash_invokes_search(cmd_str: str) -> bool:
    ...
    grep. Every false positive injects a nudge where amirify has nothing to
...
def _run_hook_guard(kind: str, strict: bool = False) -> None:
    ...
    exists, nudges the agent to use amirify instead of grepping/reading raw files.
    ...
    in-project, fresh code per session is DENIED with a redirect to `amirify query`
    ...
    """
    from amirify.paths import out_path, AMIRIFY_OUT_NAME
    ...
    try:
        if kind == "search":
            ...
            under_out = "amirify-out/" in j or (AMIRIFY_OUT_NAME.lower() + "/") in j
...
def _clone_repo(
    ...
    Clones into ~/.amirify/repos/<owner>/<repo> by default so repeated
    ...
    if out_dir:
        ...
        dest = Path.home() / ".amirify" / "repos" / owner / repo
...
def _reenter_main() -> None:
    from amirify.__main__ import main
    main()


def dispatch_command(cmd: str) -> None:
    if cmd == "provider":
        from amirify.llm import _custom_providers_path, BACKENDS
        ...
        if subcmd == "list":
            ...
            if not name:
                print("Usage: amirify provider show <name>", file=sys.stderr)
                sys.exit(1)
            existing = {}
            if global_path.is_file():
                try:
            ...
            if not name:
                print("Usage: amirify provider add <name> --base-url URL --default-model MODEL --env-key KEY", file=sys.stderr)
                sys.exit(1)
            if name in BACKENDS:
            ...
            if not base_url or not default_model or not env_key:
                ...
                sys.exit(1)
            from amirify.llm import provider_base_url_ok
            ...
            print(f"Provider '{name}' added. Use with: amirify extract . --backend {name}")

        elif subcmd == "remove":
            name = sys.argv[3] if len(sys.argv) > 3 else ""
            if not name:
                print("Usage: amirify provider remove <name>", file=sys.stderr)
                sys.exit(1)
            existing = {}
            if global_path.is_file():
                try:
            ...
            print("Usage: amirify provider [add|list|show|remove]", file=sys.stderr)
            if subcmd:
                sys.exit(1)
    elif cmd == "prs":
        from amirify.prs import cmd_prs
        cmd_prs(sys.argv[2:])
    elif cmd == "hook":
        from amirify.hooks import (
        ...
        if subcmd == "install":
            ...
            print("Usage: amirify hook [install|uninstall|status]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "query":
        if len(sys.argv) < 3:
            print("Usage: amirify query \"<question>\" [--dfs] [--context C] [--budget N] [--graph path]", file=sys.stderr)
            sys.exit(1)
        from amirify.serve import _query_graph_text
        from amirify.security import sanitize_label
        from networkx.readwrite import json_graph
        from amirify import querylog
        ...
        try:
            ...
            # per-edge below (mirrors amirify/build.py's _src/_tgt pattern)
            ...
            try:
                from amirify.build import graph_has_legacy_ids as _legacy
                if _legacy(_raw.get("nodes", [])):
                    print(
                        "[amirify] note: this graph uses the pre-#1504 node-ID scheme; "
                        "rebuild with `amirify extract --force` to get path-qualified IDs "
        ...
        if len(sys.argv) < 3:
            print("Usage: amirify affected \"<node-or-label>\" [--relation R] [--depth N] [--graph path]", file=sys.stderr)
            sys.exit(1)
        from amirify.affected import DEFAULT_AFFECTED_RELATIONS, format_affected, load_graph
        query = sys.argv[2]
        graph_path = _default_graph_path()
        depth = 2
        relations: list[str] = []
        ...
        # (#2706). The graph is written to <root>/<AMIRIFY_OUT_NAME>/graph.json,
        # so the root is the output dir's parent; a graph pointed at directly by
        # --graph falls back to its own directory.
        from amirify.paths import AMIRIFY_OUT_NAME
        graph_root = gp.parent.parent if gp.parent.name == AMIRIFY_OUT_NAME else gp.parent
        print(
            format_affected(
                graph,
                query,
        ...
        # README-advertised capability, but never a CLI subcommand — `amirify
        # god_nodes` fell through to "unknown command" (#2004). Wire it as a
        # read-only graph query, mirroring `affected`.
        from amirify.affected import load_graph
        from amirify.analyze import god_nodes as _god_nodes
        from amirify.security import sanitize_label as _sanitize_label
        graph_path = _default_graph_path()
        top_n = 10
        ...
        # amirify save-result --question Q --answer A [--type T] [--nodes N1 N2 ...]
        #                      [--outcome useful|dead_end|corrected] [--correction TEXT]
        import argparse as _ap

        p = _ap.ArgumentParser(prog="amirify save-result")
        ...
        p.add_argument("--memory-dir", default=str(Path(_AMIRIFY_OUT) / "memory"))
        ...
        from amirify.ingest import save_query_result as _sqr
        ...
        p = _ap.ArgumentParser(prog="amirify reflect")
        p.add_argument("--memory-dir", default=str(Path(_AMIRIFY_OUT) / "memory"))
        p.add_argument(
            "--out",
            default=str(Path(_AMIRIFY_OUT) / "reflections" / "LESSONS.md"),
        )
        ...
        opts = p.parse_args(sys.argv[2:])
        from amirify.reflect import reflect as _reflect, lessons_fresh as _lessons_fresh

        graph_arg = opts.graph
        if graph_arg is None:
            default_graph = Path(_AMIRIFY_OUT) / "graph.json"
        ...
        if _gp is not None:
            _analysis_path = Path(opts.analysis) if opts.analysis else (
                _gp.parent / ".amirify_analysis.json")
            _labels_path = Path(opts.labels) if opts.labels else (
                _gp.parent / ".amirify_labels.json")
        ...
        if len(sys.argv) < 4:
            print(
                'Usage: amirify path "<source>" "<target>" [--graph path] '
                "[--directed|--undirected]",
                file=sys.stderr,
            )
            sys.exit(1)
        from amirify.serve import _pick_scored_endpoint, _score_nodes
        from networkx.readwrite import json_graph
        import networkx as _nx
        ...
        segments = []
        from amirify.build import edge_datas
        ...
        if len(sys.argv) < 3:
            print('Usage: amirify explain "<node>" [--graph path]', file=sys.stderr)
            sys.exit(1)
        from amirify.serve import _find_node, find_node_ambiguity
        from networkx.readwrite import json_graph
        ...
        # Work-memory overlay: a derived experiential hint from `amirify reflect`,
        # merged in display-only from the .amirify_learning.json sidecar next to
        # graph.json. No line when the node has no overlay entry.
        try:
            from amirify.reflect import load_learning_overlay as _llo
            from amirify.security import sanitize_label as _sl
            _overlay = _llo(gp)
        ...
        from amirify.build import edge_data
        ...
        if subcmd != "multigraph":
            print(
                "Usage: amirify diagnose multigraph "
        ...
        from amirify.diagnostics import (
            diagnose_file,
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: amirify add <url> [--author Name] [--contributor Name] [--dir ./raw]",
                file=sys.stderr,
            )
            sys.exit(1)
        from amirify.ingest import ingest as _ingest
        ...
        try:
            ...
            print("Run /amirify --update in your AI assistant to update the graph.")
        except Exception as exc:
            print(f"error: {exc}", file=sys.stderr)
            sys.exit(1)
        ...
        if not watch_path.exists():
            ...
            sys.exit(1)
        from amirify.watch import watch as _watch
        ...
        # the configured backend, even when a .amirify_labels.json already exists.
        ...
        if watch_path is None:
            watch_path = Path(".")
        graph_json = graph_override if graph_override is not None else watch_path / _AMIRIFY_OUT / "graph.json"
        if not graph_json.exists():
            print(
                f"error: no graph found at {graph_json} — run /amirify first",
                file=sys.stderr,
            )
            sys.exit(1)
        from networkx.readwrite import json_graph as _jg
        from amirify.build import build_from_json
        from amirify.cluster import cluster, score_all, remap_communities_to_previous
        from amirify.analyze import (
            god_nodes,
            surprising_connections,
            suggest_questions,
        )
        from amirify.report import generate
        from amirify.export import to_json, to_html
        ...
        from amirify.security import check_graph_file_size_cap as _check_cap
        try:
        ...
        # node-overlap so the existing .amirify_labels.json keeps attaching
        ...
        # amirify-out/ dir (another project/tenant's output), write beside it,
        # not into a stray amirify-out/ in the CWD (#1747). But when `--graph`
        # points at an arbitrary path — e.g. a `backup/graph.json` archived
        # before re-clustering (#934) — fall back to the CWD's amirify-out/,
        # which is the restore-into-place workflow that test pins. The default
        # (no --graph) case already has graph_json under watch_path/amirify-out.
        _out_name = Path(_AMIRIFY_OUT).name
        if graph_override is not None and graph_json.parent.name == _out_name:
            out = graph_json.parent
        else:
            out = watch_path / _AMIRIFY_OUT
        out.mkdir(parents=True, exist_ok=True)
        labels_path = out / ".amirify_labels.json"
        ...
        if labels_path.exists() and not force_relabel:
            ...
            if _ignored_label_flags:
                print(
                    f"[amirify] warning: {'/'.join(_ignored_label_flags)} ignored: "
                    f"reusing saved labels at {labels_path}. "
                    f"Run `amirify label` to relabel, or delete the labels file.",
                    file=sys.stderr,
                )
            ...
            # is told to `amirify label` for fresh LLM names. Unchanged communities keep
            ...
            from amirify.cluster import community_member_sigs, label_communities_by_hub
            ...
            # No labels file yet (or `amirify label` forced a refresh). When run
            ...
            from amirify.cluster import label_communities_by_hub
            from amirify.llm import generate_community_labels
        ...
        if not _commit:
            from amirify.watch import _git_head as _gh
        ...
        from amirify.export import backup_if_protected as _backup
        from amirify.exporters.html import _HTML_STALE_MARKER
        _backup(out)
        ...
        if not to_json(G, communities, str(out / "graph.json"),
            ...
            print(
                ...
                "GRAPH_REPORT.md, .amirify_labels.json and .amirify_analysis.json "
                "left untouched.",
                file=sys.stderr,
            )
            sys.exit(1)
        tokens = label_token_usage
        from amirify.report import load_learning_for_report as _llfr
        ...
        (out / ".amirify_analysis.json").write_text(
        ...
        if not placeholder_only:
            from amirify.paths import write_json_atomic as _wja
            ...
            # (see reuse above).
            from amirify.cluster import community_member_sigs as _cms
        ...
        if no_viz:
            ...
            try:
                from amirify.exporters.html import _viz_node_limit
                viz_limit = _viz_node_limit()
                if viz_limit <= 0:
                    html_target.unlink(missing_ok=True)
                    _clear_html_stale_marker()
                    skip_reason = "AMIRIFY_VIZ_NODE_LIMIT=0 disables HTML visualization"
                else:
        ...
        force = os.environ.get("AMIRIFY_FORCE", "").lower() in ("1", "true", "yes")
        no_cluster = False
        args = sys.argv[2:]
        ...
        if watch_arg is not None:
            ...
            saved = Path(_AMIRIFY_OUT) / ".amirify_root"
            if saved.exists():
                ...
                # (#3028). Match the other .amirify_root readers.
        ...
        if not watch_path.exists():
            print(f"error: path not found: {watch_path}", file=sys.stderr)
            sys.exit(1)
        from amirify.watch import _rebuild_code

        print(f"Re-extracting code files in {watch_path} (no LLM needed)...")
        # Interactive CLI: block on the per-repo lock rather than skip, so the
        # user sees their explicit `amirify update` complete instead of
        ...
        if ok:
            print("Code graph updated. For doc/paper/image changes run /amirify --update in your AI assistant.")
            if not (
                os.environ.get("GEMINI_API_KEY")
                or os.environ.get("GOOGLE_API_KEY")
                or os.environ.get("MOONSHOT_API_KEY")
                or os.environ.get("DEEPSEEK_API_KEY")
                or os.environ.get("AMIRIFY_NO_TIPS")
            ):
        ...
        # toward amirify when a fresh in-project graph exists; always exits 0. In
        ...
        if len(sys.argv) < 3:
            print("Usage: amirify check-update <path>", file=sys.stderr)
            sys.exit(1)
        from amirify.watch import check_update
        ...
        from amirify.tree_html import write_tree_html, DEFAULT_MAX_CHILDREN
        graph_path = Path(_AMIRIFY_OUT) / "graph.json"
        ...
        while i_arg < len(args):
            ...
            if a == "--graph" and i_arg + 1 < len(args):
                ...
                print("Usage: amirify tree [--graph PATH] [--output HTML]")
                print("  --graph PATH         path to graph.json (default amirify-out/graph.json)")
                print("  --output HTML        output path (default amirify-out/GRAPH_TREE.html)")
        ...
        # Usage: amirify merge-driver %O %A %B  (set in .git/config merge driver)
        if len(sys.argv) < 5:
            print("Usage: amirify merge-driver <base> <current> <other>", file=sys.stderr)
            sys.exit(1)
        ...
        def _load_graph(p: str):
            ...
            from amirify.paths import load_node_link_graph as _lnlg
            return _lnlg(data), data
        try:
            G_cur, _ = _load_graph(_current_path)
            G_oth, _ = _load_graph(_other_path)
        except Exception as exc:
            print(f"[amirify merge-driver] error loading graphs: {exc}", file=sys.stderr)
        ...
        if merged.number_of_nodes() > _MERGE_MAX_NODES:
            print(
                f"[amirify merge-driver] merged graph has {merged.number_of_nodes()} nodes, "
        ...
        # amirify merge-graphs graph1.json graph2.json ... --out merged.json
        args = sys.argv[2:]
        graph_paths: list[Path] = []
        out_path = Path(_AMIRIFY_OUT) / "merged-graph.json"
        i = 0
        while i < len(args):
        ...
        if len(graph_paths) < 2:
            print(
                "Usage: amirify merge-graphs <graph1.json> <graph2.json> [...] [--out merged.json]",
                file=sys.stderr,
            )
            sys.exit(1)
        import networkx as _nx
        from networkx.readwrite import json_graph as _jg
        from amirify.build import prefix_graph_for_global as _prefix, distinct_repo_tags as _repo_tags
        graphs = []
        for gp in graph_paths:
            ...
            # Normalize edges/links key before loading — amirify writes "links"
        ...
        # (the amirify default) by converting MultiGraphs with nx.Graph().
        def _to_simple(g: "_nx.Graph") -> "_nx.Graph":
            ...
            if type(g) is not _nx.Graph:
                return _nx.Graph(g)
            return g
        # Unique repo tag per graph. The bare `amirify-out/..` dir name is not
        # unique across inputs (src/amirify-out and frontend/src/amirify-out both
        ...
        from amirify.cross_repo_types import link_shared_type_declarations as _link_shared
        ...
        from amirify.cross_repo_calls import link_cross_repo_member_calls as _link_calls
        ...
        if collected_hyperedges:
            from amirify.export import attach_hyperedges as _attach
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: amirify clone <github-url> [--branch <branch>] [--out <dir>]",
                file=sys.stderr,
            )
            sys.exit(1)
        url = sys.argv[2]
        ...
        if subcmd not in ("html", "callflow-html", "obsidian", "wiki", "svg", "graphml", "neo4j", "falkordb"):
            print("Usage: amirify export <format>", file=sys.stderr)
        ...
        labels_path = Path(_AMIRIFY_OUT) / ".amirify_labels.json"
        labels_path_explicit = False
        report_path = Path(_AMIRIFY_OUT) / "GRAPH_REPORT.md"
        ...
        analysis_path = Path(_AMIRIFY_OUT) / ".amirify_analysis.json"
        node_limit = 5000
        no_viz = False
        obsidian_dir = Path(_AMIRIFY_OUT) / "obsidian"
        ...
        while i < len(args):
            ...
            if a == "--graph" and i + 1 < len(args):
                ...
                print("Usage: amirify export callflow-html [GRAPH|DIR] [--graph PATH] [--labels PATH]")
                print("  --report PATH          path to GRAPH_REPORT.md")
                print("  --sections PATH        JSON section definitions")
                print("  --output HTML          output path (default amirify-out/<project>-callflow.html)")
                ...
                if candidate.name == "graph.json" or candidate.suffix.lower() == ".json":
                    ...
                    graph_path = candidate / _AMIRIFY_OUT / "graph.json"
                graph_path_explicit = True
                i += 1
            else:
                i += 1
        ...
        if graph_path_explicit:
            ...
            if not labels_path_explicit:
                labels_path = graph_out_dir / ".amirify_labels.json"
        ...
        if not graph_path.exists():
            print(f"error: graph not found: {graph_path}. Run /amirify <path> first.", file=sys.stderr)
            sys.exit(1)

        if subcmd == "callflow-html":
            from amirify.callflow_html import write_callflow_html as _write_callflow_html
        ...
        from networkx.readwrite import json_graph as _jg
        from amirify.build import build_from_json as _bfj
        from amirify.security import check_graph_file_size_cap as _check_cap
        ...
        # that happens, `amirify export html` previously bailed with
        ...
        if not communities:
            ...
            # without ever regenerating .amirify_analysis.json. Cheap,
            ...
            if sidecar_partition != fresh_partition:
                ...
                print(
                    ...
                    "communities from graph.json instead. Run `amirify cluster-only .` "
                    "to refresh the sidecar and its cohesion/god-node data.",
                    file=sys.stderr,
                )
                communities = reconstructed
                from amirify.cluster import score_all as _score_all_export
                from amirify.analyze import god_nodes as _god_nodes_export
        ...
        if subcmd == "html":
            from amirify.export import to_html as _to_html
            if no_viz:
            ...
            from amirify.export import to_obsidian as _to_obsidian, to_canvas as _to_canvas
            ...
            from amirify.wiki import to_wiki as _to_wiki
            from amirify.analyze import god_nodes as _god_nodes
            if not communities:
                print(
                    "error: .amirify_analysis.json is missing or empty — refusing to export wiki to prevent data loss.\n"
                    "Run `amirify extract .` (or `amirify cluster-only .`) to regenerate community data first.",
                    file=sys.stderr,
                )
                sys.exit(1)
            if not gods_data:
            ...
            if push_uri:
                ...
                from amirify.export import to_cypher as _to_cypher
            ...
            if push_uri:
                from amirify.export import push_to_falkordb as _push
                result = _push(G, uri=push_uri, user=push_user,
                               password=push_password, communities=communities)
                ...
                print(f"cypher.txt written ({out_dir}/cypher.txt) - statements are OpenCypher. "
                      ...
                      f"import), so load a graph with: amirify export falkordb --push "
                      f"falkordb://localhost:6379")

    elif cmd == "benchmark":
        from amirify.benchmark import run_benchmark, print_benchmark

        graph_path = sys.argv[2] if len(sys.argv) > 2 else _default_graph_path()
        _enforce_graph_size_cap_or_exit(Path(graph_path))
        # Try to load corpus_words from detect output
        corpus_words = None
        detect_path = Path(".amirify_detect.json")
        ...
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        from amirify.global_graph import (
        ...
        if subcmd == "add":
            # amirify global add <graph.json> [--as <tag>]
            args = sys.argv[3:]
            source = None
            tag = None
            i = 0
            while i < len(args):
                if args[i] == "--as" and i + 1 < len(args):
                    ...
                    i += 1
            if not source:
                print("Usage: amirify global add <graph.json> [--as <repo-tag>]", file=sys.stderr)
                sys.exit(1)
            if not tag:
                ...
                from amirify.build import distinct_repo_tags
            ...
            if len(sys.argv) <= 3:
                print("Usage: amirify global remove <repo-tag>", file=sys.stderr); sys.exit(1)
            tag = sys.argv[3]
            try:
            ...
            repos = _global_list()
            if not repos:
                print("Global graph is empty. Use 'amirify global add' to add a project.")
            else:
            ...
            print("Usage: amirify global [add|remove|list|path]", file=sys.stderr); sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: amirify extract <path> [--backend gemini|kimi|claude|openai|deepseek|ollama] "
        ...
        # --force parity with `amirify update`: the flag or AMIRIFY_FORCE=1
        ...
        while i < len(args):
            ...
            if a == "--backend" and i + 1 < len(args):
                ...
                # before, and `amirify tree` already documents --output, so the
        ...
        if deep_mode:
            print("[amirify extract] deep mode enabled: richer semantic extraction")

        # CLI flag wins over env var. Setting AMIRIFY_API_TIMEOUT here so
        # _call_openai_compat picks it up without needing a new kwarg path.
        if cli_api_timeout is not None:
            os.environ["AMIRIFY_API_TIMEOUT"] = str(cli_api_timeout)
        if cli_max_workers is not None:
            os.environ["AMIRIFY_MAX_WORKERS"] = str(cli_max_workers)

        # Resolve output dir. The user-facing contract is "<out>/amirify-out/"
        # so a fresh checkout writes amirify-out/ at the project root, matching
        # the skill.md pipeline.
        out_root = (out_dir.resolve() if out_dir else target)
        amirify_out = out_root / _AMIRIFY_OUT
        amirify_out.mkdir(parents=True, exist_ok=True)
        # Persist corpus-shaping options so later update/watch/hook rebuilds
        # use the same file set as the initial extraction (#1886).
        from amirify.watch import (
        ...
        # flag-less `amirify extract` must NOT clobber it back to True, which
        ...
        _effective_gitignore = False if no_gitignore else _read_build_gi(amirify_out)
        # An explicit list replaces the persisted one; omission reuses it.
        _effective_excludes = cli_excludes or _read_build_ex(amirify_out)
        _write_build_cfg(
            amirify_out,
        ...
        from amirify.detect import (
            detect as _detect,
            detect_incremental as _detect_incremental,
            save_manifest as _save_manifest,
        )
        manifest_path = amirify_out / "manifest.json"
        existing_graph_path = amirify_out / "graph.json"
        ...
        if force and code_only and existing_graph_path.exists():
            print(
                "[amirify extract] --force --code-only: full AST re-scan, "
                "existing semantic layer preserved (no semantic pass this run)"
            )
        if force:
            print("[amirify extract] --force: full re-scan, semantic cache reads skipped")
        elif incremental_mode and not manifest_path.exists():
            print(
                "[amirify extract] manifest.json missing; using existing "
        ...
        if not has_path:
            ...
            print(f"[amirify extract] incremental scan of {target}")
            ...
            if _healed_sources:
                print(
                    f"[amirify extract] re-queuing {len(_healed_sources)} "
            ...
            if _healed_sem_sources:
                print(
                    f"[amirify extract] re-queuing {len(_healed_sem_sources)} "
            ...
            print(f"[amirify extract] scanning {target}")
            detection = _detect(
                target,
                google_workspace=google_workspace or None,
                extra_excludes=_effective_excludes or None,
        ...
        if code_only and semantic_files:
            print(
                f"[amirify extract] --code-only: skipping {len(semantic_files)} "
        ...
        if deep_mode and incremental_mode and not code_only:
            ...
            if len(_deep_all) != len(semantic_files):
                print(
                    f"[amirify extract] deep mode: widening semantic pass from "
        ...
        if incremental_mode:
            ...
            print(
                f"[amirify extract] {len(code_files)} code, {len(doc_files)} docs, "
            ...
            print(
                f"[amirify extract] found {len(code_files)} code, "
        ...
        # are no longer invisible in amirify's own output (#1692).
        ...
        if _unclassified:
            ...
            print(
                f"[amirify extract] {len(_unclassified)} file(s) not classified "
        ...
        if _sec:
            ...
            print(
                f"[amirify extract] {len(_sec)} file(s) skipped as potentially sensitive "
        ...
        if needs_llm:
            if backend is None:
                ...
                sys.exit(1)
            if backend == "ollama":
                from amirify.llm import _validate_ollama_base_url
        ...
        if incremental_mode:
            from amirify.extractors.terraform import refresh_terraform_paths
        ...
        if code_files:
            from amirify.extract import extract as _ast_extract
            # Anchor the cache at the output root, not the scanned project:
            # with --out, a <target>/amirify-out/cache/ would leak a
            # amirify-out/ dir into a project that asked for external output.
            ...
            # #2437/#2438 (the `amirify update` twin of watch's #2406 fix): an
            ...
            if incremental_mode and existing_graph_path.exists():
                ...
                try:
                    from amirify.build import _is_ast_tier as _ctx_is_ast_tier
                    from amirify.security import (
            ...
            print(f"[amirify extract] AST extraction on {len(code_files)} code files...")
            try:
                ast_result = _ast_extract(code_files, **ast_kwargs)
            except Exception as exc:
                print(f"[amirify extract] AST extraction failed: {exc}", file=sys.stderr)
        ...
        from amirify.llm import _extraction_system as _sem_prompt_for
        ...
        if semantic_files:
            ...
            if sem_cache_hits:
                print(f"[amirify extract] semantic cache: {sem_cache_hits} hit / {sem_cache_misses} miss")

            if uncached_paths:
                print(f"[amirify extract] semantic extraction on {len(uncached_paths)} files via {backend}...")
                corpus_kwargs: dict = {
                    "backend": backend,
                    "model": model,
                    "root": target,
                ...
                def _progress(idx: int, total: int, _result: dict) -> None:
                    ...
                    print(
                        f"[amirify extract] chunk {idx + 1}/{total} done",
                        flush=True,
                    )
                ...
                try:
                    ...
                    print(
                        f"[amirify extract] semantic extraction failed: {exc}",
                        file=sys.stderr,
                    )
                ...
                if uncached_paths and _chunk_stats["succeeded"] == 0:
                    print(
                        f"[amirify extract] error: all semantic chunks failed "
                ...
                if _dropped_files:
                    print(
                        f"[amirify extract] dropped {_dropped_items} out-of-scope "
                ...
                if _omitted_files or _partial_semantic_files:
                    _extraction_incomplete = True
                    print(
                        f"[amirify extract] semantic extraction is incomplete: "
                ...
                try:
                    ...
                    print(f"[amirify extract] warning: could not write semantic cache: {exc}", file=sys.stderr)
        ...
        try:
            from amirify.cache import file_hash as _file_hash
            ...
            print(f"[amirify extract] warning: could not prune semantic cache: {exc}", file=sys.stderr)
        stages.mark("semantic extract")

        pg_result: dict = {"nodes": [], "edges": []}
        if cli_postgres_dsn is not None:
            from amirify.pg_introspect import introspect_postgres
            print(f"[amirify extract] introspecting PostgreSQL schema...")
            try:
                pg_result = introspect_postgres(cli_postgres_dsn)
            except (ConnectionError, ImportError) as exc:
                print(f"error: {exc}", file=sys.stderr)
                sys.exit(1)
            print(f"[amirify extract] PostgreSQL: {len(pg_result['nodes'])} nodes, "
                  f"{len(pg_result['edges'])} edges")

        cargo_result: dict = {"nodes": [], "edges": []}
        if cli_cargo:
            from amirify.cargo_introspect import introspect_cargo
            print("[amirify extract] introspecting Cargo workspace...")
            try:
                ...
                print("[amirify extract] --cargo: no Cargo.toml at scan root, "
                      "skipping crate edges")
            except (ConnectionError, ImportError, OSError) as exc:
                print(f"error: {exc}", file=sys.stderr)
                sys.exit(1)
            print(f"[amirify extract] Cargo: {len(cargo_result['nodes'])} nodes, "
        ...
        graph_json_path = amirify_out / "graph.json"
        analysis_path = amirify_out / ".amirify_analysis.json"
        ...
        if no_cluster:
            ...
            from amirify.build import dedupe_edges as _dedupe_edges, dedupe_nodes as _dedupe_nodes
            from amirify.export import (
            ...
            if (
                ...
                if graph_stale_sources:
                    ...
                    if _n_pruned:
                        print(
                            f"[amirify extract] pruned {_n_pruned} node(s) from "
                            f"{len(graph_stale_sources)} source file(s) no longer "
                            "in the scan (deleted or excluded)."
                        )
                print(
                    "[amirify extract] no incremental changes detected "
                ...
                try:
                    ...
                    print(f"[amirify extract] warning: could not write manifest: {exc}", file=sys.stderr)
                stages.total()
                sys.exit(0)
            ...
            if merge_existing_graph:
                ...
                from amirify.build import merge_raw_extraction as _merge_raw_extraction
            ...
            from amirify.build import disambiguate_file_labels_in_nodes as _disamb_labels
            ...
            if _extraction_incomplete and not cli_allow_partial:
                from amirify.export import MALFORMED_GRAPH as _MALFORMED_GRAPH
                ...
                if _malformed or _shrinks:
                    ...
                    print(
                        "[amirify extract] error: extraction was incomplete (an AST/"
            ...
            from amirify.paths import write_json_atomic as _write_json_atomic
            ...
            try:
                ...
                (amirify_out / ".amirify_root").write_text(
            ...
            try:
                ...
                print(f"[amirify extract] warning: could not write manifest: {exc}", file=sys.stderr)
            if global_merge:
                from amirify.global_graph import global_add as _global_add
                _tag = global_repo_tag or target.name
                try:
                    result = _global_add(amirify_out / "graph.json", _tag)
                    if result["skipped"]:
                        print(f"[amirify global] '{_tag}' unchanged since last add - skipped.")
                    else:
                        print(f"[amirify global] '{_tag}' merged into global graph "
                              f"(+{result['nodes_added']} nodes, -{result['nodes_removed']} pruned).")
                except Exception as exc:
                    print(f"[amirify global] warning: failed to merge into global graph: {exc}", file=sys.stderr)
            stages.total()
            sys.exit(0)

        # Build graph + cluster + score + write.
        from amirify.build import (
            build as _build,
            build_from_json as _build_from_json,
            build_merge as _build_merge,
        )
        from amirify.cluster import cluster as _cluster, score_all as _score_all
        from amirify.export import to_json as _to_json
        from amirify.analyze import god_nodes as _god_nodes, surprising_connections as _surprising
        ...
        if merge_existing_graph:
            ...
            try:
                ...
                print(f"[amirify extract] {exc}", file=sys.stderr)
                sys.exit(1)
        else:
        ...
        if G.number_of_nodes() == 0:
            print(
                "[amirify extract] graph is empty — extraction produced no nodes. "
        ...
        # command was invoked from, so `amirify extract <target>` run from
        ...
        # extract path.
        from amirify.watch import _git_head as _gh_target
        ...
        if not _wrote:
            ...
            # re-attempts.
            print(
                "[amirify extract] error: extraction was incomplete (an AST/semantic "
        ...
        if merged.get("output_tokens", 0) > 0:
            (amirify_out / ".amirify_semantic_marker").write_text(
        ...
        print(f"[amirify extract] wrote {analysis_path}")
        if incremental_mode:
            _excl_note = f", {len(excluded_files)} excluded" if excluded_files else ""
            print(
                f"[amirify extract] incremental summary: "
            ...
            print(f"[amirify extract] semantic cache: {sem_cache_hits} cached, {sem_cache_misses} re-extracted")
        if merged["input_tokens"] or merged["output_tokens"]:
            print(
                f"[amirify extract] tokens: "
                f"{merged['input_tokens']:,} in / "
        ...
        print(
            "[amirify extract] next: run "
            f"`amirify cluster-only {amirify_out.parent}` "
            "to generate GRAPH_REPORT.md and name communities"
        )
        stages.total()

    elif cmd == "cache-check":
        # amirify cache-check <files_from> [--root <dir>] [--mode <m> | --deep]
        #                       [--prompt-file <path>]
        ...
        # Writes:
        #   amirify-out/.amirify_cached.json   — already-cached nodes/edges/hyperedges
        #   amirify-out/.amirify_uncached.txt  — paths that need extraction
        # Stdout: "Cache: N hit, M miss"
        from amirify.cache import check_semantic_cache
        if len(sys.argv) < 3:
            print("Usage: amirify cache-check <files_from> [--root <dir>] "
        ...
        out = root / _AMIRIFY_OUT
        out.mkdir(parents=True, exist_ok=True)
        if cached_nodes or cached_edges or cached_hyperedges:
            (out / ".amirify_cached.json").write_text(
                json.dumps({"nodes": cached_nodes, "edges": cached_edges, "hyperedges": cached_hyperedges},
                           ensure_ascii=False),
                encoding="utf-8",
            )
        (out / ".amirify_uncached.txt").write_text("\n".join(uncached), encoding="utf-8")
        print(f"Cache: {len(files) - len(uncached)} hit, {len(uncached)} miss")

    elif cmd == "merge-chunks":
        # amirify merge-chunks <chunk_glob_or_files...> --out <path>
        # Concatenates .amirify_chunk_*.json files written by semantic subagents.
        # Deduplicates nodes by id (first writer wins). Sums token counts.
        import glob as _glob
        if len(sys.argv) < 3:
            print("Usage: amirify merge-chunks <chunk_files...> --out <path>", file=sys.stderr)
            sys.exit(1)
        ...
        from amirify.semantic_cleanup import load_validated_semantic_fragment
        for cf in chunk_files:
            chunk, _chunk_errs = load_validated_semantic_fragment(Path(cf))
            if _chunk_errs:
                print(
                    f"[amirify merge-chunks] warning: skipping invalid chunk {cf}: "
        ...
        if not valid_chunks:
            print(
                f"[amirify merge-chunks] error: no valid chunks to merge; "
        ...
        # amirify merge-semantic --cached <path> --new <path> --out <path>
        ...
        if len(sys.argv) < 3:
            print("Usage: amirify merge-semantic --cached <path> --new <path> --out <path>", file=sys.stderr)
            sys.exit(1)
        ...
        # User ran `amirify <path>` directly — treat as `amirify extract <path>`.
        # Common when following the PowerShell note in README (`amirify .`) or
        ...
        print("Run 'amirify --help' for usage.", file=sys.stderr)
        sys.exit(1)
