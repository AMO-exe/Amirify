...
from amirify.analyze import _node_community_map
...
from amirify.exporters.graphdb import push_to_falkordb, push_to_neo4j  # noqa: E402,F401
...
_BACKUP_ARTIFACTS = [
    ...
    "GRAPH_REPORT.md",
    ".amirify_labels.json",
    ".amirify_analysis.json",
    "manifest.json",
    ".amirify_semantic_marker",
    "cost.json",
]
...
def backup_if_protected(out_dir: Path) -> "Path | None":
    ...
    - .amirify_semantic_marker is present (graph cost real LLM tokens), or
    - .amirify_labels.json contains at least one non-default community label
    ...
    Set AMIRIFY_NO_BACKUP=1 to disable.
    """
    if os.environ.get("AMIRIFY_NO_BACKUP"):
        return None
    out = Path(out_dir)
    if not (out / "graph.json").exists():
        return None

    is_semantic = (out / ".amirify_semantic_marker").exists()
    is_curated = False
        ...
        for name in _BACKUP_ARTIFACTS:
            ...
            if src.exists():
                try:
                    ...
                    pass
        if copied:
            print(f"[amirify] backed up {reason} graph ({copied} files) -> {backup_dir.name}/")
        return backup_dir
    except Exception as exc:
        import sys
        print(f"[amirify] warning: backup failed ({exc}) - continuing with overwrite", file=sys.stderr)
        return None
...
def _yaml_str(s: str) -> str:
    ...
    See `amirify.ingest._yaml_str` for the full rationale; duplicated here to
...
from amirify.exporters.base import COMMUNITY_COLORS  # noqa: E402,F401

from amirify.exporters.html import to_html  # noqa: E402,F401
...
def to_json(G: nx.Graph, communities: dict[int, list[str]], output_path: str, *, force: bool = False, built_at_commit: str | None = None, community_labels: dict[int, str] | None = None) -> bool:
        ...
        if not oversized:
            ...
            if not raw.strip():
                ...
                try:
                    ...
                    import sys as _sys
                    print(
                        f"[amirify] WARNING: existing {existing_path} could not be "
            ...
            if new_n < existing_n:
                import sys as _sys
                print(
                    f"[amirify] WARNING: new graph has {new_n} nodes but existing "
                    ...
                    f"Run a full rebuild (/amirify .) to be safe, or pass force=True "
    ...
    if "hyperedges" not in getattr(G, "graph", {}):
        ...
        if _prev_hyperedges:
            print(
                f"[amirify] WARNING: graph carries no hyperedge metadata but "
    ...
    # (output_path lives in <target>/amirify-out/), never the shell's cwd —
...
def to_cypher(G: nx.Graph, output_path: str) -> None:
    lines = ["// Neo4j Cypher import - generated by /amirify", ""]
...
# A frontmatter tag entry in amirify's own namespace, e.g. "  - amirify/document".
_AMIRIFY_TAG_RE = re.compile(r"^\s*-\s+amirify/\S")
...
# Community notes carry no frontmatter; amirify identifies its own by the
# Dataview query it writes into every one of them.
_COMMUNITY_QUERY_MARKER = "FROM #community/"


def _is_amirify_note(path: Path) -> bool:
    """Whether a vault note carries amirify's own frontmatter signature.

    Every note amirify writes opens with a YAML frontmatter block tagging it in
    the ``amirify/`` namespace::
    ...
    user's note that merely mentions amirify in its prose is not adopted.

    Community overview notes are recognised separately: they carry no
    frontmatter at all, so they are identified by amirify's own filename prefix
    ...
    for line in head.splitlines()[1:]:
        if line.strip() == "---":
            return False  # frontmatter closed without an amirify tag
        if _AMIRIFY_TAG_RE.match(line):
            return True
    return False


def _adopt_pre_manifest_notes(out: Path) -> set[str]:
    """Names of notes in *out* that amirify itself wrote before manifests existed.

    Deliberately limited to top-level ``*.md``: those are the only files amirify
    can identify as its own from their content. ``.obsidian/graph.json`` is NOT
    adopted — amirify writes one, but so does Obsidian, and with no manifest
    ...
    try:
        ...
        return set()
    return {p.name for p in candidates if _is_amirify_note(p)}
...
def to_obsidian(
    ...
    # .obsidian/ config. Track the files amirify owns in a manifest; a pre-existing
    # file NOT in the manifest is the user's and is never overwritten.
    _manifest_path = out / ".amirify_obsidian_manifest.json"
    try:
    ...
    if not _manifest_existed:
        ...
        # amirify owns, so every note it wrote last time reads as the user's and
        # is skipped. The re-export then writes fresh notes BESIDE the stale ones
        # and the vault carries two generations, with a warning claiming amirify
        # "did not create" files it did (#2863). Adopt the notes that carry
        # amirify's own frontmatter, once, so the manifest starts out honest.
        _owned |= _adopt_pre_manifest_notes(out)
    _written: list[str] = []
    _skipped: list[str] = []

    def _owned_write(rel_name: str, content: str) -> bool:
        """Write an amirify-owned file, refusing to overwrite a pre-existing file
        amirify didn't create. Returns True if written."""
        target = out / rel_name
    ...
    # Map file_type → amirify tag
    _FTYPE_TAG = {
        "code": "amirify/code",
        "document": "amirify/document",
        "paper": "amirify/paper",
        "image": "amirify/image",
    }
    ...
    for node_id, data in G.nodes(data=True):
        ...
        ftype_tag = _FTYPE_TAG.get(ftype, f"amirify/{ftype}" if ftype else "amirify/document")
        dom_conf = _dominant_confidence(node_id)
        conf_tag = f"amirify/{dom_conf}"
    ...
    # view — but never clobber an existing .obsidian/graph.json amirify doesn't own
    ...
    # manifest says amirify owns are candidates, and anything written or skipped
    ...
    if pruned:
        print(
            f"[amirify] pruned {pruned} note(s) for nodes no longer in the graph",
            file=sys.stderr,
        )

    # Persist the manifest of files amirify owns, so a re-run can safely update its
    ...
    if _skipped:
        ...
        print(
            f"[amirify] WARNING: skipped {len(_skipped)} pre-existing file(s) amirify "
            f"did not create, to avoid overwriting your notes: {shown}. "
            f"Export into an empty directory (or the default amirify-out/obsidian) "
