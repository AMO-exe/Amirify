...
Run once per project to make amirify always-on in your agent sessions:

```bash
amirify agents install
```

This writes a `## amirify` section to the local `AGENTS.md` that instructs your agent to check the graph before answering codebase questions and rebuild it after code changes. No manual `/amirify` needed in future sessions.

```bash
amirify agents uninstall  # remove the section
```
