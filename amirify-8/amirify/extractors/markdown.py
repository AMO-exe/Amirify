"""Markdown extractor. Moved verbatim from amirify/extract.py."""
...
from amirify.detect import CODE_EXTENSIONS, DOC_EXTENSIONS
from amirify.extractors.base import _file_stem, _make_id
...
# not a declared dependency of amirify (see manifest_ingest._parse_apm for the
...
def _active_scan_root() -> "Path | None":
    ...
    fallback stays off.
    """
    import amirify.extract as _extract
...
def _build_link_index(root: Path) -> "dict[str, list[tuple[int, str, Path]]]":
    ...
    index dot-folders.
    """
    from amirify.detect import _SKIP_DIRS
...
def extract_markdown(path: Path) -> dict:
    ...
    ``amirify.markdown_resolution``). The dotted form travels with its
