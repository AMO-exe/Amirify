...
`amirify/extract.py` is being split into this package, one language per PR
...
3. **Facade re-export is mandatory.** `extract.py` must keep exporting every
   moved name (`from amirify.extractors.<mod> import extract_<lang>  # noqa: F401`
...
4. **Never import from `amirify.extract` inside this package.** Import
...
- run `grep -c '_name' amirify/extract.py` AFTER your candidate move;
...
2. `grep -n 'def extract_<lang>' amirify/extract.py`; the span ends at the
...
3. Save the span to a temp file. Create `amirify/extractors/<lang>.py` with
   module docstring (`"""<Lang> extractor. Moved verbatim from amirify/extract.py."""`),
