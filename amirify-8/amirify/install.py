"""amirify install/uninstall subsystem.

The per-platform skill/hook installers and uninstallers, extracted verbatim from
amirify/__main__.py so the CLI dispatcher (`main`) stays readable. Import path is
unchanged: `amirify.__main__` re-exports every name defined here, so
`from amirify.__main__ import claude_install` (etc.) keeps working.

Lives at amirify/install.py (same package dir as __main__) on purpose: the
installers resolve packaged assets via `Path(__file__).parent / "always_on"` and
`/ "skills"`, which only works from within the amirify/ directory.
"""
...
def _write_version_stamp(skill_dst: Path, version: str) -> None:
    """Atomically write ``.amirify_version`` beside ``skill_dst``.
    ...
    tmp = version_file.with_name(".amirify_version.tmp")
    try:
...
def _always_on(basename: str) -> str:
    """Read a packaged always-on instruction block from amirify/always_on/.
    ...
    try:
        ...
        raise RuntimeError(
            f"amirify install is incomplete: missing always-on block '{basename}' "
            f"at {path}. Reinstall amirify (e.g. `uv tool install --reinstall amirify`)."
        ) from exc
def _platform_skill_destination(platform_name: str, *, project: bool = False, project_dir: Path | None = None) -> Path:
    ...
    if platform_name == "gemini":
        if project:
            return (project_dir or Path(".")) / ".gemini" / "skills" / "amirify" / "SKILL.md"
        if platform.system() == "Windows":
            return Path.home() / ".agents" / "skills" / "amirify" / "SKILL.md"
        return Path.home() / ".gemini" / "skills" / "amirify" / "SKILL.md"

    if platform_name == "opencode":
        if project:
            return (project_dir or Path(".")) / ".opencode" / "skills" / "amirify" / "SKILL.md"
        return Path.home() / ".config" / "opencode" / "skills" / "amirify" / "SKILL.md"

    if platform_name == "hermes":
        if project:
            return (project_dir or Path(".")) / ".hermes" / "skills" / "amirify" / "SKILL.md"
        # On Windows, Hermes scans %LOCALAPPDATA%\hermes\skills, not ~/.hermes (#1403).
        if platform.system() == "Windows":
            local_appdata = Path(os.environ.get("LOCALAPPDATA") or (Path.home() / "AppData" / "Local"))
            return local_appdata / "hermes" / "skills" / "amirify" / "SKILL.md"
        return Path.home() / ".hermes" / "skills" / "amirify" / "SKILL.md"

    if platform_name == "devin":
        if project:
            return (project_dir or Path(".")) / ".devin" / "skills" / "amirify" / "SKILL.md"
        return Path.home() / ".config" / "devin" / "skills" / "amirify" / "SKILL.md"

    if platform_name == "amp":
        if project:
            return (project_dir or Path(".")) / ".agents" / "skills" / "amirify" / "SKILL.md"
        return Path.home() / ".config" / "agents" / "skills" / "amirify" / "SKILL.md"
    ...
    if platform_name in ("antigravity", "antigravity-windows"):
        ...
        return Path.home() / ".gemini" / "config" / "skills" / "amirify" / "SKILL.md"
    ...
    if platform_name in ("claude", "windows") and os.environ.get("CLAUDE_CONFIG_DIR"):
        return Path(os.environ["CLAUDE_CONFIG_DIR"]) / "skills" / "amirify" / "SKILL.md"
...
def _packaged_skill_refs_dir(platform_name: str) -> Path | None:
    ...
    ``amirify/skills/<bundle>/references/``. Reuse keys (e.g. trae-cn) point at
    their twin's bundle.
    ...
    ``amirify/skills/<bundle>/`` is not in this build has not gone progressive
...
def _copy_skill_file(platform_name: str, *, project: bool = False, project_dir: Path | None = None) -> Path:
    ...
    ``.amirify_version`` stamp covers both. For monolith platforms (no
    ...
    if not skill_src.exists():
        print(f"error: {skill_file} not found in package - reinstall amirify", file=sys.stderr)
        sys.exit(1)
...
def _remove_claude_skill_registration(project_dir: Path) -> None:
    ...
    # Match the exact H1 `# amirify` registration heading, never a substring of a
    # user's `## amirify`/`### amirify` (#2062). Section runs to the next H1.
    ...
    if cleaned:
        ...
        print(f"  CLAUDE.md        ->  amirify skill registration removed from {claude_md}")
    else:
        claude_md.unlink()
...
def _claude_pretooluse_hooks(strict: bool = False, project: bool = False) -> "list[dict]":
    """amirify's Claude/Codebuddy PreToolUse hooks, resolved at install time.

    The command invokes `amirify hook-guard <search|read>` via the absolute exe
    path (`_resolve_amirify_exe`) — or, for a project-scoped install, via the
    bare `amirify` command, since that config gets committed (#3129). Either
    form parses under sh, cmd.exe and PowerShell alike — this is the #522 fix,
    and mirrors the codex hook. Matchers are
    "Bash|Grep" and "Read|Glob" and the command always contains "amirify", so the
    ...
    first raw read per session (Claude Code only). The ``AMIRIFY_HOOK_STRICT`` env
    ...
    unset default.
    """
    exe = _resolve_amirify_exe(project=project)
...
def _skill_registration(skill_path: str = "~/.claude/skills/amirify/SKILL.md") -> str:
    # Heading is "# amirify" (H1) to match _SKILL_REGISTRATION_MARKER, which
    # _register_always_on_block anchors its idempotent replace-or-append on.
    return (
        "# amirify\n"
        f"- **amirify** (`{skill_path}`) "
        "- any input to knowledge graph. Trigger: `/amirify`\n"
        "When the user types `/amirify`, use the installed amirify skill "
...
def _register_always_on_block(target: Path, prefix: str, registration: str) -> None:
    ...
    of the bare "amirify" substring check silently treating any unrelated
    ...
    try:
        ...
        print(
            f"  hint: the skill files were installed; add the amirify block to "
...
_PLATFORM_CONFIG: dict[str, dict] = {
    "claude": {
        "skill_file": "skill.md",
        "skill_dst": Path(".claude") / "skills" / "amirify" / "SKILL.md",
        "claude_md": True,
        "skill_refs": "claude",
    },
    "codex": {
        "skill_file": "skill-codex.md",
        "skill_dst": Path(".codex") / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "codex",
    },
    "opencode": {
        "skill_file": "skill-opencode.md",
        "skill_dst": Path(".config") / "opencode" / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "opencode",
    },
    "kilo": {
        "skill_file": "skill-kilo.md",
        "skill_dst": Path(".config") / "kilo" / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "kilo",
    },
    "aider": {
        ...
        "skill_dst": Path(".aider") / "amirify" / "SKILL.md",
        "claude_md": False,
    },
    "copilot": {
        "skill_file": "skill-copilot.md",
        "skill_dst": Path(".copilot") / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "copilot",
    },
    "claw": {
        "skill_file": "skill-claw.md",
        "skill_dst": Path(".openclaw") / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claw",
    },
    "droid": {
        "skill_file": "skill-droid.md",
        "skill_dst": Path(".factory") / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "droid",
    },
    "trae": {
        "skill_file": "skill-trae.md",
        "skill_dst": Path(".trae") / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "trae",
    },
    "trae-cn": {
        ...
        "skill_file": "skill-trae.md",
        "skill_dst": Path(".trae-cn") / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "trae",
    },
    "hermes": {
        ...
        "skill_file": "skill-claw.md",
        "skill_dst": Path(".hermes") / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claw",
    },
    "kiro": {
        "skill_file": "skill-kiro.md",
        "skill_dst": Path(".kiro") / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "kiro",
    },
    "pi": {
        "skill_file": "skill-pi.md",
        "skill_dst": Path(".pi") / "agent" / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "pi",
    },
    "codebuddy": {
        ...
        "skill_file": "skill.md",
        "skill_dst": Path(".codebuddy") / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claude",
    },
    "antigravity": {
        ...
        "skill_file": "skill.md",
        "skill_dst": Path(".agents") / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claude",
    },
    "antigravity-windows": {
    ...
    "kimi": {
        # Reuses claude's split bundle (shares skill.md).
        "skill_file": "skill.md",
        "skill_dst": Path(".kimi") / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claude",
    },
    "amp": {
    ...
    "devin": {
        ...
        # User scope: ~/.config/devin/skills/amirify/SKILL.md
        # Project scope: .devin/skills/amirify/SKILL.md (overridden in _platform_skill_destination)
        "skill_dst": Path(".config") / "devin" / "skills" / "amirify" / "SKILL.md",
        "claude_md": False,
    },
}
...
def _replace_or_append_section(
    ...
    """Idempotently update or append an amirify-owned section in shared files.
    ...
    amirify`` inside a bullet or an inline reference used to anchor the replace
    ...
    headings exist, the last one is used, since amirify's section is appended.
    """
    lines = content.split("\n")
...
def _remove_marker_section(content: str, marker: str, boundary_prefix: str = "## ") -> "str | None":
    ...
    regex ``## amirify`` was unanchored, so it matched inside a user's
    ``### amirify`` heading and deleted hand-written content (#2062) — the same
...
def _print_banner() -> None:
    """Amber brain banner on amirify install. TTY-only, never raises."""
...
def install(platform: str = "claude", *, project: bool = False, project_dir: Path | None = None) -> None:
    ...
    if platform == "kilo":
        # Kilo Code also supports a native /amirify command file.
        command_src = Path(__file__).parent / "command-kilo.md"
        if not command_src.exists():
            print(
                f"error: command-kilo.md not found in package - reinstall amirify",
                file=sys.stderr,
            )
            sys.exit(1)
        command_dst = Path.home() / ".config" / "kilo" / "command" / "amirify.md"
    ...
    if cfg["claude_md"]:
        ...
        if project:
            claude_md = project_dir / ".claude" / "CLAUDE.md"
            skill_ref = ".claude/skills/amirify/SKILL.md"
        elif os.environ.get("CLAUDE_CONFIG_DIR"):
            config_dir = Path(os.environ["CLAUDE_CONFIG_DIR"])
            claude_md = config_dir / "CLAUDE.md"
            skill_ref = str(config_dir / "skills" / "amirify" / "SKILL.md")
        else:
    ...
    if platform == "codebuddy":
        ...
        _register_always_on_block(
            ...
            "  CODEBUDDY.md     ->  ",
            _skill_registration("~/.codebuddy/skills/amirify/SKILL.md"),
        )
    ...
    print("  /amirify .")
    print()
    print("Prefer a hosted version? Early access to the original graphify platform is")
...
def _print_install_usage() -> None:
    ...
    print("Usage: amirify install [--project] [--strict] [--platform P|P]")
    print(f"Platforms: {platforms}")
    print("  --strict  block the first raw file read per session until one "
          "`amirify query` runs (Claude Code project hook only; needs --project)")
_CLAUDE_MD_MARKER = "## amirify"
_CODEBUDDY_MD_MARKER = "## amirify"
_AGENTS_MD_MARKER = "## amirify"
_GEMINI_MD_MARKER = "## amirify"
...
# collide with a genuine user-authored "## amirify" heading elsewhere in the
# same file (#2062) — _remove_claude_skill_registration matches on this same
# constant so the two stay in sync.
_SKILL_REGISTRATION_MARKER = "# amirify"
def _gemini_hook(project: bool = False) -> dict:
    """Gemini CLI BeforeTool hook, resolved to a shell-agnostic `amirify` call.
...
def gemini_install(project_dir: Path | None = None, *, project: bool = False) -> None:
    ...
    if target.exists() and new_content == target.read_text(encoding="utf-8"):
        print(f"amirify already configured in {target.resolve()} (no change)")
    else:
        ...
        target.write_text(new_content, encoding="utf-8", newline="")
        print(f"amirify section written to {target.resolve()}")
...
def _refuse_to_modify(settings_path: Path) -> "NoReturn":
    ...
    print(
        f"[amirify] refusing to modify {settings_path}: not valid JSON "
...
def _write_settings_with_backup(settings_path: Path, settings: dict) -> None:
    ...
    the existing file to ``<name>.amirify-bak`` (single rolling backup) before
...
def _install_gemini_hook(project_dir: Path, project: bool = False) -> None:
    ...
    if not isinstance(before_tool, list):
        _refuse_to_modify(settings_path)
    hooks["BeforeTool"] = [
        h for h in before_tool if "amirify" not in str(h)
    ]
...
def _uninstall_gemini_hook(project_dir: Path) -> None:
    ...
    filtered = [h for h in before_tool if "amirify" not in str(h)]
...
def gemini_uninstall(project_dir: Path | None = None, *, project: bool = False, remove_user_skill: bool | None = None) -> None:
    """Remove the amirify section from GEMINI.md, uninstall hook, and remove skill file.
    ...
    if cleaned is None:
        print("amirify section not found in GEMINI.md - nothing to do")
        return
    if cleaned:
        target.write_text(cleaned + "\n", encoding="utf-8")
        print(f"amirify section removed from {target.resolve()}")
    else:
        target.unlink()
        print(f"GEMINI.md was empty after removal - deleted {target.resolve()}")
    _uninstall_gemini_hook(project_dir)
_VSCODE_INSTRUCTIONS_MARKER = "## amirify"
def vscode_install(project_dir: Path | None = None) -> None:
    """Install amirify skill for VS Code Copilot Chat + write .github/copilot-instructions.md."""
    skill_src = Path(__file__).parent / "skill-vscode.md"
    refs_bundle = "vscode"
    if not skill_src.exists():
        skill_src = Path(__file__).parent / "skill-copilot.md"
        refs_bundle = "copilot"
    skill_dst = Path.home() / ".copilot" / "skills" / "amirify" / "SKILL.md"
    skill_dst.parent.mkdir(parents=True, exist_ok=True)
    tmp_dst = skill_dst.with_suffix(skill_dst.suffix + ".tmp")
    try:
        shutil.copy(skill_src, tmp_dst)
    ...
    if instructions.exists():
        ...
        if new_content == content:
            ...
            print(f"  {instructions}  ->  amirify section {'updated' if _VSCODE_INSTRUCTIONS_MARKER in content else 'added'}")
    else:
    ...
    print()
    print(
        "VS Code Copilot Chat configured. Type /amirify in the chat panel to build the graph."
    )
    print("Note: for GitHub Copilot CLI (terminal), use: amirify copilot install")
def vscode_uninstall(project_dir: Path | None = None) -> None:
    """Remove amirify VS Code Copilot Chat skill and .github/copilot-instructions.md section."""
    skill_dst = Path.home() / ".copilot" / "skills" / "amirify" / "SKILL.md"
    if skill_dst.exists():
        skill_dst.unlink()
        print(f"  skill removed    ->  {skill_dst}")
    ...
    if cleaned:
        ...
        print(f"  amirify section removed from {instructions}")
    else:
        instructions.unlink()
        print(f"  {instructions}  ->  deleted (was empty after removal)")
_ANTIGRAVITY_RULES_PATH = Path(".agents") / "rules" / "amirify.md"
_ANTIGRAVITY_WORKFLOW_PATH = Path(".agents") / "workflows" / "amirify.md"
...
Follow the amirify skill to run the full pipeline.

If no path argument is given, use `.` (current directory).
"""
def _kiro_install(project_dir: Path) -> None:
    """Write amirify skill + steering file for Kiro IDE/CLI."""
    project_dir = project_dir or Path(".")

    # Skill file + references/ sidecar + .amirify_version stamp via the shared
    ...
    # Steering file → .kiro/steering/amirify.md (always-on)
    steering_dir = project_dir / ".kiro" / "steering"
    steering_dir.mkdir(parents=True, exist_ok=True)
    steering_dst = steering_dir / "amirify.md"
    if steering_dst.exists() and steering_dst.read_text(encoding="utf-8") == _always_on("kiro-steering"):
        print(f"  .kiro/steering/amirify.md  ->  already configured (no change)")
    else:
        # File is wholly amirify-owned. Overwrite on upgrade so older
        ...
        print(f"  .kiro/steering/amirify.md  ->  always-on steering {action}")

    print()
    print("Kiro will now read the knowledge graph before every conversation.")
    print("Use /amirify to build or update the graph.")
def _kiro_uninstall(project_dir: Path) -> None:
    """Remove amirify skill + steering file for Kiro."""
    project_dir = project_dir or Path(".")
    removed = []

    # Skill + .amirify_version + references/ sidecar + empty-dir walk.
    ...
    steering_dst = project_dir / ".kiro" / "steering" / "amirify.md"
...
def _antigravity_finalize(skill_dst: Path, project_dir: Path) -> None:
    ...
    writes ``.agents/rules/amirify.md`` and ``.agents/workflows/amirify.md``
    ...
    if rules_path.exists():
        ...
        if _always_on("antigravity-rules").strip() != existing.strip():
            ...
            print(f"amirify rule updated at {rules_path.resolve()}")
        else:
            print(f"amirify rule already configured at {rules_path.resolve()} (no change)")
    else:
        rules_path.write_text(_always_on("antigravity-rules"), encoding="utf-8")
        print(f"amirify rule written to {rules_path.resolve()}")

    # .agents/workflows/amirify.md
    ...
    if wf_path.exists():
        ...
        if _ANTIGRAVITY_WORKFLOW.strip() != existing.strip():
            ...
            print(f"amirify workflow updated at {wf_path.resolve()}")
        else:
            print(f"amirify workflow already configured at {wf_path.resolve()} (no change)")
    else:
        wf_path.write_text(_ANTIGRAVITY_WORKFLOW, encoding="utf-8")
        print(f"amirify workflow written to {wf_path.resolve()}")
def _antigravity_install(project_dir: Path) -> None:
    """Install amirify for Google Antigravity (global skill + .agents/rules + .agents/workflows)."""
    # Copy the skill to ~/.gemini/config/skills/amirify/SKILL.md (global), then
    ...
    print("codebase questions. Run /amirify first to build the graph.")
    print()
    print(
    ...
    print('    "command": "uv",')
    print(
        '    "args": ["run", "--with", "amirify", "--with", "mcp", "-m", "amirify.serve", "${workspace.path}/amirify-out/graph.json"]'
    )
    print("  }")
def _antigravity_uninstall(project_dir: Path, *, project: bool = False) -> None:
    """Remove amirify Antigravity rules, workflow, and skill files."""
    # Remove rules file
    rules_path = project_dir / _ANTIGRAVITY_RULES_PATH
    if rules_path.exists():
        rules_path.unlink()
        print(f"amirify rule removed from {rules_path.resolve()}")
    else:
        print("No amirify Antigravity rule found - nothing to do")

    # Remove workflow file
    wf_path = project_dir / _ANTIGRAVITY_WORKFLOW_PATH
    if wf_path.exists():
        wf_path.unlink()
        print(f"amirify workflow removed from {wf_path.resolve()}")

    # Remove skill file
    skill_dst = _platform_skill_destination("antigravity", project=project, project_dir=project_dir)
    if skill_dst.exists():
        skill_dst.unlink()
        print(f"amirify skill removed from {skill_dst}")
    version_file = skill_dst.parent / ".amirify_version"
    if version_file.exists():
        version_file.unlink()
    refs_dir = skill_dst.parent / "references"
    ...
    for d in (
        ...
        try:
            ...
            break
_CURSOR_RULE_PATH = Path(".cursor") / "rules" / "amirify.mdc"
_CURSOR_RULE = """\
---
...
**MANDATORY: Before using Read, Grep, Glob, or Bash to explore the codebase, you MUST run amirify first:**
- `amirify query "<question>"` — scoped subgraph for any codebase or architecture question
- `amirify path "<A>" "<B>"` — dependency path between two symbols
- `amirify explain "<concept>"` — all nodes related to a concept

This applies to YOU and to every subagent you spawn. Include this rule explicitly in every subagent prompt that involves code exploration. Do not skip amirify because files are "already known" or because you are executing a plan — the graph surfaces cross-file dependencies and INFERRED edges that grep and Read cannot find.

Only use Read/Grep/Glob directly when:
1. amirify has already oriented you and you need to modify or debug specific lines
2. `amirify-out/graph.json` does not exist yet

- If `amirify-out/wiki/index.md` exists, navigate it instead of reading raw files
- Read `amirify-out/GRAPH_REPORT.md` only for broad architecture review when query/path/explain do not surface enough context
- After modifying code files, run `amirify update .` to keep the graph current (AST-only, no API cost)
"""
def _cursor_install(project_dir: Path) -> None:
    """Write .cursor/rules/amirify.mdc with alwaysApply: true."""
    ...
    if rule_path.exists() and rule_path.read_text(encoding="utf-8") == _CURSOR_RULE:
        print(f"amirify rule at {rule_path} already configured (no change)")
        return
    # File is wholly amirify-owned. Overwrite on upgrade so older
    # report-first wording does not silently linger (issue #580).
    action = "updated" if rule_path.exists() else "written"
    rule_path.write_text(_CURSOR_RULE, encoding="utf-8")
    print(f"amirify rule {action} at {rule_path.resolve()}")
    print()
    print("Cursor will now always include the knowledge graph context.")
    print("Run /amirify . first to build the graph if you haven't already.")
def _cursor_uninstall(project_dir: Path) -> None:
    """Remove .cursor/rules/amirify.mdc."""
    rule_path = (project_dir or Path(".")) / _CURSOR_RULE_PATH
    if not rule_path.exists():
        print("No amirify Cursor rule found - nothing to do")
        return
    rule_path.unlink()
    print(f"amirify Cursor rule removed from {rule_path.resolve()}")
# Devin CLI — .windsurf/rules/amirify.md (always-on context)
# Devin reads .windsurf/rules/*.md files the same way Windsurf IDE does.
_DEVIN_RULES_PATH = Path(".windsurf") / "rules" / "amirify.md"
_DEVIN_RULES = """\
## amirify
...
Rules:
- For codebase or architecture questions, when `amirify-out/graph.json` exists, first run `amirify query "<question>"` (or `amirify path "<A>" "<B>"` / `amirify explain "<concept>"`). These return a scoped subgraph, usually much smaller than `GRAPH_REPORT.md` or raw grep output.
- If amirify-out/wiki/index.md exists, navigate it instead of reading raw files
...
def _devin_rules_install(project_dir: Path) -> None:
    """Write .windsurf/rules/amirify.md for always-on Devin context."""
...
def _devin_rules_uninstall(project_dir: Path) -> None:
    """Remove .windsurf/rules/amirify.md."""
    rules_path = (project_dir or Path(".")) / _DEVIN_RULES_PATH
...
export const AmirifyPlugin = async ({ directory }) => {
  let reminded = false;

  return {
    "tool.execute.before": async (input, output) => {
      if (reminded) return;
      if (!existsSync(join(directory, "amirify-out", "graph.json"))) return;
      ...
      if (input.tool === "bash") {
        ...
        output.args.command =
          'echo "[amirify] Knowledge graph available. Read amirify-out/GRAPH_REPORT.md for god nodes and architecture context before searching files." ; ' +
          output.args.command;
        reminded = true;
      }
    },
...
"""
_KILO_PLUGIN_PATH = Path(".kilo") / "plugins" / "amirify.js"
...
def _install_kilo_plugin(project_dir: Path) -> None:
    """Write amirify.js plugin and register it without rewriting user JSONC."""
...
def _uninstall_kilo_plugin(project_dir: Path) -> None:
    """Remove amirify.js plugin and deregister it without rewriting user JSONC."""
    plugin_file = project_dir / _KILO_PLUGIN_PATH
...
_OPENCODE_PLUGIN_JS = """\
// amirify OpenCode plugin
// Injects a knowledge graph reminder before bash tool calls when the graph exists.
//
// IMPORTANT: keep the reminder string free of backticks and $(...) constructs.
// The hook prepends `echo "<reminder>" && <cmd>` to the user's bash command;
// backticks inside the double-quoted echo trigger bash command substitution,
// which both corrupts tool output and silently executes the very amirify
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      if (input.tool === "bash") {
        ...
        output.args.command =
          'echo "[amirify] knowledge graph at amirify-out/. For focused questions, run amirify query with your question (scoped subgraph, usually much smaller than GRAPH_REPORT.md) instead of grepping raw files. Read GRAPH_REPORT.md only for broad architecture context." ; ' +
          output.args.command;
        reminded = true;
      }
    },
...
"""
_OPENCODE_PLUGIN_PATH = Path(".opencode") / "plugins" / "amirify.js"
_OPENCODE_CONFIG_PATH = Path(".opencode") / "opencode.json"
def _install_opencode_plugin(project_dir: Path) -> None:
    """Write amirify.js plugin and register it in opencode.json."""
...
def _uninstall_opencode_plugin(project_dir: Path) -> None:
    """Remove amirify.js plugin and deregister from opencode.json."""
    plugin_file = project_dir / _OPENCODE_PLUGIN_PATH
    if plugin_file.exists():
        plugin_file.unlink()
...
def _resolve_amirify_exe(project: bool = False) -> str:
    """Return the absolute path to the amirify executable, with forward slashes.

    With *project* set, return the bare ``amirify`` command instead. A
    ...
    checkouts. A committed hook refers to ``amirify`` the way it would refer to
    ``git`` or ``node``, and PATH resolves it per machine (#3129).

    Falls back to bare 'amirify' if resolution fails. Using an absolute path
    ...
    character: a raw ``C:\\Users\\me\\amirify.EXE`` collapses to
    ``C:Usersmeamirify.EXE: command not found`` and the guard silently fails.
...
def _install_codex_hook(project_dir: Path, project: bool = False) -> None:
    ...
    amirify_exe = _resolve_amirify_exe(project=project)
    hook_entry = {
        "hooks": {
            "PreToolUse": [
                {
                    "matcher": "Bash",
                    "hooks": [{"type": "command", "command": f"{amirify_exe} hook-check"}],
                }
            ]
        }
    }
    ...
    if not isinstance(pre_tool, list):
        _refuse_to_modify(hooks_path)
    hooks["PreToolUse"] = [h for h in pre_tool if "amirify" not in str(h)]
    hooks["PreToolUse"].extend(hook_entry["hooks"]["PreToolUse"])
    _write_settings_with_backup(hooks_path, existing)
    print(
        f"  .codex/hooks.json  ->  PreToolUse hook registered ({amirify_exe} hook-check"
...
def _uninstall_codex_hook(project_dir: Path) -> None:
    """Remove amirify PreToolUse hook from .codex/hooks.json."""
    hooks_path = project_dir / ".codex" / "hooks.json"
    ...
    filtered = [h for h in pre_tool if "amirify" not in str(h)]
    existing["hooks"]["PreToolUse"] = filtered
    hooks_path.write_text(json.dumps(existing, indent=2), encoding="utf-8")
    print(f"  .codex/hooks.json  ->  PreToolUse hook removed")
def _agents_install(project_dir: Path, platform: str, project: bool = False) -> None:
    """Write the amirify section to the local AGENTS.md for always-on platforms."""
...
def _amp_legacy_cleanup() -> None:
    """Best-effort removal of the pre-fix ~/.amp/skills/amirify install dir.

    Older amirify versions wrote the Amp skill to ~/.amp/skills, which Amp does
    ...
    """
    legacy = Path.home() / ".amp" / "skills" / "amirify"
    if legacy.exists():
...
def _agents_platform_install(project_dir: Path | None = None) -> None:
    """`amirify agents install`: skill into ~/.agents/skills + AGENTS.md.
    ...
    rendered hooks reference, which points at `amirify agents install`. The bare
    `amirify install --platform agents` path stays skill-only (via install()),
...
def _agents_platform_uninstall(project_dir: Path | None = None) -> None:
    """`amirify agents uninstall`: remove the skill and the AGENTS.md section."""
...
def _project_install(platform_name: str, project_dir: Path | None = None, strict: bool = False) -> None:
    ...
    if platform_name in ("claude", "windows"):
        ...
        # root. `agents` -> ./.agents/skills/amirify/SKILL.md.
        skill_dst = _copy_skill_file(platform_name, project=True, project_dir=project_dir)
        _print_project_git_add_hint([_project_scope_root(skill_dst, project_dir)])
    else:
        install(platform=platform_name, project=True, project_dir=project_dir)
...
def _project_uninstall_all(project_dir: Path | None = None) -> None:
    ...
    project_dir = project_dir or Path(".")
    print("Uninstalling project-scoped amirify files...\n")
...
def _agents_uninstall(project_dir: Path, platform: str = "") -> None:
    """Remove the amirify section from the local AGENTS.md."""
    target = (project_dir or Path(".")) / "AGENTS.md"
    ...
    if cleaned is None:
        print("amirify section not found in AGENTS.md - nothing to do")
        if platform == "opencode":
            _uninstall_opencode_plugin(project_dir or Path("."))
        elif platform == "kilo":
            _uninstall_kilo_plugin(project_dir or Path("."))
...
def claude_install(project_dir: Path | None = None, strict: bool = False, project: bool = False) -> None:
    """Write the amirify section to the local CLAUDE.md."""
    ...
    if strict:
        ...
        print("one `amirify query` runs (toggle with AMIRIFY_HOOK_STRICT=0).")
def _install_claude_hook(project_dir: Path, strict: bool = False, project: bool = False) -> None:
    """Add amirify PreToolUse hook to .claude/settings.json.
    ...
    hooks["PreToolUse"] = [h for h in pre_tool if not (isinstance(h, dict) and h.get("matcher") in ("Glob|Grep", "Bash", "Bash|Grep", "Read|Glob") and "amirify" in str(h))]
...
def _uninstall_claude_hook(project_dir: Path) -> None:
    """Remove the amirify PreToolUse hook from .claude/settings.json and its
    ...
    for name in ("settings.json", "settings.local.json"):
        _strip_amirify_hook(claude_dir / name)
def _strip_amirify_hook(settings_path: Path) -> None:
    """Drop amirify PreToolUse hooks from a single Claude settings file, if present."""
    if not settings_path.exists():
        return
    try:
        settings = json.loads(settings_path.read_text(encoding="utf-8"))
    ...
    filtered = [h for h in pre_tool if not (h.get("matcher") in ("Glob|Grep", "Bash", "Bash|Grep", "Read|Glob") and "amirify" in str(h))]
...
def uninstall_all(project_dir: Path | None = None, purge: bool = False) -> None:
    """Remove amirify from every platform detected in the current project."""
    pd = project_dir or Path(".")
    print("Uninstalling amirify from all detected platforms...\n")

    # Skill-file / config-section uninstallers. remove_user_skill=True keeps the
    # historical `amirify uninstall` behavior: global skill delete plus md/hook
    ...
    # Git hook
    try:
        from amirify.hooks import uninstall as hook_uninstall
    ...
    if purge:
        ...
        if out.exists():
            _shutil.rmtree(out)
            print(f"\n  {_AMIRIFY_OUT}/  ->  deleted (--purge)")
        else:
            print(f"\n  {_AMIRIFY_OUT}/  ->  not found (nothing to purge)")

    print("\nDone. Run 'pip uninstall amirify' to remove the package itself.")
def claude_uninstall(project_dir: Path | None = None, *, project: bool = False, remove_user_skill: bool | None = None) -> None:
    """Remove the amirify skill tree (SKILL.md + references/) and the amirify
    section from CLAUDE.md and its local-only variants, plus the PreToolUse hook.

    Mirrors gemini_uninstall: the bare `amirify uninstall` and `amirify claude
    ...
    for target in existing:
        ...
        if _strip_amirify_md_section(target):
            removed_any = True

    if not existing:
        print("No CLAUDE.md found in current directory - nothing to do")
    elif not removed_any:
        print("amirify section not found in CLAUDE.md - nothing to do")

    _uninstall_claude_hook(project_dir)
def _strip_amirify_md_section(target: Path) -> bool:
    """Strip the ## amirify section from one CLAUDE.md-style file.
    ...
    try:
        ...
        return False
    # Remove amirify's ## amirify section (heading matched exactly, never as a
    # substring of a user's ### amirify) from the marker to the next H2 or EOF.
...
def codebuddy_install(project_dir: Path | None = None) -> None:
    """Install the amirify skill and CODEBUDDY.md section for CodeBuddy."""
...
def _install_codebuddy_hook(project_dir: Path) -> None:
    """Add amirify PreToolUse hook to .codebuddy/settings.json."""
...
def _uninstall_codebuddy_hook(project_dir: Path) -> None:
    """Remove amirify PreToolUse hook from .codebuddy/settings.json."""
    settings_path = project_dir / ".codebuddy" / "settings.json"
    if not settings_path.exists():
        return
    try:
...
def codebuddy_uninstall(project_dir: Path | None = None, *, project: bool = False, remove_user_skill: bool | None = None) -> None:
    """Remove the amirify skill tree (SKILL.md + references/) and the CODEBUDDY.md section.
    ...
    if cleaned is None:
        print("amirify section not found in CODEBUDDY.md - nothing to do")
        return
...
def dispatch_install_cli(cmd: str) -> bool:
    """Handle `amirify <install-command>` dispatch (install/uninstall + every
    ...
    if cmd == "install":
        ...
        if project_scope:
            ...
            if strict:
                print(
                    ...
                    "`amirify install --project --strict` or `amirify claude install --strict`.",
                    file=sys.stderr,
                )
        ...
        if subcmd == "install":
            ...
            print("Usage: amirify claude [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "codebuddy":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: amirify codebuddy [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "gemini":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: amirify gemini [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "cursor":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: amirify cursor [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "vscode":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: amirify vscode [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "copilot":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: amirify copilot [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "kilo":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: amirify kilo [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "kiro":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: amirify kiro [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "devin":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: amirify devin [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "pi":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: amirify pi [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "amp":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: amirify amp [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print(f"Usage: amirify {cmd} [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print("Usage: amirify antigravity [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    return True
