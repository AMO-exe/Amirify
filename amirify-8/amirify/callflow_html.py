#!/usr/bin/env python3
"""
callflow_html.py — Generate call-flow architecture HTML from amirify knowledge graph outputs.

Reads graph.json plus optional GRAPH_REPORT.md, .amirify_labels.json, and sections JSON,
...
Usage:
  python3 -m amirify export callflow-html
  python3 -m amirify export callflow-html /path/to/project/amirify-out/graph.json
  python3 -m amirify export callflow-html --graph /path/to/graph.json --output docs/architecture.html
"""
...
from amirify.paths import AMIRIFY_OUT, AMIRIFY_OUT_NAME
...
def normalize_node(raw: dict, index: int) -> dict:
    """Normalize an amirify node across common graph.json schema variants."""
    node = dict(raw)
...
def normalize_edge(raw: dict, index: int) -> dict | None:
    """Normalize amirify edges while preserving original fields."""
    edge = dict(raw)
...
def _node_link_payload(data: dict) -> tuple[list, list] | None:
    """Read current amirify graph.json via NetworkX's node-link parser."""
...
def load_labels(path: str | Path | None) -> dict:
    """Load community labels from .amirify_labels.json, tolerating wrapper keys."""
...
def infer_project_name(graph_path: str, meta: dict) -> str:
    ...
    if path.parent.name == AMIRIFY_OUT_NAME and len(path.parents) > 1:
        return path.parents[1].name
    return path.parent.name or "Project"


def resolve_amirify_paths(args) -> dict:
    """Resolve project root, amirify output dir, and optional files."""
    base = Path(args.project).expanduser() if args.project else Path.cwd()
    if args.amirify_out:
        amirify_out = Path(args.amirify_out).expanduser()
    elif args.graph:
        amirify_out = Path(args.graph).expanduser().parent
    elif (base / "graph.json").exists():
        amirify_out = base
    else:
        amirify_out = base / AMIRIFY_OUT

    project_root = amirify_out.parent if amirify_out.name == AMIRIFY_OUT_NAME else base
    graph = Path(args.graph).expanduser() if args.graph else amirify_out / "graph.json"
    report = Path(args.report).expanduser() if args.report else amirify_out / "GRAPH_REPORT.md"
    labels = Path(args.labels).expanduser() if args.labels else amirify_out / ".amirify_labels.json"
    sections = Path(args.sections).expanduser() if args.sections else None
    return {
        "base": project_root,
        "amirify_out": amirify_out,
        "graph": graph,
        "report": report,
        "labels": labels,
        "sections": sections,
...
def node_importance(node: dict) -> float:
    """Use amirify centrality fields when available."""
...
def generate_header(sections: list, meta: dict, lang: str) -> str:
    ...
    if lang.startswith("zh"):
        ...
        subtitle = (
            f"由 amirify 知识图谱生成：{meta.get('node_count', '?')} 个节点、"
        ...
        subtitle = (
            f"Generated from amirify knowledge graph: {meta.get('node_count', '?')} nodes, "
...
def generate_section_cards(sec: dict, nodes: list, section_edges: list, lang: str) -> str:
    ...
    note = pick_text(
        lang,
        f"本节由 amirify 社区聚类生成。关系概况：{relation_text}。图表优先展示高置信、跨节点调用或使用关系，完整节点清单位于表格中。",
        f"This section comes from amirify community clustering. Relationship summary: {relation_text}. The diagram prioritizes high-confidence calls or usage relationships; the table keeps the broader node inventory.",
    )
...
class CallflowOptions:
    ...
    def __init__(
        ...
        *,
        amirify_out: str | Path | None = None,
        ...
        self.amirify_out = str(amirify_out) if amirify_out is not None else None
...
def write_callflow_html(
    ...
    *,
    ...
    """Generate call-flow architecture HTML from amirify output files."""
    ...
    paths = resolve_amirify_paths(args)
    if not paths["graph"].exists():
        raise FileNotFoundError(
            f"amirify output not found: {paths['graph']}. "
            "Run amirify first or pass --graph /path/to/graph.json."
        )
    ...
    if args.output:
        ...
        output_path = paths["amirify_out"] / f"{safe_filename(meta['project_name'])}-callflow.html"
...
""")
  ...
  <p>Generated: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')} · amirify callflow-html</p>
</div>
""")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate call-flow architecture HTML from amirify knowledge graph outputs"
    )
    parser.add_argument("project", nargs="?", default=None, help="Project root or amirify output directory")
    parser.add_argument("--amirify-out", default=None, help="Path to amirify output directory")
    ...
    parser.add_argument("--labels", default=None, help="Path to .amirify_labels.json")
