...
from amirify.paths import stem_filename_budget
...
# and resolves fine on every filesystem amirify targets; stripping it would
...
def _community_article(
    ...
    lines += ["---", "", f"*Part of the amirify knowledge wiki. See {_md_link('index', resolver)} to navigate.*"]
    return "\n".join(lines)
...
def _index_md(
    ...
    lines: list[str] = [
        ...
        "",
        "> Auto-generated by amirify. Start here — read community articles for context, then drill into god nodes for detail.",
        "",
    ...
    lines += [
        "---",
        "",
        "*Generated by amirify (built on [graphify](https://github.com/Graphify-Labs/graphify))*",
    ]
    return "\n".join(lines)
...
def to_wiki(
    ...
    if not communities:
        raise ValueError(
            ...
            "Run `amirify extract .` or `amirify cluster-only .` first."
        )
    ...
    if not communities:
        raise ValueError(
            ...
            "Re-run `amirify extract .` to regenerate .amirify_analysis.json."
        )
    ...
    # Windows a 200-char slug under an ordinary amirify-out/wiki/ overruns
