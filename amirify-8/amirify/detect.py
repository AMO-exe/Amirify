...
from amirify.google_workspace import (
    GOOGLE_WORKSPACE_EXTENSIONS,
    convert_google_workspace_file,
    google_workspace_enabled,
)
from amirify.paths import AMIRIFY_OUT, out_path
...
# attacker-controllable (amirify runs on cloned/shared folders), and .docx/.xlsx
...
def classify_file(path: Path) -> FileType | None:
    ...
    from amirify.manifest_ingest import is_package_manifest_path
...
def xlsx_extract_structure(path: Path) -> dict:
    ...
    Returns a nodes/edges dict compatible with the amirify extract pipeline.
...
def convert_office_file(path: Path, out_dir: Path, root: "Path | None" = None) -> Path | None:
    ...
    # byte-identical sidecars — unbounded duplicates when amirify-out/ is
    ...
    import unicodedata
    if root is None:
        # Default layout: out_dir is <root>/<amirify-out>/converted.
        root = out_dir.parent.parent
    try:
        key = path.resolve().relative_to(Path(root).resolve()).as_posix()
    except (ValueError, OSError):
        # Not under the scan root (custom AMIRIFY_OUT layouts, --include
...
def count_words(path: Path) -> int:
    try:
        ...
        # cannot help, and `amirify update` hangs with no output. os.stat
...
_SKIP_DIRS = {
    ...
    "amirify-out",  # never treat the default output as source input (#524)
    ...
    ".amirify",  # amirify's own extraction cache — never index self-generated data
...
def _parse_gitignore_line(raw: str) -> str:
    """Parse one raw line from a .amirifyignore file per gitignore spec.
...
def _git_info_exclude(vcs_root: Path) -> Path | None:
    ...
    a directory without any ``.gitignore`` entry. amirify only read
    ``.gitignore``/``.amirifyignore``, so it walked into those worktree copies and
...
def _read_ignore_text(path: Path) -> str:
    ...
    for enc in (fallback, "latin-1"):
        ...
        if key not in _warned_ignore_encodings:
            ...
            print(
                f"[amirify] WARNING: {path} is not valid UTF-8; read it as "
...
def _load_dir_own_ignore(d: Path, *, gitignore: bool = True) -> list[tuple[Path, str]]:
    """Read .gitignore/.amirifyignore directly inside *d* (not its ancestors).

    Merges .gitignore and .amirifyignore for this one directory (#1363):
    .gitignore is read first and .amirifyignore last, so .amirifyignore
    patterns (including `!` negations) win on conflict via last-match-wins;
    adding a .amirifyignore can only ever exclude MORE, never re-include a
    .gitignore-excluded file (#945 kept: a dir with only a .gitignore still
    gets sensible defaults).

    Shared by `_load_amirifyignore` (ancestor chain, loaded once before the
    ...
    for fname in ((".gitignore", ".amirifyignore") if gitignore else (".amirifyignore",)):
        ignore_file = d / fname
...
def _load_amirifyignore(root: Path, *, gitignore: bool = True) -> list[tuple[Path, str]]:
    """Read .amirifyignore files and return (anchor_dir, pattern) pairs.
    ...
    # per-directory .gitignore/.amirifyignore — so load it first (lowest priority
...
# `amirify watch` process spanning many repos could still accumulate keys over
...
def _is_ignored(
    ...
    *,
    ...
    """Return True if the path should be ignored per .amirifyignore patterns.
...
def _is_scan_ignored(
    ...
    *,
    ...
    contains only .amirifyignore/--exclude rules, which remain authoritative
...
def ignored_predicate(
    ...
    *,
    ...
    .amirifyignore/.gitignore chain (_load_amirifyignore), CLI/persisted
    ...
    """
    root = root.resolve()
    patterns = _load_amirifyignore(root, gitignore=gitignore)
    explicit_patterns = _load_amirifyignore(root, gitignore=False)
    # Only shell out to git when .gitignore actually contributes patterns beyond
    # the explicit (.amirifyignore/--exclude) set: with no .gitignore in play,
    ...
    cache: dict[Path, bool] = {}
    explicit_cache: dict[Path, bool] = {}
    # root's own ignore file is the last entry of _load_amirifyignore's chain.
...
def detect(root: Path, *, follow_symlinks: bool | None = None, google_workspace: bool | None = None, extra_excludes: list[str] | None = None, cache_root: Path | None = None, gitignore: bool = True) -> dict:
    root = root.resolve()
    configured_out_dir = root / AMIRIFY_OUT
    ...
    # .amirifyinclude support was removed (#2112): its loader and matchers had
    ...
    if (root / ".amirifyinclude").is_file():
        import sys as _sys
        print(
            "[amirify] WARNING: .amirifyinclude is no longer supported "
    ...
    def _wc(path: Path) -> int:
        ...
        from amirify import cache as _cache
        return _cache.cached_word_count(path, root, count_words, cache_root=cache_root)

    skipped_sensitive: list[str] = []
    unclassified: list[str] = []
    # Files/dirs dropped by a .gitignore/.amirifyignore rule. Recorded so an
    ...
    ignore_patterns = _load_amirifyignore(root, gitignore=gitignore)
    explicit_ignore_patterns = _load_amirifyignore(root, gitignore=False)
    ...
    # so they win over any .amirifyignore/.gitignore rules (#947).
    if extra_excludes:
        for pat in extra_excludes:
            line = _parse_gitignore_line(pat)
            if line:
    ...
    # Always include amirify-out/memory/ - query results filed back into the graph
    memory_dir = root / AMIRIFY_OUT / "memory"
    scan_paths = [root]
    ...
    def _on_walk_error(err: OSError) -> None:
        ...
        print(
            f"[amirify] WARNING: could not scan {target} ({err}); "
    ...
    converted_dir = out_base / AMIRIFY_OUT / "converted"
        ...
        if ftype:
            if p.suffix.lower() in GOOGLE_WORKSPACE_EXTENSIONS:
                if not google_workspace:
                    skipped_sensitive.append(
                        ...
                        "or set AMIRIFY_GOOGLE_WORKSPACE=1]"
                    )
                    continue
                try:
                ...
                if md_path:
                    ...
                    # amirify-out/ -- an ignore check here would reject the
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - pip install amirify[office]]")
                continue
    ...
    return {
        ...
        "amirifyignore_patterns": len(ignore_patterns),
...
def _to_relative_for_storage(key: str, root: Path) -> str:
    ...
    fallback in :func:`amirify.watch._relativize_source_files` so the
...
def load_manifest(
    ...
    *,
    ...
    manifests with absolute keys pass through unchanged, so an amirify-out/
...
def save_manifest(
    ...
    *,
    ...
    kind="ast"      — written by `amirify update` (AST-only rebuild). Stamps
                      ast_hash; preserves an existing semantic_hash only when
                      the file content is unchanged (mtime + hash match).
    kind="semantic" — written by `amirify extract` after semantic extraction.
    ...
    .amirifyignore/.gitignore/--exclude) are dropped instead of surviving
...
def detect_incremental(
    ...
    *,
    ...
    kind="semantic" (default for extract): a file is "changed" when its
        ...
        semantic extraction pass. Use this for `amirify extract` so that
        files touched by `amirify update` (AST-only) are re-extracted
        semantically.
    kind="ast": a file is "changed" when its ast_hash is missing or its
        content has changed. Use this for `amirify update`.
