"""amirify prs — graph-aware PR dashboard.
...
Usage:
  amirify prs                   # dashboard of all open PRs
  amirify prs <number>          # deep dive on one PR
  amirify prs --triage          # Opus ranks your review queue
  amirify prs --worktrees       # show worktree → branch → PR mapping
  amirify prs --conflicts       # PRs sharing graph communities (merge-order risk)
  amirify prs --base <branch>   # filter to PRs targeting this base (default: v8)
"""
...
def render_dashboard(prs: list[PRInfo], base: str = "v8", show_wrong_base: bool = False) -> None:
    ...
    print()
    print(bold(f"  amirify prs  ·  base: {base}  ·  {len(actionable)} PRs"))
    print()
...
def _resolve_triage_backend() -> tuple[str, str]:
    """Return (backend, model) using AMIRIFY_TRIAGE_BACKEND or first available key."""
    from amirify.llm import BACKENDS, _get_backend_api_key, _default_model_for_backend

    explicit = os.environ.get("AMIRIFY_TRIAGE_BACKEND", "").strip()
    if explicit in BACKENDS:
        model = (os.environ.get("AMIRIFY_TRIAGE_MODEL")
...
def triage_with_opus(prs: list[PRInfo], base: str) -> None:
    try:
        from amirify.llm import BACKENDS, _get_backend_api_key
    except ImportError:
        print(red("  amirify.llm not available - cannot run triage."), file=sys.stderr)
        sys.exit(1)
