...
# --- migrated to amirify/extractors/ (see amirify/extractors/MIGRATION.md) ---
from amirify.extractors.base import (  # noqa: F401
    _LANGUAGE_BUILTIN_GLOBALS,
    _file_stem,
    _make_id,
    _read_text,
)
from amirify.extractors.apex import extract_apex  # noqa: F401
from amirify.extractors.bash import extract_bash  # noqa: F401
from amirify.extractors.blade import extract_blade  # noqa: F401
from amirify.extractors.cobol import extract_cobol  # noqa: F401
from amirify.extractors.csharp import (
    CsharpNameResolver,
    _resolve_cross_file_csharp_imports,
    _resolve_csharp_type_references,
)
from amirify.extractors.dart import extract_dart  # noqa: F401
from amirify.extractors.dm import extract_dm, extract_dmf, extract_dmi, extract_dmm  # noqa: F401
from amirify.extractors.elixir import extract_elixir  # noqa: F401
from amirify.extractors.erlang import extract_erlang, resolve_erlang_remote_calls  # noqa: F401
from amirify.extractors.fortran import _cpp_preprocess, extract_fortran  # noqa: F401
from amirify.extractors.go import _GO_PREDECLARED_FUNCS, extract_go  # noqa: F401
from amirify.extractors.json_config import extract_json  # noqa: F401
from amirify.extractors.commonlisp import extract_commonlisp  # noqa: F401
from amirify.extractors.markdown import extract_markdown, _MD_LINK_INDEX_CACHE  # noqa: F401
from amirify.extractors.ocaml import extract_ocaml  # noqa: F401
from amirify.extractors.pascal_forms import extract_delphi_form, extract_lazarus_form  # noqa: F401
from amirify.extractors.powershell import extract_powershell, extract_powershell_manifest  # noqa: F401
from amirify.extractors.r import extract_r, resolve_r_sourced_calls  # noqa: F401
from amirify.extractors.razor import extract_razor  # noqa: F401
from amirify.extractors.robot import extract_robot  # noqa: F401
from amirify.extractors.rust import extract_rust  # noqa: F401
from amirify.extractors.sln import extract_sln  # noqa: F401
from amirify.extractors.solidity import (  # noqa: F401
    extract_solidity,
    resolve_solidity_type_references,
)
from amirify.extractors.sql import extract_sql  # noqa: F401
from amirify.extractors.terraform import extract_terraform, prepare_terraform, resolve_terraform_modules  # noqa: F401
from amirify.extractors.verilog import extract_verilog  # noqa: F401
from amirify.extractors.vbnet import extract_vbnet, resolve_vbnet_partial_calls  # noqa: F401
from amirify.extractors.zig import extract_zig  # noqa: F401
from amirify.security import sanitize_metadata
from amirify.paths import disambiguate_ambiguous_candidates

from amirify.extractors.models import LanguageConfig, _JS_CACHE_BYPASS_SUFFIXES, _NamespaceExportFact, _StarExportFact, _SymbolAliasFact, _SymbolDeclarationFact, _SymbolExportFact, _SymbolImportFact, _SymbolResolutionFacts, _SymbolUseFact, _WORKSPACE_PACKAGE_CACHE  # noqa: E402,F401

from amirify.extractors.resolution import (  # noqa: E402,F401
...
def _safe_extract(extractor: Callable, path: Path) -> dict:
    try:
        ...
        if os.environ.get("AMIRIFY_DEBUG"):
            import traceback
...
def _remap_objc_field_tables(per_file: list, mapping: dict) -> None:
    ...
    `amirify update <dir>`) the table keys went stale, every
...
def _resolve_elixir_import_targets(
    ...
    # this gate keeps working on the `amirify update` / watch path.
    node_by_id: dict[str, dict] = {}
...
# registry (framework lives in amirify.resolver_registry). A new language plugs in
...
# amirify.ruby_resolution; registered here as a second consumer of the framework.
register_language_resolver(
...
# scope. Lives in amirify.pascal_resolution; registered here as a consumer
...
# graph unreachable from the call site. Lives in amirify.csharp_dispatch;
...
# final. Lives in amirify.markdown_resolution; the shared call pass above skips
...
# Extensions amirify creates document file nodes for. A link to one of these
...
def _check_tree_sitter_version() -> None:
    ...
    if LANGUAGE_VERSION < 14:
        ...
        raise RuntimeError(
            ...
            f"amirify requires tree-sitter >= 0.23.0 (Language API v2). "
...
def _xaml_csharp_class_nodes(path: Path) -> dict[str, list[dict]]:
    from amirify.detect import _is_ignored, _is_noise_dir, _load_amirifyignore
    ...
    patterns = _load_amirifyignore(root)
...
def _extract_parallel(
    ...
    if max_workers is None:
        # Honour AMIRIFY_MAX_WORKERS env override; otherwise scale to the
        ...
        env_raw = os.environ.get("AMIRIFY_MAX_WORKERS", "").strip()
        env_cap = None
        if env_raw:
            try:
                v = int(env_raw)
    ...
    # AMIRIFY_MAX_WORKERS, and --max-workers — stays valid on >61-core boxes
    ...
    # Windows post-commit hook exports AMIRIFY_MAX_WORKERS=1, so this is the
...
def extract(
    ...
    *,
    ...
    Args:
        ...
        cache_root: explicit root for amirify-out/cache/ (overrides the
            inferred common path prefix). Pass Path('.') when running on a
            subdirectory so the cache stays at ./amirify-out/cache/.
        ...
        max_workers: max subprocess count. Defaults to cpu_count (or the
            value of AMIRIFY_MAX_WORKERS if set), bounded by len(uncached_work).
    ...
    # was never observed again for the life of the process. `amirify watch`
    ...
    # that starts/stops existing between rebuilds in a long-lived `amirify
    ...
    # common parent of the inputs), which would drop amirify-out/ inside a
    ...
    # (or the transient failure self-heals) without deleting amirify-out/.
    ...
    from amirify.detect import CODE_EXTENSIONS as _CODE_EXTS
    ...
    if _no_extractor:
        ...
        print(
            f"  warning: {_tot} file(s) are classified as code but amirify has no AST "
    ...
    for _ext, _n in sorted(_missing_dep_count.items(), key=lambda kv: (-kv[1], kv[0])):
        ...
        if _extra and _DEP_MISSING_MARKER in _err_text:
            ...
            _hint = f' Install it with: pip install "amirify[{_extra}]"'
    ...
    # single-definition god-node guard. Registered in amirify.resolver_registry so
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m amirify.extract <file_or_dir> ...", file=sys.stderr)
        sys.exit(1)
