...
# Single source of truth in amirify.paths (#1423); re-exported as _AMIRIFY_OUT.
from amirify.paths import (
    AMIRIFY_OUT as _AMIRIFY_OUT,
...
_BUILD_CONFIG_FILENAME = ".amirify_build.json"
...
def _apply_resource_limits() -> None:
    ...
    AMIRIFY_REBUILD_MEMORY_LIMIT_MB caps RSS-ish memory. Uses RLIMIT_DATA on
    ...
    try:
        ...
        pass
    mb = os.environ.get("AMIRIFY_REBUILD_MEMORY_LIMIT_MB", "").strip()
    if not mb:
        return
    try:
...
def _git_head(cwd: Path | str | None = None) -> str | None:
    ...
    inherits the caller's working directory, so `amirify update <target>`
...
def _amirify_root_marker_value(watch_path: Path) -> str:
    """The value to write into ``.amirify_root``.

    Ordinarily preserves the caller-supplied path verbatim (relative or
    absolute) so a committed ``amirify-out/.amirify_root`` stays portable
    ...
    of it (the generated git hooks, an unqualified ``amirify watch``) only
    runs with that same CWD anyway.

    That assumption breaks when ``AMIRIFY_OUT`` itself is an absolute,
    ...
    """
    if Path(_AMIRIFY_OUT).is_absolute():
...
def _reconcile_markdown_links(
    ...
    """
    from amirify.build import _is_ast_tier
    from amirify.extract import _file_node_id, _safe_extract_with_xaml_root
    from amirify.extractors.base import _make_id
    from amirify.extractors.markdown import extract_markdown
    from amirify.markdown_resolution import MARKDOWN_MENTION_SUFFIXES, _is_file_node
    ...
    for source_rep, target_rep, edge, source_file, _ in desired_edges:
        ...
        # amirify uses a simple graph. Never overwrite another relation that
...
def _reconcile_existing_graph(
    ...
    *,
    ...
    .amirifyignore + persisted ``--exclude`` patterns — unambiguous
    ...
    ``amirify update``) so the incremental/hook path keeps preserving a
    ...
    from amirify.build import _load_existing_graph
    ...
    try:
        from amirify.build import _norm_source_file as _nsf
        ...
        def _ignored_now(identity: str) -> bool:
            ...
            .amirifyignore/--exclude matches evict on every rebuild;
        ...
        if newly_ignored_files:
            print(
                f"[amirify watch] pruned {newly_ignored_nodes} node(s) from "
        ...
        if excluded_alive_files:
            print(
                f"[amirify watch] fail-closed: kept {excluded_alive_nodes} node(s) "
                ...
                "changed?). Add them to .amirifyignore if the exclusion is intentional."
            )
        ...
        print(
            "[amirify watch] reconcile of existing graph failed "
...
def _node_community_map(graph_data: dict) -> dict[str, int]:
    ...
    for node in graph_data.get("nodes", []):
        ...
        try:
            ...
            print(
                f"[amirify watch] Skipping node with invalid community id: "
...
def _check_shrink(
    ...
    *,
    ...
    plain ``amirify update`` after deleting a function refresh the graph without
    ...
    if rebuilt_sources is not None:
        from amirify.build import _norm_source_file
    ...
    print(
        f"[amirify] WARNING: new graph has {len(new_nodes)} nodes but existing "
...
def _stabilize_rebuild_cwd(watch_path: Path) -> bool:
    ...
    Path('.').resolve(), and relative amirify-out mkdirs raise FileNotFoundError
    before the normal rebuild error handling can run. Hooks that know the repo
    root export AMIRIFY_REPO_ROOT so the rebuild can recover by chdir'ing there.
    """
    if watch_path.is_absolute():
        return True

    repo_root = os.environ.get("AMIRIFY_REPO_ROOT", "").strip()
    ...
    try:
        ...
        print(
            "[amirify watch] Rebuild failed: current working directory "
            "no longer exists and AMIRIFY_REPO_ROOT is not set."
        )
        return False
...
def _reconcile_graph_html(out: Path, graph_data: dict) -> str | None:
    ...
    from amirify.exporters.html import _HTML_STALE_MARKER, _viz_node_limit
    ...
    def clear_stale_marker() -> None:
        try:
            ...
            print(
                "[amirify watch] graph.html stale marker could not be cleared; "
    ...
    try:
        from amirify.export import to_html
        from amirify.paths import load_node_link_graph
        ...
        if had_html:
            print(
                "[amirify watch] Stale graph.html left unchanged; "
                f"regeneration will be retried: {exc}"
            )
        else:
            print(f"[amirify watch] Missing graph.html could not be regenerated: {exc}")
        return None
    ...
    if not written or not html_target.exists():
        if had_html:
            ...
            print("[amirify watch] Missing graph.html has no useful community view; skipped.")
        return None
    clear_stale_marker()
    return "rendered"
...
def _rebuild_code(
    ...
    *,
    ...
    JSON to amirify-out/graph.json (mirrors ``extract --no-cluster``).
    ...
    if acquire_lock:
        ...
        with _rebuild_lock(out, blocking=block_on_lock) as got:
            if not got:
                print("[amirify watch] Rebuild already in progress for "
    ...
    try:
        from amirify.extract import extract, _get_extractor
        from amirify.detect import detect
        from amirify.build import build_from_json, _is_ast_tier, _norm_source_file as _nsf
        from amirify.cluster import cluster, remap_communities_to_previous, score_all
        from amirify.analyze import god_nodes, surprising_connections, suggest_questions
        from amirify.report import generate
        from amirify.export import to_json
        ...
        if _unclassified:
            ...
            print(
                f"[amirify watch] {len(_unclassified)} file(s) not classified "
        ...
        # keep. Two tiers: .amirifyignore + persisted --exclude patterns are
        ...
        from amirify.detect import ignored_predicate
        ...
        if not code_files and not existing_graph.exists():
            print("[amirify watch] No code files found - nothing to rebuild.")
            return False
        ...
        if changed_paths is not None:
            ...
            if not wanted and not deleted_paths:
                print("[amirify watch] No tracked code files in change set - skipping rebuild.")
                return True
        ...
        # gate reports them unchanged forever and only deleting amirify-out/
        ...
        if no_cluster:
            ...
            candidate_graph_data = {
                **{k: v for k, v in result.items() if k not in ("edges", "nodes")},
                ...
                # `amirify update --no-cluster` can't silently drop it -
            ...
            # _amirify_root_marker_value for why this isn't always the raw
            # caller-supplied value (#3375).
            (out / ".amirify_root").write_text(
                _amirify_root_marker_value(watch_path), encoding="utf-8"
            )
            ...
            if same_graph:
                print("[amirify watch] No code-graph changes detected (--no-cluster); outputs left untouched.")
            else:
                print(
                    "[amirify watch] Rebuilt (no clustering): "
                    f"{len(candidate_graph_data.get('nodes', []))} nodes, "
                    f"{len(candidate_graph_data.get('links', []))} edges"
                )
                print(f"[amirify watch] graph.json updated in {out}")
            return True
        ...
        # Inherit the existing graph's directed flag (#2342) so `amirify
        ...
        if existing_graph_data:
            ...
            if same_topology:
                ...
                if html_action == "rendered":
                    print(
                        "[amirify watch] No code-graph topology changes detected; "
                    ...
                    print("[amirify watch] No code-graph topology changes detected; outputs left untouched.")
                return True
        ...
        # Without this, an incremental `amirify update` launders stale names into
        ...
        if stale:
            print(
                f"[amirify watch] community set changed since labeling "
        ...
        if no_change:
            graph_tmp.unlink(missing_ok=True)
            print("[amirify watch] No code-graph changes detected; graph.json/GRAPH_REPORT.md left untouched.")
        else:
            if not _check_shrink(
                force, existing_graph_data, candidate_graph_data,
                tmp=graph_tmp,
        ...
        # See _amirify_root_marker_value for why this isn't always the raw
        # caller-supplied value (#3375).
        (out / ".amirify_root").write_text(
            _amirify_root_marker_value(watch_path), encoding="utf-8"
        )
        ...
        if callflow_files and not no_change:
            try:
                from amirify.callflow_html import write_callflow_html
                for cf in callflow_files:
                    write_callflow_html(
                        graph=out / "graph.json",
                        report=out / "GRAPH_REPORT.md",
                        labels=out / ".amirify_labels.json",
                        output=cf,
                        verbose=False,
                    )
            except Exception as cf_err:
                print(f"[amirify watch] callflow HTML update skipped: {cf_err}")

        if not no_change:
            print(f"[amirify watch] Rebuilt: {G.number_of_nodes()} nodes, "
            ...
            print(f"[amirify watch] {products} updated in {out}")
        return True

    except Exception as exc:
        print(f"[amirify watch] Rebuild failed: {exc}")
        return False
...
def check_update(watch_path: Path) -> bool:
    ...
    re-extraction via `/amirify --update` — this function only signals
    that the update is needed.
    """
    flag = Path(watch_path) / _AMIRIFY_OUT / "needs_update"
    if flag.exists():
        print(f"[amirify check-update] Pending non-code changes in {watch_path}.")
        print("[amirify check-update] Run `/amirify --update` to apply semantic re-extraction.")
    return True


def _notify_only(watch_path: Path) -> None:
    """Write a flag file and print a notification (fallback for non-code-only corpora)."""
    flag = watch_path / _AMIRIFY_OUT / "needs_update"
    flag.parent.mkdir(parents=True, exist_ok=True)
    flag.write_text("1", encoding="utf-8")
    print(f"\n[amirify watch] New or changed files detected in {watch_path}")
    print("[amirify watch] Non-code files changed - semantic re-extraction requires LLM.")
    print("[amirify watch] Run `/amirify --update` in Claude Code to update the graph.")
    print(f"[amirify watch] Flag written to {flag}")
...
def _batch_triggers_rebuild(batch: list[Path]) -> bool:
    ...
    until the next code event or a manual `amirify update` (#2580).
    """
...
def watch(watch_path: Path, debounce: float = 3.0) -> None:
    ...
    to run /amirify --update (LLM extraction required).
    ...
    # Load .amirifyignore patterns ONCE at startup so the handler does not
    ...
    ignore_patterns = _load_amirifyignore(
        watch_root_for_ignore,
        gitignore=_read_build_gitignore(watch_path / _AMIRIFY_OUT),
    )
    ...
    class Handler(FileSystemEventHandler):
        def on_any_event(self, event):
            ...
            # Check .amirifyignore BEFORE the extension/dotfile/out filters so
            ...
            if any(part.startswith(".") for part in filter_parts):
                return
            if _AMIRIFY_OUT in filter_parts:
                return
    ...
    print(f"[amirify watch] Watching {watch_path.resolve()} - press Ctrl+C to stop")
    print(f"[amirify watch] Code changes rebuild graph automatically. "
          f"Doc/image changes require /amirify --update.")
    print(f"[amirify watch] Debounce: {debounce}s")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Watch a folder and auto-update the amirify graph")
