...
from amirify.security import safe_fetch, safe_fetch_text, validate_url
...
def ingest(url: str, target_dir: Path, author: str | None = None, contributor: str | None = None) -> Path:
    """
    Fetch a URL and save it into target_dir as an amirify-ready file.
    ...
    try:
        ...
        if url_type == "youtube":
            from amirify.transcribe import download_audio
...
def save_query_result(
    ...
    Files are stored in memory_dir (typically amirify-out/memory/) with YAML frontmatter
    that amirify's extractor reads as node metadata. This closes the feedback loop:
    ...
    signals: they are written both to the frontmatter (so `amirify reflect` can
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Fetch a URL into an amirify /raw folder")
