"""html — moved verbatim from amirify/export.py."""
...
def _viz_node_limit() -> int:
    """Return the effective viz node limit, honoring AMIRIFY_VIZ_NODE_LIMIT env var.
    ...
    """
    import os
    raw = os.environ.get("AMIRIFY_VIZ_NODE_LIMIT")
...
// (amirify add) can contain a double-quote; dropping the stringified id
...
def _html_document_title(output_path: str) -> str:
    ...
    configured output-dir bare name (``amirify-out`` / ``AMIRIFY_OUT``
    ...
    marker = AMIRIFY_OUT_NAME
    for i, part in enumerate(parts):
        if part == marker or part.startswith("amirify-out"):
            return "/".join(parts[i:])
...
def to_html(
    ...
    if G.number_of_nodes() > limit:
        ...
        raise ValueError(
            ...
            f"(limit: {limit}). Use --no-viz, raise AMIRIFY_VIZ_NODE_LIMIT, "
    ...
    # best-effort from the sibling .amirify_learning.json next to the output
